...
import unittest
from pyAgrum import LabelizedVar, Potential_double, ListPotentials_double
...
class ListTestCase(pyAgrumTestCase):
    ...
    def setUp(self):
        ...
        self.pot1 = Potential_double()

        self.pot2 = Potential_double()
        self.pot2.add(a)
        self.pot2[:] = [0.4, 0.6]

        self.pot3 = Potential_double()
        self.pot3.add(b)
        self.pot3[:] = [0, 1]
...
class TestConstructors(ListTestCase):
    ...
    def testCopyConstructor(self):
        list_pot = ListPotentials_double()
        list_pot2 = ListPotentials_double(list_pot)
...
class TestAssessors(ListTestCase):
    ...
    def testInsertion(self):
        ...
        self.assertFalse(list_pot.exists(Potential_double()))
    ...
    def testDeletion(self):
        ...
        list_pot.append(Potential_double())
        list_pot.append(Potential_double())
        self.assertEqual(list_pot.size(), 2)
        list_pot.clear()
        self.assertTrue(list_pot.empty())
    ...
    def testPersistence(self):
        list_pot = ListPotentials_double()
        potA = Potential_double()
        potB = Potential_double()
        list_pot.append(potA)
        list_pot.append(potB)

        del potA, potB
        for pot in list_pot:
            self.assertTrue(isinstance(pot, Potential_double))
