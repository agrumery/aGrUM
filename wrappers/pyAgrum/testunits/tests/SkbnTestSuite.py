...
class SkbnTestCase(pyAgrumTestCase):
  def test_no_discretization(self):
    ...
    classifier.fit(X,y)
    res=classifier.toDiscretizedDataFrame(X,y)
  ...
  def test_with_nparray(self):
    ...
    classifier.fit(X,y)
    res=classifier.toDiscretizedDataFrame(X,y)
    # X0 and X1 are discretized so the labels starts with '[' but the rest is random (choosen by load_iris)...
    self.assertEquals(res["x0"][149][0],"[")
    self.assertEquals(res["x1"][149][0],"[")

  def test_with_file(self):
    classifier=skbn.BNClassifier()
    classifier.fit(filename=self.agrumSrcDir("miniasia.csv"),targetName="dyspnoea")
    res=classifier.toDiscretizedDataFrame(filename=self.agrumSrcDir("miniasia.csv"))
