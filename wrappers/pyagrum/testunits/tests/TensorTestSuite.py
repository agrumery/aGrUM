...
class TestOperators(pyAgrumTestCase):
  ...
  def testFillWithTensor(self):
    ...
    self.assertAlmostEqual(
      np.max(pABC2.reorganize(["A", "B", "C"]).toarray() - pABC.reorganize(["A", "B", "C"]).toarray()),
      0,
    )
    self.assertAlmostEqual(
      np.max(pABC.reorganize(["A", "B", "C"]).toarray() - pABC2.reorganize(["A", "B", "C"]).toarray()),
      0,
    )
  ...
  def testFillWithTensorAndMap(self):
    ...
    pp.fillWith(p, ["w", "v"])
    self.assertAlmostEqual(
      np.max(p.reorganize(["v", "w"]).toarray() - pp.reorganize(["vv", "ww"]).toarray()),
      0,
    )
...
class TestGetSetItemPaths(pyAgrumTestCase):
  ...
  def testSetItem_fullTensor_nestedList(self):
    ...
    t = self._make_standalone()
    t[{}] = [[10.0, 20.0], [30.0, 40.0], [50.0, 60.0]]
    np.testing.assert_array_almost_equal(t.toarray(), np.array([[10, 20], [30, 40], [50, 60]], dtype=float))
