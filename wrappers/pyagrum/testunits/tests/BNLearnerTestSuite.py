...
class BNLearnerCSVTestCase(pyAgrumTestCase):
    ...
    def testExtendedHillClimbingAccurate(self):
    ...
    def testParameterLearning(self):
        ...
        for i in range(bn.size()):
            ...
            self.assertEqual(
                ...
                set(bn.variable(bn.idFromName(bn2.variable(i).name())).labels()),
            )

        bn3 = learner.learnParameters(bn.dag())
        for i in range(bn.size()):
            self.assertListsAlmostEqual(
                bn3.cpt(i).tolist(), bn2.cpt(i).tolist(), delta=0.01
            )
    ...
    def test_setSliceOrder_with_names(self):
        ...
        learner.setSliceOrder(
            [[SMOKING, LUNG], [BRONCHITIS, VISIT_ASIA], [TUBERCULOSIS]]
        )
        ...
        with self.assertRaises(gum.MissingVariableInDatabase):
            learner.setSliceOrder(
                [[SMOKING, LUNG], [BRONCHITIS, "CRUCRU?"], [TUBERCULOSIS]]
            )

    def test_totalOrder_with_names(self):
        learner = gum.BNLearner(self.agrumSrcDir("asia3.csv"))
        learner.setTotalOrder(
            [
                VISIT_ASIA,
                TUBERCULOSIS,
                SMOKING,
    ...
    def test_dirichlet(self):
        ...
        gum.generateSample(
            bn, 2000, self.agrumSrcDir("dirichlet.csv"), with_labels=True
        )

        bn2 = gum.fastBN("A->B->C->D->E")
        gum.generateSample(
            bn2, 2000, self.agrumSrcDir("database.csv"), with_labels=True
        )
    ...
    def skipped_EM(self):
        ...
        learner.useEM(1e-3, 0.15).EMsetMaxIter(10).EMsetMaxTime(
            200
        ).EMdisableMaxIter().EMdisableMaxTime()
    ...
    def skipped_EM2(self):
        ...
        for i in range(10):
            ...
            for i in range(bn1.size()):
                ...
                self.assertListsAlmostEqual(
                    cpt1.tolist(), cpt2.tolist(), delta=delta * 1.3
                )
    ...
    def testHybridLearning(self):
        ...
        self.assertEqual(
            bn.parents(bn.idFromName("Y")), {bn.idFromName("X"), bn.idFromName("V")}
        )
...
""")
        ...
        self.assertEqual(
            learner.rawPseudoCount(["X", "Z"]), (1.1, 0.1, 2.1, 1.1, 2.1, 1.1)
        )
        self.assertEqual(
            learner.rawPseudoCount(["Y", "Z"]), (0.1, 1.1, 0.1, 3.1, 2.1, 1.1)
        )
...
""")
    ...
    def test_mutualInfo(self):
        ...
        self.assertAlmostEqual(
            learner.mutualInformation(LUNG, SMOKING), 0.0332346154587978, delta=1e-6
        )
        ...
        learner.useNoCorrection()
        self.assertAlmostEqual(
            learner.correctedMutualInformation(LUNG, SMOKING),
            0.0332346154587978,
            delta=1e-6,
        )
        ...
        learner.useMDLCorrection()
        self.assertAlmostEqual(
            learner.correctedMutualInformation(LUNG, SMOKING), 0.0279523, delta=1e-6
        )

        learner.useNMLCorrection()
        self.assertAlmostEqual(
            learner.correctedMutualInformation(LUNG, SMOKING), 0.0292368, delta=1e-6
        )
    ...
    def test_conditionalMutualInfo(self):
        ...
        self.assertAlmostEqual(
            learner.mutualInformation(BRONCHITIS, LUNG, [SMOKING]),
            0.00159046091867418,
            delta=1e-6,
        )
        ...
        learner.useNoCorrection()
        self.assertAlmostEqual(
            learner.correctedMutualInformation(BRONCHITIS, LUNG, [SMOKING]),
            0.00159046091867418,
            delta=1e-6,
        )
    ...
    def testNonRegression_GHCWithInitialDAG(self):
        # Regression: GHC raised DuplicateElement when setInitialDAG was called
        # with a initial DAG learned
        bn_ref = gum.loadBN(self.agrumSrcDir("alarm.bif"), verbose=False)
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as f:
            csv_path = f.name
        try:
            gum.generateSample(bn_ref, 2000, csv_path, with_labels=True)
            learner = gum.BNLearner(csv_path, bn_ref)
            bn2 = gum.fastBN(
                "VENTTUBE{ZERO|LOW|NORMAL|HIGH}->MINVOL{ZERO|LOW|NORMAL|HIGH};HYPOVOLEMIA{TRUE|FALSE}->LVEDVOLUME{LOW|NORMAL|HIGH};CO{LOW|NORMAL|HIGH}->BP{LOW|NORMAL|HIGH};LVFAILURE{TRUE|FALSE}->LVEDVOLUME;PAP{LOW|NORMAL|HIGH}->PULMEMBOLUS{TRUE|FALSE};VENTALV{ZERO|LOW|NORMAL|HIGH}->ARTCO2{LOW|NORMAL|HIGH};PULMEMBOLUS->SHUNT{NORMAL|HIGH};HISTORY{TRUE|FALSE}->LVFAILURE;ERRCAUTER{TRUE|FALSE}->HREKG{LOW|NORMAL|HIGH};VENTLUNG{ZERO|LOW|NORMAL|HIGH}->EXPCO2{ZERO|LOW|NORMAL|HIGH};MINVOL->VENTLUNG;MINVOLSET{LOW|NORMAL|HIGH}->VENTMACH{ZERO|LOW|NORMAL|HIGH};PRESS{ZERO|LOW|NORMAL|HIGH}->KINKEDTUBE{TRUE|FALSE};ARTCO2->EXPCO2;ANAPHYLAXIS{TRUE|FALSE}->TPR{LOW|NORMAL|HIGH};INTUBATION{NORMAL|ESOPHAGEAL|ONESIDED}->SHUNT;ARTCO2->CATECHOL{NORMAL|HIGH};LVEDVOLUME->PCWP{LOW|NORMAL|HIGH};FIO2{LOW|NORMAL}->PVSAT{LOW|NORMAL|HIGH};HR{LOW|NORMAL|HIGH}->HREKG;VENTTUBE->PRESS;DISCONNECT{TRUE|FALSE}->VENTTUBE;VENTALV->PVSAT;LVFAILURE->STROKEVOLUME{LOW|NORMAL|HIGH};HYPOVOLEMIA->STROKEVOLUME;STROKEVOLUME->CO;ERRCAUTER->HRSAT{LOW|NORMAL|HIGH};TPR->CATECHOL;VENTMACH->VENTTUBE;PVSAT->SAO2{LOW|NORMAL|HIGH};TPR->BP;STROKEVOLUME->LVEDVOLUME;CATECHOL->HR;HR->HRSAT;LVEDVOLUME->CVP{LOW|NORMAL|HIGH};HR->HRBP{LOW|NORMAL|HIGH};VENTALV->INTUBATION;SHUNT->SAO2;HRSAT->HREKG;HR->CO;MINVOL->VENTALV;VENTLUNG->VENTALV;ERRLOWOUTPUT{TRUE|FALSE}->HRBP;INSUFFANESTH{TRUE|FALSE}"
            )
            bn4 = (
                learner.useGreedyHillClimbing()
                .useNMLCorrection()
                .useScoreBDeu()
                .setInitialDAG(bn2.dag())
                .learnBN()
            )
            self.assertEqual(bn4.size(), bn_ref.size())
        finally:
            os.unlink(csv_path)
