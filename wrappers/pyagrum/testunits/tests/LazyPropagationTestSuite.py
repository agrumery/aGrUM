...
class LazyPropagationTestCase(pyAgrumTestCase):
  def testEvidenceImpactWithNodeId(self):
    bn = gum.loadBN(self.agrumSrcDir("asia.bgum"), [], verbose=False)  # verbose=False : don't want to see the warnings
  ...
  def testEvidenceImpactWithName(self):
    ...
    self.assertEqual(
      res.extract({"tuberculosis?": 0}), gum.getPosterior(bn, target="visit_to_Asia?", evs={"tuberculosis?": 0})
    )
    self.assertEqual(
  ...
  def testJointMutualInformation(self):
    ...
    byHandJMI += (
      ie.jointPosterior({1, 4}).entropy() + ie.jointPosterior({1, 3}).entropy() + ie.jointPosterior({4, 3}).entropy()
    )
    ...
    byHandJMI += (
      ie.posterior(0).entropy() + ie.posterior(1).entropy() + ie.posterior(2).entropy() + ie.posterior(3).entropy()
    )
