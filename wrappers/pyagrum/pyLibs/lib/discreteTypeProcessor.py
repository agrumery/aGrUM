...
class DiscreteTypeProcessor:
    ...
    def __init__(
        ...
        self.setDiscretizationParameters(
            None, defaultDiscretizationMethod, defaultNumberOfBins
        )
    ...
    @gum.deprecated_arg(
        newA="parameters", oldA="paramDiscretizationMethod", version="2.0.0"
    )
    def setDiscretizationParameters(
        self, variableName: str, method: str, parameters: Any = None
    ):
        """
        ...
        match method:
            case "quantile" | "NML":
                if type(parameters) is not int:
                    raise ValueError(
                        "The parameter for the quantile/NML method must be an integer. You have entered: "
                        + str(parameters)
                    )
            case "kmeans" | "uniform":
            ...
            case "expert":
                if not (
                    isinstance(parameters, list) and all(map(check_float, parameters))
                ):
                    raise ValueError(
                        "The parameter for the expert method must be a list of float. You have entered: "
                        + str(parameters)
                    )
            ...
            case _:
                raise ValueError(
                    ...
                    "CAIM, MDLP, NoDiscretization or expert. You have entered "
                    + str(method)
                )
    ...
    def audit(self, X, y=None):
        ...
        if isinstance(y, str):
            yp = Xp[y]
        elif y is not None and hasattr(
            y, "to_pandas"
        ):  # for instance, polars dataframe
            yp = y.to_pandas()
        else:
            yp = y
    ...
    def _audit(self, X, y=None):
        ...
        for i in range(d):
            ...
            if variable in self.discretizationParametersDictionary.keys():
                ...
                if (
                    self.discretizationParametersDictionary[variable]["method"]
                    != "NoDiscretization"
                    and not isNumeric
                ):
                    raise ValueError(
    ...
    def _discretizationMDLP(self, X, y, possibleValuesX, possibleValuesY):
        ...
        shannonEntropyByLargeInterval = [
            (-1)
            * (
                probabilityClass0 * math.log2(probabilityClass0)
                + probabilityClass1 * math.log2(probabilityClass1)
            )
        ]
        ...
        while any(continueDividingInterval):
            ...
            while currentValues["boundaryIndex"] < len(B):
                ...
                if (
                    position < len(binEdgesIndex)
                    and currentValues["boundaryIndex"] == binEdgesIndex[position]
                ):
                    ...
                    position += 1
                    while (
                        position < len(continueDividingInterval)
                        and not continueDividingInterval[position]
                    ):
                        position += 1
                ...
                currentValues["leftSubintervalClass0"] += nbElementsByIntervalClass0[
                    currentValues["boundaryIndex"]
                ]
                currentValues["leftSubintervalClass1"] += nbElementsByIntervalClass1[
                    currentValues["boundaryIndex"]
                ]

                totalCountLeftInterval = (
                    currentValues["leftSubintervalClass0"]
                    + currentValues["leftSubintervalClass1"]
                )

                probabilityClass0 = (
                    currentValues["leftSubintervalClass0"] / totalCountLeftInterval
                )
                probabilityClass1 = (
                    currentValues["leftSubintervalClass1"] / totalCountLeftInterval
                )
                currentValues["leftSubintervalShannonEntropy"] = 0
                if probabilityClass0 > 0:
                    currentValues["leftSubintervalShannonEntropy"] += (
                        (-1) * probabilityClass0 * math.log2(probabilityClass0)
                    )
                if probabilityClass1 > 0:
                    currentValues["leftSubintervalShannonEntropy"] += (
                        (-1) * probabilityClass1 * math.log2(probabilityClass1)
                    )

                classInformationEntropy = (
                    totalCountLeftInterval / totalCountByLargeInterval[position]
                ) * currentValues["leftSubintervalShannonEntropy"]

                currentValues["rightSubintervalClass0"] = (
                    Class0ByLargeInterval[position]
                    - currentValues["leftSubintervalClass0"]
                )
                currentValues["rightSubintervalClass1"] = (
                    Class1ByLargeInterval[position]
                    - currentValues["leftSubintervalClass1"]
                )
                NRightInterval = (
                    currentValues["rightSubintervalClass0"]
                ...
                probabilityClass0 = (
                    currentValues["rightSubintervalClass0"] / NRightInterval
                )
                probabilityClass1 = (
                    currentValues["rightSubintervalClass1"] / NRightInterval
                )
                currentValues["rightSubintervalShannonEntropy"] = 0
                if probabilityClass0 > 0:
                    currentValues["rightSubintervalShannonEntropy"] += (
                        (-1) * probabilityClass0 * math.log2(probabilityClass0)
                    )
                if probabilityClass1 > 0:
                    currentValues["rightSubintervalShannonEntropy"] += (
                ...
                classInformationEntropy += (
                    NRightInterval / totalCountByLargeInterval[position]
                ) * currentValues["rightSubintervalShannonEntropy"]
    ...
    def _discretizationCAIM(X, y, possibleValuesX, possibleValuesY):
        ...
        while True:
            ...
            for boundaryIndex in range(len(B)):
                if (
                    position < len(binEdgesIndex)
                    and boundaryIndex == binEdgesIndex[position]
                ):
                    position += 1
                    if (
                        Class0ByLargeInterval[position]
                        ...
                        oldCaim = globalCAIM * len(Class0ByLargeInterval) - math.pow(
                            Class0ByLargeInterval[position], 2
                        ) / (
                            Class0ByLargeInterval[position]
                            + Class1ByLargeInterval[position]
                        )
                        ...
                        oldCaim = globalCAIM * len(Class0ByLargeInterval) - math.pow(
                            Class1ByLargeInterval[position], 2
                        ) / (
                            Class0ByLargeInterval[position]
                            + Class1ByLargeInterval[position]
                        )
                ...
                if currentSumClass0 > currentSumClass1:
                    caim = caim + math.pow(currentSumClass0, 2) / (
                        currentSumClass0 + currentSumClass1
                    )
                else:
                    caim = caim + math.pow(currentSumClass1, 2) / (
                        currentSumClass0 + currentSumClass1
                    )
                ...
                if intervalClass0 > intervalClass1:
                    caim = caim + math.pow(intervalClass0, 2) / (
                        intervalClass0 + intervalClass1
                    )
                else:
                    caim = caim + math.pow(intervalClass1, 2) / (
                        intervalClass0 + intervalClass1
                    )
            ...
            if caimMax > globalCAIM:
                ...
                if Class0ByLargeInterval[0] > Class1ByLargeInterval[0]:
                    oldCaim = globalCAIM * len(Class0ByLargeInterval) - math.pow(
                        Class0ByLargeInterval[0], 2
                    ) / (Class0ByLargeInterval[0] + Class1ByLargeInterval[0])
                else:
                    oldCaim = globalCAIM * len(Class0ByLargeInterval) - math.pow(
                        Class1ByLargeInterval[0], 2
                    ) / (Class0ByLargeInterval[0] + Class1ByLargeInterval[0])
    ...
    def _discretizationNML(X, possibleValuesX, kMax=10, epsilon=None):
        ...
        for e in range(1, E):
            ne = binCount[e]
            Bkminus1[e] = -ne * (
                math.log(2 * epsilon * ne)
                - math.log(n * (candidateCutPoints[e] - candidateCutPoints[0]))
            )
    ...
    def _createVariable(self, variableName, X, y=None, possibleValuesY=None):
        ...
        if y is not None:
            X, y = sklearn.utils.check_X_y(
                X, y, dtype=None, accept_sparse=True, ensure_2d=False
            )
        ...
        if (
            ...
            if (
                ...
                or (
                    self.discretizationThreshold < 1
                    and len(foundValuesX) / len(X) > self.discretizationThreshold
                )
            ):
                self.discretizationParametersDictionary[variableName] = {}
            ...
            if (
                ...
                raise ValueError(
                    f"The variable {variableName} is not numeric and cannot be discretized!"
                )
        ...
        if (
            ...
            if "param" in self.discretizationParametersDictionary[variableName]:
                varSyntax = self.discretizationParametersDictionary[variableName][
                    "param"
                ]
            ...
            if is_int_var:
                ...
                if (
                    len(possibleValuesX) == max_v - min_v + 1
                ):  # no hole in the list of int
                    return gum.RangeVariable(variableName, variableName, min_v, max_v)
                else:
                    return gum.IntegerVariable(
                        variableName, variableName, possibleValuesX
                    )

            is_float_var = all(map(check_float, possibleValuesX))
            if is_float_var:
                possibleValuesX = [float(x) for x in possibleValuesX]
                return gum.NumericalDiscreteVariable(
                    variableName, variableName, possibleValuesX
                )
            else:
                return gum.LabelizedVariable(
                    variableName, variableName, [str(v) for v in possibleValuesX]
                )
        else:
            ...
            if (
                ...
                binEdges = self.discretizationParametersDictionary[variableName][
                    "param"
                ]
            elif (
                self.discretizationParametersDictionary[variableName]["method"]
                ...
                self.discretizationParametersDictionary[variableName]["method"] == "NML"
            ):
                ...
                if (
                    ...
                    discre = skp.KBinsDiscretizer(
                        self.discretizationParametersDictionary[variableName]["param"],
                        strategy=self.discretizationParametersDictionary[variableName][
                            "method"
                        ],
                        quantile_method="averaged_inverted_cdf",
                        subsample=None,
    ...
    def _divideIntervalMDLP(
        ...
        if (
            gain
            > (math.log2(totalCountByLargeInterval[position] - 1) + deltaS)
            ...
            Class0ByLargeInterval.insert(
                position + 1, minimalValues["rightSubintervalClass0"]
            )
            Class1ByLargeInterval.insert(
                position + 1, minimalValues["rightSubintervalClass1"]
            )
            continueDividingInterval.insert(position + 1, True)
            totalCountByLargeInterval.insert(
                position + 1,
                minimalValues["rightSubintervalClass0"]
                + minimalValues["rightSubintervalClass1"],
            )
            shannonEntropyByLargeInterval.insert(
                position + 1, minimalValues["rightSubintervalShannonEntropy"]
            )
            ...
            totalCountByLargeInterval[position] = (
                minimalValues["leftSubintervalClass0"]
                + minimalValues["leftSubintervalClass1"]
            )
            shannonEntropyByLargeInterval[position] = minimalValues[
                "leftSubintervalShannonEntropy"
            ]
            ...
            if (
                position > 0
                and minimalValues["boundaryIndex"] - 1 == binEdgesIndex[position - 1]
            ):
                continueDividingInterval[position] = False
            ...
            if (
                position < len(binEdgesIndex) - 1
                and binEdgesIndex[position] + 1 == binEdgesIndex[position + 1]
            ):
                continueDividingInterval[position + 1] = False
            position += 1
        else:
