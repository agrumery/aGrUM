...
/**
 * @file
 * @brief Sentinel typedefs used as %extend return types so that
 * passForPyType.py can map each sentinel name to the correct Python type
 * annotation. All are aliases for PyObject at the C level -- no runtime
 * overhead. Shared via #include (not SWIG %import, which does not carry
 * raw %{ %} code blocks across modules) by every generated SWIG module
 * (core pyagrum + each split submodule) that uses these typemaps.
 *
