...
def BN2dot(bn, size="4", arcvals=None, arcols=None, nodcols=None, cmap=None, showValues=None):
  """
  ...
  :param arcol: a arcMap of values to be shown as bold arcs
  :param arcvals: a arcMap of values to be shown as bold arcs
  :param nodcols: a nodeMap of values to be shown as color nodes (with special colors for 0 and 1)
  ...
  for n in bn.names():
    if nodcols is None or n not in nodcols:
      bgcol = "#444444"
      fgcol = "#FFFFFF"
      res = ""
    else:
      bgcol = _proba2bgcolor(nodcols[n], cmap)
      fgcol = _proba2fgcolor(nodcols[n], cmap)
      res = " : {0:2.5f}".format(nodcols[n] if showValues is None else showValues[n])
