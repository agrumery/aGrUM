...
string Session::getPackageSystemName(string systemNameInRequest){
  GUM_TRACE_VAR(systemNameInRequest);
  GUM_CHECKPOINT;
  GUM_TRACE_VAR(getPackageName());
...
string Session::getPackageSystemFilename(string systemImport){
  TokenizableString str(systemImport);
  vector<string> elts = str.strtoken(".;");
  string repertory;
  if (elts.size() > 1) {
    for (vector<string>::iterator iter = elts.begin(); iter != elts.end(); ++iter) {
      repertory += *iter;
      if ((iter+1) != (elts.end()))
        repertory += "/";
    }
  } else if (elts.size() == 1) {
    repertory += elts.front();
  }
  repertory += ".skool";
  return repertory;
}
...
void Session::act (string requestFile, ostream& o, bool option) {
  ...
  if (getImports().size() != 1) {
    std::cerr << "Expecting only 1 import instruction in query file." << std::cerr;
    return;
  }
  ...
  // Decomposing the system import instruction to retrieve the file's path
  string packageSystemFilename = getPackageSystemFilename(getImports().at(0).getImportPackage());

  // Loading the PRM and the system
  gum::prm::skool::SkoolReader reader;
  try {
    reader.readFile(packageSystemFilename);
  } catch (gum::Exception & e) {
    std::cerr << "An error occured while parsing the skool files: ";
    std::cerr << e.getContent () << std::endl;
    return;
  }

  // Getting the inference engine
