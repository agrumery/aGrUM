...
//=====================
    ...
    prm::PRMInference* inf;
    bool verbose;
    std::ostream& log;
    // We need this when using grounded inference
    BayesNetInference<prm::prm_float>* bn;
    ...
    std::string findAttributeName(const std::string& s, const prm::Instance& instance);
    void addObservation(const std::string& left_val, const std::string& right_val);
    void removeObervation(const std::string name);
    void generateInfEngine(const prm::System& sys);
