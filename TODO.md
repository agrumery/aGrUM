...
1. **`wrappers/pyagrum/pyLibs/**/*.py` — Compléter les annotations de type** | I:3 D:2
...
4. **`pyagrum.Tensor` — accès numpy sans copie** | I:3 D:2

   `pyagrum.Tensor.as_nparray` permet un accès direct sans copie via numpy.
   - Documenter la version numpy minimale requise dans les docs et les requirements.
   - Vérifier si les opérateurs `__getitem__`/`__setitem__` et les opérations de slicing dans
     `tensor.i` peuvent exploiter cette fonctionnalité pour éliminer des copies intermédiaires.

---

### B2. SWIG | I:3 D:3

- **Templated vectors dans `aGrUM_wrap_BASE.i`** | I:2 D:2

  Les templates de vecteurs définis en ligne ~74-77 semblent utilisés uniquement dans certains
  typemaps de `pyagrum.i`. Les déplacer directement dans `pyagrum.i`, ou les remplacer par des
  typemaps utilisant `std::vector` directement.
...
~~**`src/agrum/` — Macros GUM_CONSTRUCTOR/DESTRUCTOR/CONS_CPY/CONS_MOV/OP_CPY/OP_MOV**~~

~~Audit complet et ajout des macros de comptage d'instances dans toute la hiérarchie C++
(branche `feature/moveConstructor`). Inclut l'ajout des move constructors manquants
sur toute la chaîne NodeGraphPart → DAG → BayesNet.~~
