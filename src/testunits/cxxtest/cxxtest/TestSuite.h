...
#define TS_ABORT() CxxTest::doAbortTest()
  ...
  void doAssertEquals(const char *file, unsigned line, const char *xExpr, const X &x,
  ...
  void doAssertLessThan(const char *file, unsigned line, const char *xExpr,
    ...
    if (!lessThan(x, y)) {
      ...
      tracker().failedAssertLessThan(file, line, xExpr, yExpr, TS_AS_STRING(x),
                                     TS_AS_STRING(y));
      TS_ABORT();
    }
  }
  ...
  template <class X, class Y>
  void doAssertLessThanEquals(const char *file, unsigned line, const char *xExpr,
                              X x, const char *yExpr, Y y, const char *message) {
  ...
  void doAssertRelation(const char *file, unsigned line, const char *rExpr,
                        const R &r, const char *xExpr, const X &x, const char *yExpr,
                        const Y &y, const char *message) {
    if (!r(x, y)) {
      if (message)
        tracker().failedTest(file, line, message);
