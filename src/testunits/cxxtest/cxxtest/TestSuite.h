...
#define TS_ABORT() CxxTest::doAbortTest()
  ...
  template <class X, class Y> bool equals( const X& x, const Y& y ) {
    return ( x == y );
  }

  template <class X, class Y>
  void doAssertEquals( const char* file, unsigned line, const char* xExpr,
                       const X& x, const char* yExpr, const Y& y,
                       const char* message ) {
    if ( !equals( x, y ) ) {
      if ( message )
        tracker().failedTest( file, line, message );

      tracker().failedAssertEquals( file, line, xExpr, yExpr, TS_AS_STRING( x ),
                                    TS_AS_STRING( y ) );
      TS_ABORT();
    }
  }
  ...
  template <class X, class Y> bool differs( const X& x, const Y& y ) {
    return !( x == y );
  }

  template <class X, class Y>
  void doAssertDiffers( const char* file, unsigned line, const char* xExpr,
                        const X& x, const char* yExpr, const Y& y,
                        const char* message ) {
    if ( !differs( x, y ) ) {
      if ( message )
      ...
      tracker().failedAssertDiffers( file, line, xExpr, yExpr,
                                     TS_AS_STRING( x ) );
      TS_ABORT();
    }
  }

  template <class X, class Y> bool lessThan( const X& x, const Y& y ) {
    return ( x < y );
  }

  template <class X, class Y>
  void doAssertLessThan( const char* file, unsigned line, const char* xExpr,
                         const X& x, const char* yExpr, const Y& y,
                         const char* message ) {
    if ( !lessThan( x, y ) ) {
      if ( message )
      ...
      tracker().failedAssertLessThan( file, line, xExpr, yExpr,
                                      TS_AS_STRING( x ), TS_AS_STRING( y ) );
      TS_ABORT();
    }
  }

  template <class X, class Y> bool lessThanEquals( const X& x, const Y& y ) {
    return ( x <= y );
  }

  template <class X, class Y>
  void doAssertLessThanEquals( const char* file, unsigned line,
                               const char* xExpr, X x, const char* yExpr, Y y,
                               const char* message ) {
  ...
  void doAssertPredicate( const char* file, unsigned line, const char* pExpr,
                          const P& p, const char* xExpr, const X& x,
                          const char* message ) {
    if ( !p( x ) ) {
      if ( message )
        tracker().failedTest( file, line, message );

      tracker().failedAssertPredicate( file, line, pExpr, xExpr,
                                       TS_AS_STRING( x ) );
      TS_ABORT();
    }
  }
  ...
  void doAssertRelation( const char* file, unsigned line, const char* rExpr,
                         const R& r, const char* xExpr, const X& x,
                         const char* yExpr, const Y& y, const char* message ) {
    if ( !r( x, y ) ) {
      if ( message )
        tracker().failedTest( file, line, message );

      tracker().failedAssertRelation( file, line, rExpr, xExpr, yExpr,
                                      TS_AS_STRING( x ), TS_AS_STRING( y ) );
      TS_ABORT();
    }
  }

  template <class X, class Y, class D> bool delta( X x, Y y, D d ) {
    return ( ( y >= x - d ) && ( y <= x + d ) );
  }

  template <class X, class Y, class D>
  void doAssertDelta( const char* file, unsigned line, const char* xExpr, X x,
                      const char* yExpr, Y y, const char* dExpr, D d,
                      const char* message ) {
    if ( !delta( x, y, d ) ) {
      if ( message )
        tracker().failedTest( file, line, message );
...
#define ___ETS_ASSERT_SAME_DATA( f, l, x, y, s, m )                            \
  CxxTest::doAssertSameData( ( f ), ( l ), #x, ( x ), #y, ( y ), #s, ( s ),    \
                             ( m ) )
...
#define ___ETS_ASSERT_RELATION( f, l, r, x, y, m )                             \
  CxxTest::doAssertRelation( ( f ), ( l ), #r, r(), #x, ( x ), #y, ( y ),      \
                             ( m ) )
...
#define ___TS_ASSERT_THROWS( f, l, e, t, m )                                   \
  {                                                                            \
    ...
    if ( !_ts_threw_expected ) {                                               \
      CxxTest::doFailAssertThrows( ( f ), ( l ), #e, #t, _ts_threw_else,       \
                                   ( m ) );                                    \
    }                                                                          \
  }
...
#define ___TS_ASSERT_THROWS_NOTHING( f, l, e, m )                              \
  {                                                                            \
    ...
    _TS_CATCH_ABORT( { throw; } )                                              \
    _TS_LAST_CATCH(                                                            \
        { CxxTest::doFailAssertThrowsNot( ( f ), ( l ), #e, ( m ) ); } )       \
  }
...
#define CXXTEST_COMPARISONS( CXXTEST_X, CXXTEST_Y, CXXTEST_T )                 \
  inline bool equals( const CXXTEST_X& x, const CXXTEST_Y& y ) {               \
    return ( ( (CXXTEST_T)x ) == ( (CXXTEST_T)y ) );                           \
  }                                                                            \
  inline bool equals( const CXXTEST_Y& y, const CXXTEST_X& x ) {               \
    return ( ( (CXXTEST_T)y ) == ( (CXXTEST_T)x ) );                           \
  }                                                                            \
  inline bool differs( const CXXTEST_X& x, const CXXTEST_Y& y ) {              \
    return ( ( (CXXTEST_T)x ) != ( (CXXTEST_T)y ) );                           \
  }                                                                            \
  inline bool differs( const CXXTEST_Y& y, const CXXTEST_X& x ) {              \
    return ( ( (CXXTEST_T)y ) != ( (CXXTEST_T)x ) );                           \
  }                                                                            \
  inline bool lessThan( const CXXTEST_X& x, const CXXTEST_Y& y ) {             \
    return ( ( (CXXTEST_T)x ) < ( (CXXTEST_T)y ) );                            \
  }                                                                            \
  inline bool lessThan( const CXXTEST_Y& y, const CXXTEST_X& x ) {             \
    return ( ( (CXXTEST_T)y ) < ( (CXXTEST_T)x ) );                            \
  }                                                                            \
  inline bool lessThanEquals( const CXXTEST_X& x, const CXXTEST_Y& y ) {       \
    return ( ( (CXXTEST_T)x ) <= ( (CXXTEST_T)y ) );                           \
  }                                                                            \
  inline bool lessThanEquals( const CXXTEST_Y& y, const CXXTEST_X& x ) {       \
    return ( ( (CXXTEST_T)y ) <= ( (CXXTEST_T)x ) );                           \
  }

#define CXXTEST_INTEGRAL( CXXTEST_T )                                          \
  CXXTEST_COMPARISONS( signed CXXTEST_T, unsigned CXXTEST_T,                   \
                       unsigned CXXTEST_T )
...
#define CXXTEST_SMALL_BIG( CXXTEST_SMALL, CXXTEST_BIG )                        \
  CXXTEST_COMPARISONS( signed CXXTEST_SMALL, unsigned CXXTEST_BIG,             \
                       unsigned CXXTEST_BIG )                                  \
  CXXTEST_COMPARISONS( signed CXXTEST_BIG, unsigned CXXTEST_SMALL,             \
                       unsigned CXXTEST_BIG )
