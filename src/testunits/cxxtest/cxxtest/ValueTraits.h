...
#define TS_AS_STRING(x) CxxTest::traits(x).asString()
  ...
  //
  template <class T> inline ValueTraits<T> traits(T t) { return ValueTraits<T>(t); }
...
#define CXXTEST_NON_NEGATIVE(Type)                                                  \
  CXXTEST_TEMPLATE_INSTANTIATION inline bool negative<Type>(Type) { return false; } \
  CXXTEST_TEMPLATE_INSTANTIATION inline Type abs<Type>(Type value) { return value; }
  ...
  char *numberToString(N n, char *s, N base = 10, unsigned skipDigits = 0,
    ...
    for (; digit >= 1 && skipDigits;
         n -= digit * digitValue, digit /= base, --skipDigits)
      digitValue = (unsigned)(n / digit);

    for (; digit >= 1 && maxDigits;
         n -= digit * digitValue, digit /= base, --maxDigits)
...
//
  ...
  class ValueTraits<const char> {
    ...
    public:
    ValueTraits(char c) {
      copyString(charToString(c, copyString(_asString, "'")), "'");
    }
    const char *asString(void) const { return _asString; }
  };
  ...
  class ValueTraits<const double> {
    ...
    ValueTraits(double t) {
      (requiredDigitsOnLeft(t) > MAX_DIGITS_ON_LEFT) ? hugeNumber(t)
                                                     : normalNumber(t);
    }
  ...
  namespace CxxTest {                                                               \
    CXXTEST_TEMPLATE_INSTANTIATION class ValueTraits<TYPE> {                        \
      ...
      ValueTraits(TYPE value) {                                                     \
        _value = value;                                                             \
        numberToString<unsigned long int>(_value,                                   \
                                          copyString(_fallback, "(" #TYPE ")"));    \
      }                                                                             \
