...
namespace CxxTest {
  ...
  class AgrumErrorFormatter : public TestListener {
    ...
    public:
    AgrumErrorFormatter(OutputStream *o, const char *preLine = ":",
                        const char *postLine = "")
    ...
    void enterWorld(const WorldDescription & /*desc*/) {
      ...
      __totalTime = 0.0;
      (*_o) << endl << "======================" << endl << "Agrum Test Unit Module"
            << endl << "======================" << endl << endl << "Running "
            << totalTests << endl;
      _o->flush();
      _dotting = true;
      _reported = false;
    ...
    void leaveWorld(const WorldDescription &desc) {
      ...
      (*_o) << endl << "## Profiling : " << (unsigned long)(1000.0 * __totalTime)
            << " ms ##" << endl;
      ...
      (*_o) << "Success rate: " << (numPassed * 100 / desc.numTotalTests()) << "%"
            << endl;
    }
    ...
    void failedAssertSameData(const char *file, unsigned line, const char *xStr,
                              const char *yStr, const char *sizeStr, const void *x,
    ...
    void failedAssertDelta(const char *file, unsigned line, const char *xStr,
      ...
      stop(file, line) << "Error: Expected (" << xStr << " == " << yStr << ") up to "
                       << dStr << " (" << d << "), found (" << x << " != " << y
                       << ")" << endl;
    }
    ...
    void failedAssertLessThan(const char *file, unsigned line, const char *xStr,
                              const char *yStr, const char *x, const char *y) {
      stop(file, line) << "Error: Expected (" << xStr << " < " << yStr
                       << "), found (" << x << " >= " << y << ")" << endl;
    }
    ...
    void failedAssertRelation(const char *file, unsigned line, const char *relation,
                              const char *xStr, const char *yStr, const char *x,
                              const char *y) {
    ...
    void failedAssertThrows(const char *file, unsigned line, const char *expression,
