...
namespace gum_tests {
  ...
  GUM_TEST(NullEffect_DSeparation) {
  ...
  GUM_TEST(Backdoor_WithObservedConfounder) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(Frontdoor_SimpleLatent) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
    auto bn = gum::BayesNet< double >::fastPrototype("X->Z->Y");
    bn.generateCPTs();
  ...
  GUM_TEST(Backdoor_Trivial_XtoY) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(WHY19_Hedge_Unidentifiable) {
  ...
  GUM_TEST(PairwiseOverlapsThrow) {
    // Repeat with directDoCalculus: force do-calculus path, should throw as above
    ...
    CHECK_THROWS((gum::CausalImpact< double >(cm, names({"B"}), names({"A"}), names({"B"}), true)));
    CHECK_THROWS((gum::CausalImpact< double >(cm, names({"B"}), names({"A"}), names({"A"}), true)));
  }

  GUM_TEST(AccentsInVariables) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(SimpsonParadox_BackdoorAverage) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
    auto bn = gum::BayesNet< double >::fastPrototype(
  ...
  GUM_TEST(Tobacco_DirectCausality_TrivialBackdoor) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(Tobacco_Frontdoor_WithMediatorTar) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(Tobacco_Frontdoor_Latex_Primes_And_Parentheses) {
    auto bn = gum::BayesNet< double >::fastPrototype("Smoking->Tar->Cancer;Smoking->Cancer");

    gum::LatentDescriptorVector lat;
    lat.emplace_back("Genotype",
                     std::vector< gum::NodeId >{bn.idFromName("Smoking"), bn.idFromName("Cancer")});
  ...
  GUM_TEST(Tobacco_Unidentifiable_Hedge) {
    // Repeat with directDoCalculus: force do-calculus path, should throw as above
    auto bn = gum::BayesNet< double >::fastPrototype("Smoking->Cancer");
    bn.generateCPTs();
  ...
  GUM_TEST(DoCalculus_WXZYW_BackdoorW) {
    // Repeat with directDoCalculus: force do-calculus path, should match above
  ...
  GUM_TEST(CustomDoOperatorLatex) {
    auto bn = gum::BayesNet< double >::fastPrototype("X->Y");
    bn.generateCPTs();
    gum::CausalModel< double >  cm(bn);
    gum::CausalImpact< double > ci(cm, names({"Y"}), names({"X"}), StrSet{});
  ...
  GUM_TEST(causalImpact_FreeFn_matches_Class_on_deterministic_chain) {
    // BN: X{a|b} -> Y{a|b}
  ...
  GUM_TEST(causalImpact_FreeFn_no_effect_independent_roots) {
  ...
  GUM_TEST(FromNotebookFrontdoor) {
  ...
  GUM_TEST(FromNotebookTobacco) {
    auto bn           = gum::BayesNet< double >::fastPrototype("A->B");
    auto causal_model = gum::CausalModel< double >(bn, {{"L1", bn.ids({"A", "B"})}});
    const auto [ci, tensor, explanation]
        = gum::causalImpact< double >(causal_model,
