...
namespace gum_tests {
  ...
  public:
    ...
    */
    ...
    static BN makeRefBN() {
      return BN::fastPrototype("A->C->E->F->G;F<-D<-B->C;H->E");
    }
    ...
    GUM_ACTIVE_TEST( test_general_dsep_basic_paths ) {
      ...
      auto A = ns(bn, {"A"}); auto B = ns(bn, {"B"}); auto C = ns(bn, {"C"});
    ...
    GUM_ACTIVE_TEST( test_backdoor_examples_from_pyagrum_suite ) {
      ...
      hasAllBackDoors({{"N5"}, {"N6"}},
                      "N0[1,3]<-N1->N2[1,4];N0<-N3[0,3]->N2;N0<-N4[1,4]->N2;N2->N0;N1->N4;N1<-N5->N6->N0",
                      "N1",
                      "N0");

      hasNoBackDoor("N0[1,3]<-N1->N2[1,4];N0<-N3[0,3]->N2;N0<-N4[1,4]->N2;N2->N0;N1->N4;N1<-N5->N6<-N0",
                    "N1",
                    "N0");
    ...
    GUM_ACTIVE_TEST( test_barren_nodes_and_reduction ) {
      ...
      gum::NodeSet targets; targets.insert(idA); targets.insert(idC);
