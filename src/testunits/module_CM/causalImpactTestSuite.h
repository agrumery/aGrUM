...
namespace gum_tests {
  ...
  class GUM_TEST_SUITE(CausalImpact) {
    ...
    GUM_ACTIVE_TEST(test_SimpsonParadox_BackdoorAverage) {
      ...
      TS_ASSERT_DELTA(got.get(_inst(got, {{"Patient", "Healed"}, {"Drug", "With"}})), 0.45, 1e-10);

      TS_ASSERT_DELTA(got.get(_inst(got, {{"Patient", "Healed"}, {"Drug", "Without"}})),
                      0.60,
                      1e-10);
      ...
      TS_ASSERT_DELTA(got_dc.get(_inst(got_dc, {{"Patient", "Healed"}, {"Drug", "With"}})),
                      0.45,
                      1e-10);
      TS_ASSERT_DELTA(got_dc.get(_inst(got_dc, {{"Patient", "Healed"}, {"Drug", "Without"}})),
                      0.60,
                      1e-10);
    }
