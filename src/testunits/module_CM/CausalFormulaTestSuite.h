...
namespace gum_tests {
  ...
  GUM_TEST(observational_marginal) {
    ...
    GUM_CHECK_TENSOR_ALMOST_EQUALS(result_pot, expected_pot);
  }

  GUM_TEST(observational_conditional) {
    // Setup: A simple X -> Y model
    auto bn = gum::BayesNet< double >::fastPrototype("X{0|1}->Y{0|1}");
    bn.cpt(bn.idFromName("X")).fillWith({0.2, 0.8});
    bn.cpt(bn.idFromName("Y")).fillWith({0.9, 0.1, 0.4, 0.6});
    ...
    auto ast_denominator
        = std::make_unique< gum::ASTsum< double > >("Y", std::move(ast_denominator_leaf));

    auto ast = std::make_unique< gum::ASTdiv< double > >(std::move(ast_numerator),
                                                         std::move(ast_denominator));
  ...
  GUM_TEST(interventional_backdoor) {
    ...
    auto ast_p_y_given_xz
        = std::make_unique< gum::ASTposteriorProba< double > >(NameSet{"Y"}, NameSet{"X", "Z"});
    auto ast_p_z     = std::make_unique< gum::ASTjointProba< double > >(NameSet{"Z"});
    auto ast_product = std::make_unique< gum::ASTmult< double > >(std::move(ast_p_y_given_xz),
                                                                  std::move(ast_p_z));
  ...
  GUM_TEST(error_handling_unknown_variable) {
    ...
    CHECK_THROWS_AS(gum::CausalFormula< double >(cm, std::move(ast), {"Y"}, {"W"}),
                    const gum::NotFound&);
  }

  GUM_TEST(LatexQueryRendering) {
    ...
    // Case 2: P(Y | do(X), Z)
    {
      gum::CausalFormula< double > cf(cm,
                                      dummyAST->copy(),
                                      NameSet{"Y"},
                                      NameSet{"X"},
                                      NameSet{"Z"});
  ...
  GUM_TEST(ToString_DelegatesToRoot) {
