...
namespace gum_tests {
  ...
  GUM_TEST(ConditionalSimplification) {
  ...
  GUM_TEST(Eval_Posterior_CPT_Shortcut) {
  ...
  GUM_TEST(Eval_Posterior_NoConditioning) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B->C<-D");
  ...
  GUM_TEST(Eval_Posterior_WithConditioning) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B->C<-D");
  ...
  GUM_TEST(Eval_BinaryOps_SameScope) {
  ...
  GUM_TEST(Throws_On_Empty_VarSets) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B");

    gum::Set< std::string > empty, B;
    B.insert("B");
    CHECK_THROWS_AS((gum::ASTposteriorProba< double >(bn, empty, B)), const gum::InvalidArgument&);
    CHECK_THROWS_AS((gum::ASTjointProba< double >(empty)), const gum::InvalidArgument&);
  }

  GUM_TEST(Copy_DeepCopy) {
  ...
  GUM_TEST(ProductOfTrees_EdgeCases) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B");
  ...
  GUM_TEST(Eval_Compose_DifferentScopes) {
  ...
  GUM_TEST(Eval_Sum_Flattening_Equivalence) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B->C<-D");
  ...
  GUM_TEST(Deterministic_Ordering) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B->C<-D");
  ...
  GUM_TEST(Latex_like_pyagrum) {
    ...
    // division: note the intentional leading space before \frac
    {
      ...
      auto                  p2 = std::make_unique< gum::ASTposteriorProba< double > >(bn, sA, sC);
      gum::ASTdiv< double > op(std::move(p1), std::move(p2));
      CHECK_EQ((op.toLatex()), (" \\frac {P\\left(A,C\\mid B\\right)}{P\\left(A\\mid C\\right)}"));
    }
  ...
  GUM_TEST(Latex_ConditionalSimplification) {
    auto m = gum::BayesNet< double >::fastPrototype("z2->x->z1->y;z2->z1;z2->z3->y");
    gum::Set< std::string > y, knw;
    y.insert("y");
    knw.insert("x");
