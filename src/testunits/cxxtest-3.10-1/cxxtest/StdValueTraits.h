...
#  ifdef _CXXTEST_PARTIAL_TEMPLATE_SPECIALIZATION
  ...
  template < class First, class Second >
      class ValueTraits < CXXTEST_STD(pair) < First,
      Second >>: public StdTraitsBase {
    public:
  ...
  template < class Key, class Value >
      class ValueTraits < CXXTEST_STD(map) < Key,
      Value >>: public StdTraitsBase {
    public:
  ...
  //
  template < class Key, class Value >
      class ValueTraits < CXXTEST_STD(multimap) < Key,
      Value >>: public StdTraitsBase {
    public:
  ...
  template < class Number >
    ...
    ValueTraits(const CXXTEST_STD(complex) < Number > &c) {
      if (!c.imag()) *this << TS_AS_STRING(c.real());
      else if (!c.real()) *this << "(" << TS_AS_STRING(c.imag()) << " * i)";
      else *this << "(" << TS_AS_STRING(c.real()) << " + " << TS_AS_STRING(c.imag()) << " * i)";
    }
  };
