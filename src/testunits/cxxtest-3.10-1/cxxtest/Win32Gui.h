...
namespace CxxTest {
  class Win32Gui: public GuiListener {
    ...
    void redBar() {
      if (_startMinimized) showMainWindow(SW_SHOWNORMAL);
    ...
    void parseCommandLine(int argc, char** argv) {
      ...
      for (int i = 1; i < argc; ++i) {
        if (!lstrcmpA(argv[i], "-minimized")) _startMinimized = true;
        else if (!lstrcmpA(argv[i], "-keep")) _keep = true;
        else if (!lstrcmpA(argv[i], "-title") && (i + 1 < argc)) _title = argv[++i];
      }
    }
    ...
    void centerMainWindow() {
      ...
      if (windowHeight < minimumHeight) windowHeight = minimumHeight;
    ...
    void getScreenArea(RECT& area) {
      if (!getScreenAreaWithoutTaskbar(area)) getWholeScreenArea(area);
    }
    ...
    void messageLoop() {
      ...
      while (BOOL haveMessage = GetMessage(&message, nullptr, 0, 0))
        if (haveMessage != -1) DispatchMessage(&message);
    }
    ...
    static LRESULT CALLBACK windowProcedure(HWND   window,
                                            ...
                                            WPARAM wParam,
                                            LPARAM lParam) {
      if (message == WM_CREATE) setUp(window, (LPCREATESTRUCT)lParam);
    ...
    HICON loadStandardIcon(LPCTSTR icon) { return LoadIcon((HINSTANCE) nullptr, icon); }
    ...
    void
        setCaption(const char* a = "", const char* b = "", const char* c = "", const char* d = "") {
      unsigned length = lstrlenA(_title) + sizeof(" - ") + lstrlenA(a) + lstrlenA(b) + lstrlenA(c)
                      + lstrlenA(d);
    ...
    void setStatusTime(unsigned part, DWORD start) {
      ...
      if (hours) wsprintfA(_timeString, "%u:%02u:%02u", hours, minutes, seconds);
      else wsprintfA(_timeString, "%02u:%02u", minutes, seconds);
    ...
    void setSummaryStatusBar() {
      ...
      if (tracker().failedTests())
        wsprintfA(_statusTestsDone,
                  ...
                  _strTotalTests,
                  tests);
      else wsprintfA(_statusTestsDone, "%s %s passed", _strTotalTests, tests);
