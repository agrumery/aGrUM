  ...
  class QtGui: public GuiListener {
    ...
    void parseCommandLine(int argc, char** argv) {
      ...
      for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-minimized") _startMinimized = true;
        else if (arg == "-keep") _keep = true;
        else if (arg == "-title" && (i + 1 < argc)) _title = argv[++i];
      }
    }
    ...
    void createWindow(const WorldDescription& wd) {
      ...
      if (_startMinimized) showMinimized();
      else showNormal();
    }
