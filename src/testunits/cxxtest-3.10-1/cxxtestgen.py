...
def parseCommandline():
  ...
  try:
    options, patterns = getopt.getopt(
      sys.argv[1:],
      "o:r:",
      [
        "version",
...
unusedsuite_re = re.compile(
  r"\bclass\s+\[\[maybe_unused\]\]\s+(\w+)\s*:\s*public\s+((::)?\s*CxxTest\s*::\s*)?TestSuite\b"
)
...
def scanLineForTest(suite, lineNo, line):
  ...
  m = test_re1.search(line)
  if m:
    addTest(suite, "test" + m.group(1), lineNo)
  else:
    m = test_re2.search(line)
    if m:
...
def fixBlockLine(suite, lineNo, line):
  ...
  return re.sub(r"\b(E?TSM?_(ASSERT[A-Z_]*|FAIL))\s*\(", r"_\1(%s,%s," % (suite["cfile"], lineNo), line, 0)
...
def verifySuite(suite):
  ...
  if "create" in suite and "destroy" not in suite:
    abort("%s:%s: Suite %s has createSuite() but no destroySuite()" % (suite["file"], suite["create"], suite["name"]))
  if "destroy" in suite and "create" not in suite:
    abort("%s:%s: Suite %s has destroySuite() but no createSuite()" % (suite["file"], suite["destroy"], suite["name"]))
...
def writeTestDescription(output, suite, test):
  ...
  if not noStaticInit:
    output.write(
      ' %s() : CxxTest::RealTestDescription( %s, %s, %s, "%s" ) {}\n'
...
def writeStaticDescription(output, suite):
  ...
  if not noStaticInit:
    output.write(
      '( %s, %s, "%s", %s, %s )' % (suite["cfile"], suite["line"], suite["name"], suite["object"], suite["tlist"])
    )
  output.write(";\n\n")
