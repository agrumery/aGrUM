	...
	class CredalNetTestSuite : public CxxTest::TestSuite {
		...
		public:
			...
			void testCredalNetByLP () {
				...
				std::vector< std::vector< gum::credal::LpInterface > > lps ( 3 );
				std::vector< std::vector< std::vector< std::vector< double > > > > lps_sols( 3 );
				
				lps[ 0 ] = std::vector< gum::credal::LpInterface > ( 1 ); // A lp
				lps_sols[ 0 ] = std::vector< std::vector< std::vector< double > > >( 1 );
				
				lps[ 1 ] = std::vector< gum::credal::LpInterface > ( 1 ); // B lp
				lps_sols[ 1 ] = std::vector< std::vector< std::vector< double > > >( 1 );
				
				lps[ 2 ] = std::vector< gum::credal::LpInterface > ( 9 ); // C lps
				lps_sols[ 2 ] = std::vector< std::vector< std::vector< double > > >( 9 );
				
				/// A - intervals ( not a linear - vacuous mixture, i.e. exists i : max Pxi + sum_(j != i) min Pxj != 1 )
				std::vector< gum::credal::LpCol > A = lps[ 0 ][ 0 ].addCols( 3 ); 
				...
				for ( const auto & id : ids ) {
					...
					while ( ! ins.end() ) {
						...
						std::vector< std::vector< double > > vertices = lps[ id ][ entry ].solve();
						
						unsigned int sols_size = lps_sols[ id ][ entry ].size();
						
						if ( vertices.size() != sols_size ) {
							TS_ASSERT ( false );
							std::cout << "node id : " << id << std::endl;
							std::cout << "entry : " << entry << " : " << ins << std::endl;
							std::cout << "vertices : " << vertices << "\n\t != \n" << lps_sols[ id ][ entry ] << std::endl;
						}
						...
						if ( ! r ) {
							TS_ASSERT ( false );
							std::cout << "not a bijection between lps found solutions and (hardcoded) solutions" << std::endl;
						}
						
						cn->setCPT( id, ins, vertices );
						
						ins += cn->domainSize( id ); // next instantiation without taking into account the head variable; ins increased by cardinality of head variable
						entry++;
					}
