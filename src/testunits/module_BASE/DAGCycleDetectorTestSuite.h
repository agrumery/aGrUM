  ...
  class DAGCycleDetectorTestSuite : public CxxTest::TestSuite {
    ...
    void __createChanges(
        ...
        switch ( chgt_type ) {
          case 0: {  // deletions
            ...
            for ( auto iter = gg.arcs().begin(); iter != gg.arcs().end();
                  ++iter, --nb_del_arc ) {
              if ( !nb_del_arc ) {
                changes.push_back( gum::DAGCycleDetector::ArcDel(
                    iter->tail(), iter->head() ) );
          ...
          case 1: {  // additions
            ...
            while ( true ) {
              ...
              if ( ( node1 != node2 ) && !gg.existsArc( node1, node2 ) &&
                ...
                changes.push_back(
                    gum::DAGCycleDetector::ArcAdd( node1, node2 ) );
          ...
          case 2: {  // reversal
            ...
            for ( auto iter = gg.arcs().begin(); iter != gg.arcs().end();
              ...
              if ( !nb_del_arc ) {
                ...
                changes.push_back(
                    gum::DAGCycleDetector::ArcReverse( tail, head ) );
      ...
      for ( auto iter = deletions.cbegin(); iter != deletions.cend(); ++iter ) {
        del_add_changes.push_back( gum::DAGCycleDetector::ArcDel(
            iter.key().tail(), iter.key().head() ) );
      }
      for ( auto iter = additions.cbegin(); iter != additions.cend(); ++iter ) {
        del_add_changes.push_back( gum::DAGCycleDetector::ArcAdd(
            iter.key().tail(), iter.key().head() ) );
      }
    }
      ...
      for ( gum::Idx i = 15; i < 25; ++i ) {
        ...
        for ( gum::Idx j = 0; j < 20; ++j ) {
          ...
          bool hasCycle = false;
          {
            ...
            try {
              gum::DAG dag = g;
              for ( auto iter = g.arcs().begin(); iter != g.arcs().end();
                    ++iter ) {
                dag.eraseArc( *iter );
              }
              for ( auto iter = gg.arcs().begin(); iter != gg.arcs().end();
          ...
          TS_ASSERT( detector.hasCycleFromModifications( changes ) ==
                     hasCycle );
        }
      }
    }
      ...
      for ( gum::Idx i = 7; i < 15; ++i ) {
        ...
        for ( gum::Idx j = 0; j < 30; ++j ) {
          ...
          for ( auto& chgt : changes ) {
            switch ( chgt.type() ) {
              case gum::DAGCycleDetector::ChangeType::ARC_DELETION:
                TS_ASSERT( detector1.hasCycleFromDeletion( chgt.tail(),
                                                           chgt.head() ) ==
                           detector2.hasCycleFromModifications( changes ) );
                break;

              case gum::DAGCycleDetector::ChangeType::ARC_ADDITION:
                TS_ASSERT( detector1.hasCycleFromAddition( chgt.tail(),
                                                           chgt.head() ) ==
                           detector2.hasCycleFromModifications( changes ) );
                break;

              case gum::DAGCycleDetector::ChangeType::ARC_REVERSAL:
                TS_ASSERT( detector1.hasCycleFromReversal( chgt.tail(),
                                                           chgt.head() ) ==
                           detector2.hasCycleFromModifications( changes ) );
                break;
