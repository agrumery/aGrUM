    ...
    gum::DAG __createDAG ( unsigned int nb_nodes, unsigned int nb_arcs ) {
      ...
      while ( nb_arcs ) {
        ...
        if ( ( id1 != id2 ) && ( ! dag.existsArc ( id1, id2 ) ) ) {
          try { dag.insertArc ( id1, id2 ); --nb_arcs; }
          catch ( gum::Exception& ) {}
        }
      }
    ...
    ( const gum::DAG& g,
        ...
        case 0: // deletions
          {
            ...
            for ( auto iter = gg.arcs().begin();
              ...
              if ( ! nb_del_arc ) {
                changes.push_back ( gum::DAGCycleDetector::ArcDel ( iter->tail (),
                                                                    iter->head () ) );
        ...
        case 1: // additions
          {
            ...
            while ( true ) {
              ...
              if ( ( node1 != node2 ) &&
                   ! gg.existsArc ( node1, node2 ) && ! gg.existsArc ( node2, node1 ) ) {
                changes.push_back ( gum::DAGCycleDetector::ArcAdd ( node1, node2 ) );
                gg.insertArc ( node1, node2 );
                break;
              }
            }
          }
      ...
      for ( auto iter = deletions.cbegin (); iter != deletions.cend (); ++iter ) {
        del_add_changes.push_back
          ( gum::DAGCycleDetector::ArcDel ( iter.key().tail (),
                                            iter.key().head () ) );
      }
      for ( auto iter = additions.cbegin (); iter != additions.cend (); ++iter ) {
        del_add_changes.push_back
          ( gum::DAGCycleDetector::ArcAdd ( iter.key().tail (),
                                            iter.key().head () ) );
      }
    }
        ...
        for ( unsigned int j = 0; j < 20; ++j ) {
          ...
          bool hasCycle = false;
          {
            ...
            for ( auto& chgt : del_add_changes ) {
              ...
              case gum::DAGCycleDetector::ChangeType::ARC_ADDITION:
                gg.insertArc ( chgt.tail (), chgt.head () );
                break;
            ...
            try {
              gum::DAG dag = g;
              for ( auto iter = g.arcs().begin ();
                    iter != g.arcs().end (); ++iter ) {
                dag.eraseArc ( *iter );
              }
              for ( auto iter = gg.arcs().begin ();
                    iter != gg.arcs().end (); ++iter ) {
                dag.insertArc ( iter->tail (), iter->head () );
              }
            }
      ...
      for ( unsigned int i = 7; i < 15; ++i ) {
        ...
        for ( unsigned int j = 0; j < 30; ++j ) {
          ...
          for ( auto& chgt : changes ) {
            ...
            case gum::DAGCycleDetector::ChangeType::ARC_ADDITION:
              g.insertArc ( chgt.tail (), chgt.head () );
              detector1.insertArc ( chgt.tail (), chgt.head () );
              break;

            case gum::DAGCycleDetector::ChangeType::ARC_REVERSAL:
              g.eraseArc ( gum::Arc ( chgt.tail (), chgt.head () ) );
              g.insertArc ( chgt.head (), chgt.tail () );
