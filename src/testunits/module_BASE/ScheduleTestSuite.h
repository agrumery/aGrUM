...
namespace gum_tests {
  ...
  GUM_TEST(_construct1) {
    ...
    comb2.execute();
    const auto& op2_res
        = dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >& >(*op2.results()[0]);
    ...
    comb1.execute();
    const auto& op1_res
        = dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >& >(*op1.results()[0]);
    ...
    comb3.execute();
    const auto& op3_res
        = dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >& >(*op3.results()[0]);
    ...
    comb4.execute();
    const auto& op6_res
        = dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >& >(*op6.results()[0]);
  ...
  GUM_TEST(_del_store) {
    ...
    const auto  xres1
        = static_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >* >(xcomb.results()[0]);
    const auto& xproj = schedule2.emplaceProjection(*xres1, del_vars, myProjectSum);
    const auto  xres2
        = static_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >* >(xproj.results()[0]);
