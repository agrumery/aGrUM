...
namespace gum_tests {
  ...
  GUM_TEST(_MultiDimProjection) {
...

    {
      ...
      auto t2 = t1.maxOut(proj_set);
      CHECK(
          dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >* >(ptrRes)->multiDim()
          == t2);
    }

    {
      ...
      auto t2 = t1.maxOut(proj_set);
      CHECK(
          dynamic_cast< const gum::ScheduleMultiDim< gum::Tensor< double > >* >(ops_plus_res.second)
              ->multiDim()
          == t2);
