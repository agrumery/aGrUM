...
namespace gum_tests {
  ...
  class SmallObjectAllocatorTestSuite : public CxxTest::TestSuite {
    ...
    void est_Fixed_Allocator_ALLOC_DEALLOC_1_ELEM() {
      ...
      gum::FixedAllocator* fa =
          new gum::FixedAllocator( 5 * sizeof( gum::Idx ), 50 );
      void* pVoid;
    ...
    void est_Small_Object_Allocator_CONST_AND_DEST() {
      ...
      // Test constructor
      TS_ASSERT_THROWS_NOTHING( soa = new gum::SmallObjectAllocator(
                                    8192, 24 * sizeof( gum::Idx ) ) );
      // Test destructor
    ...
    void est_Small_Object_Allocator_ALLOC_DEALLOC_1_ELEM() {
      gum::SmallObjectAllocator* soa =
          new gum::SmallObjectAllocator( 8192, 24 * sizeof( gum::Idx ) );
      void* pVoid;
      TS_ASSERT_THROWS_NOTHING( pVoid =
                                    soa->allocate( 5 * sizeof( gum::Idx ) ) );
      TS_ASSERT_THROWS_NOTHING(
          soa->deallocate( pVoid, 5 * sizeof( gum::Idx ) ) );
      TS_ASSERT_THROWS_NOTHING( delete soa );
    }
    ...
    void est_Small_Object_Allocator_ALLOC_DEALLOC_ELEMS() {
      ...
      for ( std::size_t i = 1; i < 13; ++i )
        TS_ASSERT_THROWS_NOTHING(
            vVoid.push_back( soa->allocate( 2 * i * sizeof( gum::Idx ) ) ) );

      std::vector<std::size_t> dv = {12, 6, 3, 9, 4, 11, 2, 7, 1, 5, 8, 10};
      for ( int i = 0; i < 12; ++i )
        TS_ASSERT_THROWS_NOTHING( soa->deallocate(
