...
namespace gum_tests {
  struct TensorTestSuite {
    ...
    void /*test*/ EliminatationOffAllVariables() {
      ...
      CHECK_EQ(p.sumOut({&a, &b}).toString(), "[45]");
    }
  ...
  GUM_TEST(Normalisation) {
    ...
    try {
      ...
      i.setLast();
      CHECK_EQ(p[i], 2.0 / (taille + 1));

      i.setFirst();
      CHECK_EQ(p[i], 2.0 / (taille * (taille + 1)));
  ...
  GUM_TEST(DomainSizeChanges) {
    ...
    CHECK_EQ(p1.domainSize(), (var1.domainSize() * var2.domainSize() * var3.domainSize()));
  }
  ...
  GUM_TEST(MinMax) {
    ...
    CHECK_EQ(p.min(), 0.0);
    CHECK_EQ(p.max(), 1.0);
    CHECK_EQ(p.minNonZero(), 1.0 / 13.0);
    CHECK_EQ(p.maxNonOne(), 9.0 / 13.0);

    p.fillWith({1, 1, 1, 1, 1, 1, 1, 1, 1});
    CHECK_EQ(p.maxNonOne(), 1);

    p.fillWith({0, 0, 0, 0, 0, 0, 0, 0, 0});
    CHECK_EQ(p.minNonZero(), 0);
  }
  ...
  GUM_TEST(Operators) {
    ...
    CHECK((pA) == ((gum::Tensor< int >() << b << c << a).fillWith({5, 18, 7, 24, 10, 24, 14, 32})));

    auto pB = p1 + p2;
    CHECK_EQ(pB, (gum::Tensor< int >() << b << c << a).fillWith({6, 9, 8, 11, 7, 10, 9, 12}));

    auto pC = p2 / p1;
    CHECK_EQ(pC, (gum::Tensor< int >() << b << a << c).fillWith({5, 2, 2, 1, 7, 2, 3, 2}));

    auto pD = p2 - p1;
    CHECK_EQ(pD, (gum::Tensor< int >() << b << a << c).fillWith({4, 3, 3, 2, 6, 5, 5, 4}));
    ...
    p -= (p2 / p1);
    p += p1;
    CHECK((p) == ((gum::Tensor< int >() << b << a << c).fillWith({1, 19, 10, 27, 1, 25, 13, 34})));
  }
  ...
  GUM_TEST(MargOutFunctions) {
    try {
      ...
      p.fillWith({1, 2, 3, 4, 5, 6, 7, 8, 9});
      CHECK_EQ(p.prodOut({&a}), (gum::Tensor< double >() << b).fillWith({6, 120, 504}));
      CHECK_EQ(p.prodOut({&b}), (gum::Tensor< double >() << a).fillWith({28, 80, 162}));

      CHECK_EQ(p.maxOut({&a}), (gum::Tensor< double >() << b).fillWith({3, 6, 9}));
      CHECK_EQ(p.maxOut({&b}), (gum::Tensor< double >() << a).fillWith({7, 8, 9}));

      CHECK_EQ(p.minOut({&a}), (gum::Tensor< double >() << b).fillWith({1, 4, 7}));
      CHECK_EQ(p.minOut({&b}), (gum::Tensor< double >() << a).fillWith({1, 2, 3}));
    } catch (gum::Exception& e) { GUM_SHOWERROR(e); }
  }

  GUM_TEST(MarginalizingFunctions) {
    auto a = gum::LabelizedVariable("a", "afoo", 3);
    auto b = gum::LabelizedVariable("b", "bfoo", 3);
    auto c = gum::LabelizedVariable("c", "cfoo", 3);
    auto d = gum::LabelizedVariable("d", "dfoo", 3);
    ...
    CHECK_EQ(joint.sumOut({&c, &d}), joint.sumIn({&a, &b}));
    CHECK_EQ(joint.sumOut({&c, &d}), joint.sumIn({&b, &a}));

    CHECK_EQ(joint.prodOut({&c, &d}), joint.prodIn({&a, &b}));

    CHECK_EQ(joint.minOut({&c, &d}), joint.minIn({&a, &b}));

    CHECK_EQ(joint.maxOut({&c, &d}), joint.maxIn({&a, &b}));
  }

  GUM_TEST(DeprecatedMarginalizingFunctions) {
    auto a = gum::LabelizedVariable("a", "afoo", 3);
    auto b = gum::LabelizedVariable("b", "bfoo", 3);
    auto c = gum::LabelizedVariable("c", "cfoo", 3);
    auto d = gum::LabelizedVariable("d", "dfoo", 3);
  ...
  GUM_TEST(AbsTensor) {
    ...
    CHECK_EQ((p - q).abs(), (gum::Tensor< double >() << a << b).fillWith({0, 2, 2, 0}));
    CHECK_EQ((q - p).abs(), (gum::Tensor< double >() << a << b).fillWith({0, 2, 2, 0}));
    CHECK_EQ((q - p).abs().max(), 2);
    CHECK_EQ((q - p).abs().min(), 0);
  }
  ...
  GUM_TEST(SqTensor) {
    ...
    CHECK_EQ((p - q).sq(), (gum::Tensor< double >() << a << b).fillWith({0, 4, 4, 0}));
    CHECK_EQ((q - p).sq(), (gum::Tensor< double >() << b << a).fillWith({0, 4, 4, 0}));
    CHECK_EQ((q - p).sq().max(), 4);
    CHECK_EQ((q - p).sq().min(), 0);
  }

  GUM_TEST(EntropyTensor) {
    auto                  a = gum::LabelizedVariable("a", "afoo", 2);
    gum::Tensor< double > p;
    p.add(a);
    CHECK_EQ(p.fillWith({0, 1}).entropy(), 0.0);
    CHECK_EQ(p.fillWith({1, 0}).entropy(), 0.0);
    CHECK_EQ(p.fillWith({0.5, 0.5}).entropy(), 1.0);
  }

  GUM_TEST(ReorganizeTensor) {
    auto a = gum::LabelizedVariable("a", "afoo", 3);
    auto b = gum::LabelizedVariable("b", "bfoo", 3);
    auto c = gum::LabelizedVariable("c", "cfoo", 3);
    auto d = gum::LabelizedVariable("d", "dfoo", 3);
    ...
    auto joint1 = p * q;
    auto joint2 = q * p;
    CHECK_NE(joint1.toString(), joint2.toString());
    auto joint3 = (q * p).reorganize({&c, &d, &a, &b});
    CHECK_EQ(joint1.toString(), joint3.toString());

    gum::Instantiation inst;
    inst << a << b << c << d;
    for (inst.setFirst(); !inst.end(); ++inst) {
      CHECK_EQ(joint1.get(inst), joint2.get(inst));
      CHECK_EQ(joint1.get(inst), joint3.get(inst));
    }
  ...
  GUM_TEST(ReorganizeTensorFromNames) {
    auto a = gum::LabelizedVariable("a", "afoo", 3);
    auto b = gum::LabelizedVariable("b", "bfoo", 3);
    auto c = gum::LabelizedVariable("c", "cfoo", 3);
    auto d = gum::LabelizedVariable("d", "dfoo", 3);
  ...
  GUM_TEST(PutFirst) {
    ...
    CHECK_NE(p.toString(), p.putFirst(&b).toString());
    CHECK_EQ(p.toString(), p.putFirst(&b).putFirst(&a).toString());
    CHECK_EQ(p.toString(), p.putFirst(&a).toString());
  ...
  GUM_TEST(Extraction) {
    ...
    I << c;
    I.chgVal(c, 0);
    CHECK_EQ(pot.extract(I), p);
    I.chgVal(c, 2);
    gum::Tensor< double > r;
    r << a << b;
    r.fillWith({3, 6, 9, 12, 15, 18, 21, 24, 27});
    CHECK_EQ(pot.reorganize({&b, &c, &a}).extract(I), r);
  }
  ...
  GUM_TEST(EmptyTensor) {
    ...
    CHECK_EQ(p[inst], 5.0f);

    a = 0.3f;
    GUM_CHECK_ASSERT_THROWS_NOTHING(a = p.reduce([](double x, double y) { return x + y; }, 0.0f););
    CHECK_EQ(a, 0.0f);

    GUM_CHECK_ASSERT_THROWS_NOTHING(p.populate({33.0f}));
    CHECK_EQ(p.domainSize(), 1u);
    CHECK_EQ(p.get(gum::Instantiation()), 33.0f);


    GUM_CHECK_ASSERT_THROWS_NOTHING(p.set(inst, static_cast< double >(3.0f)););
    CHECK_EQ(p.sum(), 3.0f);
    CHECK_EQ(p.product(), 3.0f);
    CHECK_EQ(p.max(), 3.0f);
    CHECK_EQ(p.min(), 3.0f);
    CHECK_EQ(p.minNonZero(), 3.0f);
    CHECK_EQ(p.maxNonOne(), 3.0f);
    ...
    CHECK_EQ(p.maxNonOne(), 1.0f);

    GUM_CHECK_ASSERT_THROWS_NOTHING(p.fill(2.0f););
    CHECK_EQ(p[inst], 2.0f);
    GUM_CHECK_ASSERT_THROWS_NOTHING(p.fillWith(3.0f););
    CHECK_EQ(p[inst], 3.0f);
    GUM_CHECK_ASSERT_THROWS_NOTHING(p.fillWith({4.0f}););
    CHECK_EQ(p[inst], 4.0f);
    GUM_CHECK_ASSERT_THROWS_NOTHING(p.fillWith(std::vector< double >({5.0f})););
    CHECK_EQ(p[inst], 5.0f);

    CHECK_EQ(p.entropy(), 0.0f);

    CHECK_EQ(p.fillWith(2.0f).sq()[inst], 4.0f);
    CHECK_EQ(p.fillWith(-2.0f).sq()[inst], 4.0f);
    CHECK_EQ(p.fillWith(2.0f).abs()[inst], 2.0f);
    CHECK_EQ(p.fillWith(-2.0f).abs()[inst], 2.0f);

    CHECK_EQ(p.fillWith(2.0f).normalize()[inst], 1.0f);
    CHECK_EQ(p.fillWith(0.0f).normalize()[inst], 0.0f);

    CHECK_EQ(p.fillWith(2.0f).scale(4)[inst], 8.0f);
    CHECK_EQ(p.fillWith(2.0f).translate(4)[inst], 6.0f);


    CHECK_EQ(p.fillWith(2.0f).sumOut({&var_a})[inst], 2.0f);
    CHECK_EQ(p.fillWith(3.0f).sumIn({&var_a})[inst], 3.0f);
    CHECK_EQ(p.fillWith(4.0f).prodOut({&var_a})[inst], 4.0f);
    CHECK_EQ(p.fillWith(5.0f).prodIn({&var_a})[inst], 5.0f);
    CHECK_EQ(p.fillWith(6.0f).maxIn({&var_a})[inst], 6.0f);
    CHECK_EQ(p.fillWith(7.0f).maxOut({&var_a})[inst], 7.0f);
    CHECK_EQ(p.fillWith(8.0f).minOut({&var_a})[inst], 8.0f);
    CHECK_EQ(p.fillWith(9.0f).minOut({&var_a})[inst], 9.0f);

    CHECK_EQ(p.fillWith(0.0f).isNonZeroMap()[inst], 0.0f);
    CHECK_EQ(p.fillWith(1.0f).isNonZeroMap()[inst], 1.0f);
    CHECK_EQ(p.fillWith(0.5f).isNonZeroMap()[inst], 1.0f);
    CHECK_EQ(p.fillWith(-0.5f).isNonZeroMap()[inst], 1.0f);
  }

  GUM_TEST(OperationForEmptyTensor) {
    auto a = gum::LabelizedVariable("a", "afoo", 3);
    auto b = gum::LabelizedVariable("b", "bfoo", 3);
    ...
    res.fillWith(
        {1 / 2.0f, 2 / 2.0f, 3 / 2.0f, 4 / 2.0f, 5 / 2.0f, 6 / 2.0f, 7 / 2.0f, 8 / 2.0f, 9 / 2.0f});
    CHECK_EQ(p / q, res);
    tmp = p;
    tmp /= q;
    CHECK_EQ(tmp, res);
    ...
    res.fillWith(
        {2.0f / 1, 2.0f / 2, 2.0f / 3, 2.0f / 4, 2.0f / 5, 2.0f / 6, 2.0f / 7, 2.0f / 8, 2.0f / 9});
    CHECK_EQ(q / p, res);
    tmp = q;
    tmp /= p;
    CHECK_EQ(tmp, res);
  ...
  GUM_TEST(OperationForTwoEmptyTensors) {
    gum::Tensor< double > p;
    p.fill(3);
    ...
    res.fillWith({2.0f / 3});
    CHECK_LT((q / p - res).abs().max(), 1e-7);
    tmp = q;
    tmp /= p;
    CHECK_LT((tmp - res).abs().max(), 1e-7);
  }

  GUM_TEST(LoopsForEmptyTensor) {
    gum::Tensor< double > p;
    gum::Instantiation    inst(p);
    p.fill(3);
    ...
    CHECK_EQ(cpt, static_cast< gum::Size >(1));
    CHECK_EQ(total, 3);
  ...
  GUM_TEST(KL) {
    ...
    CHECK(
        (res)
        == doctest::Approx(0.5 * log2(0.5 / 0.7) + 0.5 * log2(0.5 / 0.3)).epsilon(GUM_SMALL_ERROR));

    GUM_CHECK_ASSERT_THROWS_NOTHING(res = r.KL(q));
    CHECK(
        (res)
        == doctest::Approx(0.7 * log2(0.7 / 0.5) + 0.3 * log2(0.3 / 0.5)).epsilon(GUM_SMALL_ERROR));
  }
  ...
  GUM_TEST(FillWithTensorMethod) {
    ...
    CHECK_EQ(p.domainSize(), static_cast< gum::Size >(6));
  ...
  GUM_TEST(FillWithTensorAndMapMethod) {
    gum::LabelizedVariable v("v", "v", 2), w("w", "w", 3);
    gum::Tensor< int >     p;
    p.add(v);
    p.add(w);
    ...
    CHECK_EQ(p.domainSize(), static_cast< gum::Size >(6));
    CHECK_EQ(pp.domainSize(), static_cast< gum::Size >(6));
  ...
  GUM_TEST(AddDummyVariables) {
    {
      ...
      gum::Tensor< double > p;
      CHECK_EQ(v.domainSize(), static_cast< gum::Size >(0));
      CHECK_THROWS_AS(p.add(v), const gum::InvalidArgument&);

      v.addLabel("first");
      CHECK_EQ(v.domainSize(), static_cast< gum::Size >(1));
      GUM_CHECK_ASSERT_THROWS_NOTHING(p.add(v));

      p = gum::Tensor< double >();
      v.addLabel("second");
      CHECK_EQ(v.domainSize(), static_cast< gum::Size >(2));
      GUM_CHECK_ASSERT_THROWS_NOTHING(p.add(v));
    }
  ...
  GUM_TEST(ExpectedValue) {
    ...
    CHECK(
        fabs((q.expectedValue([](const gum::Instantiation& i) -> auto { return double(i.val(0)); }))
             - (mean))
        < 1e-10);
  }

  GUM_TEST(MeanVarianceStdDev) {
  ...
  GUM_TEST(Sgn) {
    ...
    auto r = p * 2;
    CHECK_EQ((r - p).sgn().min(), 1.0);
    CHECK_EQ((r - p).sgn().max(), 1.0);
    CHECK_EQ((p - r).sgn().min(), -1.0);
    CHECK_EQ((p - r).sgn().max(), -1.0);
    CHECK_EQ((p - p).sgn().min(), 0.0);
    CHECK_EQ((p - p).sgn().max(), 0.0);
  }
  ...
  GUM_TEST(DeterministicTensor_CreatesTensorWithCorrectValue) {
    gum::RangeVariable var("var", "Variable1", 1, 4);
    const gum::Idx     value = 2;

    auto tensor = gum::Tensor< double >::deterministicTensor(var, value);

    CHECK_EQ(tensor.sum(), 1.0);

    CHECK_EQ(tensor.max(), 1.0);
    const auto& [x, y] = tensor.argmax();
    CHECK_EQ(x.size(), gum::Size(1));
    const auto& i = *x.begin();
    CHECK_EQ(i.val(0), value);

    CHECK_EQ(tensor.min(), 0.0);
    CHECK_EQ(tensor.minNonZero(), 1.0);
    CHECK_EQ(tensor.maxNonOne(), 0.0);
  }

  GUM_TEST(DeterministicTensor_CreatesTensorWithCorrectLabel) {
  ...
  GUM_TEST(DataAccessorViaTensorContent) {
    gum::LabelizedVariable a("a", "first", 2);
    gum::LabelizedVariable b("b", "second", 3);
    gum::Tensor< double >  t;
    t << a << b;
  ...
  GUM_TEST(DataAccessorScalarTensor) {
    gum::Tensor< double > t;
