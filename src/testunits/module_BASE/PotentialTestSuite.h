    ...
    public:
      ...
      void testNormalisation() {
        ...
        try {
          ...
          int cpt = 1; for ( i.setFirst(); ! i.end() ; ++i ) p.set ( i, ( double ) ( cpt++ ) );
          ...
          i.setLast(); TS_ASSERT_EQUALS ( p[i], 2.0 / ( taille + 1 ) );



          i.setFirst(); TS_ASSERT_EQUALS ( p[i], 2.0 / ( taille * ( taille + 1 ) ) );
      ...
      void testMarginalisation() {
        ...
        try {
          ...
          for ( i.setFirst(); ! i.end() ; ++i ) p.set ( i, 1 - i.val ( a ) );

          gum::Potential<double> q ( new gum::MultiDimArray<double>() );

          gum::DiscretizedVariable<int> d ( "d", "fourth var" ); d.addTick ( 3 ).addTick ( 5 ).addTick ( 4 );
          ...
          i.setLast(); TS_ASSERT_EQUALS ( qq[i], 0.0 );

          i.setFirst(); TS_ASSERT_EQUALS ( qq[i], 4.0 );
      ...
      void testDomainSizeChanges() {
        gum::Potential<double> p1 ( new gum::MultiDimArray<double>() );
        gum::LabelizedVariable var1 ( "var1", "first var", 2 ), var2 ( "var2", "second var", 2 ), var3 ( "var3", "third var", 2 );
        p1 << var1 << var2 << var3;

        TS_ASSERT_EQUALS ( p1.domainSize(), ( var1.domainSize() * var2.domainSize() * var3.domainSize() ) );
      }
      ...
      void testCopyProba() {
        ...
        gum::LabelizedVariable x ( "x", "first var", 2 ), y ( "y", "second var", 4 ), z ( "z", "third var", 5 );
        mm << x << z ;
