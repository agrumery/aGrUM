...
namespace gum_tests {
  ...
  struct SharedAVLTreeTestSuite {
    ...
    static void test_int() {
      ...
      CHECK_EQ(tree1.empty(), true);
      CHECK_EQ(tree1.size(), gum::Size(0));
      ...
      for (int i = 0; tree1_beg != tree1_end; ++tree1_beg, ++i)
        CHECK_EQ((*tree1_beg).value, vect[i]);
      int i = 0;
      for (auto iter = tree1.begin(); iter != tree1.end(); ++iter) {
        CHECK_EQ(iter->value, vect[i++]);
      }
      i = 0;
      for (const auto& node: tree2) {
        CHECK_EQ(node.value, vect[i++]);
      }

      {
        ...
        str << '}';
        CHECK_EQ(sss, str.str());
        std::stringstream str2;
        str2 << tree2;
        CHECK_EQ(str.str(), str2.str());
      }
      ...
      CHECK_EQ(tree4.size(), gum::Size(100));
      ...
      for (auto iter = tree4.beginSafe(); iter != tree4.endSafe(); i++, iter3 += 3) {
        ...
        if (i == 1) {
          ...
          auto iter2 = iter3;
          --iter2;
          CHECK_EQ(iter2->value, 5);
        } else if (i >= 2) CHECK_EQ(iter3->value, (i - 2) * 4 + 9);
      }
      ...
      for (auto iter = tree5.rbeginSafe(); iter != tree5.rendSafe(); iter5 += 3, ++i) {
        ...
        if (i == 1) {
          ...
          auto iter2 = iter5;
          --iter2;
          CHECK_EQ(iter2->value, 95);
        } else if (i >= 2) CHECK_EQ(iter5->value, 99 - (i - 2) * 4 - 9);
      }

      auto iter6 = gum::SharedAVLTreeIteratorSafe(tree5, false);   // equiv rbegin
      CHECK_EQ(++iter6, tree5.endSafe());
      CHECK_NOTHROW(tree5.erase(iter6));

      iter3 = tree2.beginSafe();
      iter5 = tree2.rbeginSafe();
      tree2.clear();
      CHECK_EQ(iter3, tree2.endSafe());
      CHECK_EQ(iter5, tree2.rendSafe());
    }
    ...
    static void test_pairs() {
      ...
      CHECK_EQ(tree1.empty(), true);
      CHECK_EQ(tree1.size(), gum::Size(0));
      CHECK_EQ(tree1.lowestNode(), nullptr);
      CHECK_EQ(tree1.highestNode(), nullptr);
      ...
      gum::SharedAVLTreeIterator< std::pair< int, int >, Mycmp > tree1_beg(tree1, true),
          tree1_end = tree1.end();
      for (int i = 0; tree1_beg != tree1_end; ++tree1_beg, ++i)
        CHECK_EQ((*tree1_beg).value, vectp[i]);

      int i = 0;
      for (auto iter = tree1.begin(); iter != tree1.end(); ++iter) {
        CHECK_EQ(iter->value, vectp[i++]);
      }
      i = 0;
      for (const auto& node: tree2) {
        CHECK_EQ(node.value, vectp[i++]);
      }
      ...
      for (auto iter = tree4.beginSafe(); iter != tree4.endSafe(); i++, iter3 += 3) {
        ...
        if (i == 1) {
          ...
          auto iter2 = iter3;
          --iter2;
          CHECK_EQ(iter2->value, vectp2[5]);
        } else if (i >= 2) CHECK_EQ(iter3->value, vectp2[(i - 2) * 4 + 9]);
      }
      ...
      for (auto iter = tree5.rbeginSafe(); iter != tree5.rendSafe(); iter5 += 3, ++i) {
        ...
        if (i == 1) {
          ...
          auto iter2 = iter5;
          --iter2;
          CHECK_EQ(iter2->value, vectp2[95]);
        } else if (i >= 2) CHECK_EQ(iter5->value, vectp2[99 - (i - 2) * 4 - 9]);
      }
    ...
    static void test_shuffle() {
      ...
      CHECK_EQ(vect2string(vect), tree.toString());
