...
namespace gum_tests {
  ...
  GUM_TEST(_MultiDimFunctionGraph_Diagram_Handlers_Methods) {
    try {
      ...
      // Test isTerminalNode
      CHECK_EQ(functionGraph->isTerminalNode(16), true);
      CHECK_EQ(functionGraph->isTerminalNode(6), false);
      CHECK_EQ(functionGraph->isTerminalNode(24), false);

      // Test isInternalNode
      CHECK_EQ(functionGraph->isInternalNode(6), true);
      CHECK_EQ(functionGraph->isInternalNode(16), false);
      CHECK_EQ(functionGraph->isInternalNode(24), false);

      // Test Root
      CHECK_EQ(functionGraph->root(), (gum::NodeId)1);

      // test terminal node value
      GUM_CHECK_ASSERT_THROWS_NOTHING(functionGraph->nodeValue(16));
      CHECK_EQ(functionGraph->nodeValue(16), 0);
  ...
  GUM_TEST(_Manager_Graphical_Functions) {
    try {
      ...
      CHECK_EQ(functionGraph->manager()->addTerminalNode(10), idList[16]);
  ...
  GUM_TEST(_MultiDimFunctionGraph_Accessors_Modifiers_Methods) {
    try {
      ...
      CHECK_EQ(varSeq.exists(Cprimevar), true);
      CHECK_EQ(varSeq.exists(Cvar), true);
      CHECK_EQ(varSeq.exists(PLvar), true);
      CHECK_EQ(varSeq.exists(APUvar), true);
      CHECK_EQ(varSeq.exists(BPUvar), true);
      CHECK_EQ(varSeq.exists(ADRvar), true);
      CHECK_EQ(varSeq.exists(BDRvar), true);
      CHECK_EQ(varSeq.exists(BOvar), true);


      // *********************************************************************
      // Test name() and basename()
      // *********************************************************************
      CHECK_EQ(functionGraph->name(), "MultiDimFunctionGraph");

      CHECK_EQ(functionGraph->basename(), "MultiDimImplementation");
  ...
  GUM_TEST(_MultiDimFunctionGraph_Implementation_Methods) {
    try {
      ...
      CHECK_EQ(functionGraph->nbrDim(), static_cast< gum::Idx >(8));

      CHECK_EQ(functionGraph->domainSize(), static_cast< gum::Size >(256));

      CHECK_EQ(functionGraph->realSize(), static_cast< gum::Size >(15));   // 19)
      ...
      CHECK_EQ(functionGraph->contains(*Cprimevar), true);
      CHECK_EQ(functionGraph->contains(*lv), false);

      CHECK_EQ(functionGraph->empty(), false);
      ...
      CHECK_EQ(functionGraph->contains(*lv), true);


      GUM_CHECK_ASSERT_THROWS_NOTHING(functionGraph->erase(*Cvar));
      CHECK_EQ(functionGraph->contains(*Cvar), false);
  ...
  GUM_TEST(_MultiDimFunctionGraph_Copy_Methods) {
    try {
      ...
      // *********************************************************************
      GUM_CHECK_ASSERT_THROWS_NOTHING(functionGraph2->copyAndMultiplyByScalar(*functionGraph, 2););

      GUM_CHECK_ASSERT_THROWS_NOTHING(functionGraph2->copyAndMultiplyByScalar(*functionGraph, 0););
  ...
  GUM_TEST(_MultiDimFunctionGraph_Various_Methods) {
    try {
      ...
      // test == et !=
      // *********************************************************************
      CHECK_EQ(*dg1 == *dg2, true);
      CHECK_EQ(*dg1 == *dg3, false);

      CHECK_EQ(*dg1 != *dg2, false);
      CHECK_EQ(*dg1 != *dg3, true);
