...
namespace gum_tests {
  ...
  class InterfaceGraphTestSuite : public CxxTest::TestSuite {
    ...
    void testConstructorDestructor() {
      ...
      TS_GUM_ASSERT_THROWS_NOTHING(
          ig = new gum::prm::gspan::InterfaceGraph<double>(
              __prm->getSystem( "microSys" ) ) );
      TS_GUM_ASSERT_THROWS_NOTHING( delete ig );
    }

    void testMicroTopologie() {
      gum::prm::gspan::InterfaceGraph<double>* ig = 0;
      gum::prm::PRMSystem<double>& m = __prm->getSystem( "microSys" );
      TS_GUM_ASSERT_THROWS_NOTHING(
      ...
      TS_ASSERT( ig->graph().existsEdge( ig->id( m.get( "pow" ) ),
                                         ig->id( m.get( "e" ) ) ) );
      TS_ASSERT( ig->graph().existsEdge( ig->id( m.get( "p" ) ),
                                         ig->id( m.get( "c" ) ) ) );
    ...
    void testMicroLabelsOnNodes() {
      ...
      TS_ASSERT_EQUALS( ig->size( ig->node( ig->id( m.get( "pow" ) ) ).l ),
                        (gum::Size)1 );
      ...
      // Testing rooms
      TS_ASSERT_EQUALS( ig->size( ig->node( ig->id( m.get( "r" ) ) ).l ),
                        (gum::Size)1 );
      ...
      TS_ASSERT_DIFFERS( ig->node( ig->id( m.get( "r" ) ) ).l->l,
                         ig->node( ig->id( m.get( "c" ) ) ).l->l );
      // Testing printers
      TS_ASSERT_EQUALS( ig->size( ig->node( ig->id( m.get( "p" ) ) ).l ),
                        (gum::Size)1 );
    ...
    void testMicroLabelsOnEdges() {
      ...
      // Test difference
      TS_ASSERT_DIFFERS(
          ig->edge( ig->id( m.get( "pow" ) ), ig->id( m.get( "p" ) ) ).l,
    ...
    void testSmallTopologie() {
      gum::prm::gspan::InterfaceGraph<double>* ig = 0;
      gum::prm::PRMSystem<double>& m = __prm->getSystem( "smallSys" );
      TS_GUM_ASSERT_THROWS_NOTHING(
          ig = new gum::prm::gspan::InterfaceGraph<double>( m ) );
      // Checking existing nodes
      int node_count = 0;
      ...
      for ( int i = 0; i < 2; ++i, ++node_count ) {
        TS_ASSERT(
            ig->graph().existsNode( ig->id( m.get( getPrinter( i ) ) ) ) );
      }

      TS_ASSERT(
          ig->graph().existsNode( ig->id( m.get( "another_printer" ) ) ) );

      ++node_count;

      for ( int i = 0; i < 4; ++i, ++node_count ) {
        TS_ASSERT(
            ig->graph().existsNode( ig->id( m.get( getComputer( i ) ) ) ) );
      }

      TS_ASSERT(
          ig->graph().existsNode( ig->id( m.get( "another_computer" ) ) ) );
      ...
      for ( int i = 0; i < 2; ++i, ++edge_count ) {
        TS_ASSERT( ig->graph().existsEdge(
            ig->id( m.get( "pow" ) ), ig->id( m.get( getPrinter( i ) ) ) ) );
      }

      TS_ASSERT( ig->graph().existsEdge(
          ig->id( m.get( "pow" ) ), ig->id( m.get( "another_printer" ) ) ) );

      ++edge_count;

      for ( int i = 0; i < 4; ++i, ++edge_count ) {
        TS_ASSERT( ig->graph().existsEdge(
            ig->id( m.get( "pow" ) ), ig->id( m.get( getComputer( i ) ) ) ) );
      }

      TS_ASSERT( ig->graph().existsEdge(
          ig->id( m.get( "pow" ) ), ig->id( m.get( "another_computer" ) ) ) );
      ...
      for ( int i = 0; i < 2; ++i, ++edge_count ) {
        for ( int j = 0; j < 4; ++j, ++edge_count ) {
          TS_ASSERT(
              ig->graph().existsEdge( ig->id( m.get( getPrinter( i ) ) ),
      ...
      for ( int i = 0; i < 4; ++i, ++edge_count ) {
        TS_ASSERT(
            ig->graph().existsEdge( ig->id( m.get( "another_printer" ) ),
    ...
    void testSmallLabelsOnNodes() {
      ...
      TS_ASSERT_EQUALS( ig->size( ig->node( ig->id( m.get( "r" ) ) ).l ),
                        (gum::Size)1 );
      TS_ASSERT_EQUALS(
          ig->size( ig->node( ig->id( m.get( "another_printer" ) ) ).l ),
          (gum::Size)3 );
      TS_ASSERT_EQUALS(
          ig->size( ig->node( ig->id( m.get( "another_computer" ) ) ).l ),
          (gum::Size)5 );
      TS_GUM_ASSERT_THROWS_NOTHING( delete ig );
    }
    ...
    void
    generateLayer( std::vector<gum::prm::LayerGenerator<double>::LayerData>& v,
                   size_t layer_count ) {
