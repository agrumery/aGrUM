...
class EliminationSequenceTriangulationTestSuite: public CxxTest::TestSuite {
  public:
    ...
    void testFillIns() {
      ...
      TS_ASSERT_EQUALS( fillIns.size(), ( gum::Size )5 );
      TS_ASSERT( fillIns.contains( gum::Edge( 2, 3 ) ) ); // BC
      TS_ASSERT( fillIns.contains( gum::Edge( 3, 4 ) ) ); // CD
      TS_ASSERT( fillIns.contains( gum::Edge( 4, 5 ) ) ); // DE
      TS_ASSERT( fillIns.contains( gum::Edge( 3, 5 ) ) ); // CE
      TS_ASSERT( fillIns.contains( gum::Edge( 3, 6 ) ) ); // CF
    };
    ...
    void testJunctionTree() {
      ...
      TS_ASSERT( junction.clique( tr.createdClique( 1 ) ).size() == 3 );
      ...
      TS_ASSERT( junction.clique( tr.createdClique( 6 ) ).size() == 4 );
      TS_ASSERT( junction.clique( tr.createdClique( 7 ) ).size() == 3 );
      TS_ASSERT( junction.separator( tr.createdClique( 1 ),
                                     tr.createdClique( 2 ) ).size() == 2 ); // BC
      TS_ASSERT( junction.separator( tr.createdClique( 2 ),
                                     tr.createdClique( 5 ) ).size() == 3 ); // CED
      TS_ASSERT( junction.separator( tr.createdClique( 5 ),
                                     tr.createdClique( 7 ) ).size() == 2 ); // CF
    }
