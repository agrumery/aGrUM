...
namespace gum_tests {
  struct ShaferShenoyInferenceTestSuite {
    public:
    ...
    ShaferShenoyInferenceTestSuite() {
    ...
    ~ShaferShenoyInferenceTestSuite() { delete bn; }
  ...
  GUM_TEST(SmartManagementOfJointTarget) {
    ...
    vars.insert(&(bn->variable(3)));
    vars.insert(&(bn->variable(4)));
    GUM_CHECK_TENSOR_ALMOST_EQUALS(inf.jointPosterior(gum::NodeSet{2, 3, 4}), bn_joint.sumIn(vars));
  }
  ...
  GUM_TEST(EvidenceProbabilityAsia) {
    ...
    CHECK_EQ(nbrErr, (gum::Size) static_cast< gum::Size >(0));
    ...
    for (auto node: bn.internalDag()) {
      ...
      for (inst.setFirst(); !inst.end(); ++inst) {
        ...
        for (auto node: bn.internalDag()) {
          ...
          CHECK(equalTensors(inf2.posterior(node), joint.sumIn({&bn.variable(node)}).normalize()));
        }
  ...
  GUM_TEST(IrrelevantSoftEvidence) {
    ...
    GUM_CHECK_TENSOR_ALMOST_EQUALS(phard, jointhard.sumIn({&bn.variableFromName("C")}).normalize());
    GUM_CHECK_TENSOR_ALMOST_EQUALS(psoft, jointsoft.sumIn({&bn.variableFromName("C")}).normalize());
  }
