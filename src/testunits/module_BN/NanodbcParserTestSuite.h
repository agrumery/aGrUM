  ...
  struct NanodbcParserTestSuite {
    ...
    static void testSimpleSQL() {
      ...
      try {
        ...
        CHECK_EQ(parser.nbColumns(), std::size_t(8));

        parser.next();
        CHECK_EQ(parser.nbLine(), std::size_t(0));
        ...
        CHECK_EQ(row1[3], "1");
        CHECK_EQ(row1[4], "1");
        CHECK_EQ(row1[5], "false");
        CHECK_EQ(row1[6], "toto titi");
        CHECK_EQ(row1[7], "-1.2");

        parser.next();
        CHECK_EQ(parser.nbLine(), std::size_t(1));
        ...
        CHECK_EQ(row2[3], "1");
        CHECK_EQ(row2[4], "1");
        CHECK_EQ(row2[5], "true");
        CHECK_EQ(row2[6], "toto titi");
        CHECK_EQ(row2[7], "2.45");

        parser.next();
        CHECK_EQ(parser.nbLine(), std::size_t(2));
        ...
        CHECK_EQ(row3[3], "1");
        CHECK_EQ(row3[4], "1");
        CHECK_EQ(row3[5], "true");
        CHECK_EQ(row3[6], "toto titi");
        CHECK_EQ(row3[7], "4");

        CHECK_EQ(parser.columnName(std::size_t(0)), "smoking");
        CHECK_EQ(parser.columnName(std::size_t(1)), "lung_cancer");
        CHECK_EQ(parser.columnName(std::size_t(2)), "bronchitis");
        CHECK_EQ(parser.columnName(std::size_t(3)), "visit_to_Asia");
        CHECK_EQ(parser.columnName(std::size_t(4)), "tuberculosis");
        CHECK_EQ(parser.columnName(std::size_t(5)), "tuberculos_or_cancer");
        CHECK_EQ(parser.columnName(std::size_t(6)), "dyspnoea");
        CHECK_EQ(parser.columnName(std::size_t(7)), "positive_XraY");

        CHECK_EQ(parser.next(), false);
        ...
        CHECK_EQ(xrow1[3], "1");
        CHECK_EQ(xrow1[4], "1");
        CHECK_EQ(xrow1[5], "false");
        CHECK_EQ(xrow1[6], "toto titi");
        CHECK_EQ(xrow1[7], "-1.2");
        ...
        CHECK_EQ(xrow2[3], "1");
        CHECK_EQ(xrow2[4], "1");
        CHECK_EQ(xrow2[5], "true");
        CHECK_EQ(xrow2[6], "toto titi");
        CHECK_EQ(xrow2[7], "2.45");
        ...
        CHECK_EQ(xrow3[3], "1");
        CHECK_EQ(xrow3[4], "1");
        CHECK_EQ(xrow3[5], "true");
        CHECK_EQ(xrow3[6], "toto titi");
        CHECK_EQ(xrow3[7], "4");
...
#  endif   // _WIN32
      ...
      try {
        ...
        row_count.next();
        CHECK_EQ(row_count.get< int >(0), 3);

        const std::string query = "select * from asia;";

        auto row = nanodbc::execute(connection, query);
        row.next();
        CHECK_EQ(row.column_name(0), "smoking");
        CHECK_EQ(row.columns(), 8);

        CHECK_EQ(row.get< int >(0, 5), 1);
        CHECK_EQ(row.get< std::string >(6), "toto titi");
        CHECK_EQ(row.get< double >(7), -1.2);
