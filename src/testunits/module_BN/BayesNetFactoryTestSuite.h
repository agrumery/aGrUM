...
namespace gum_tests {
  ...
  GUM_TEST(LegalsCallsInNONE) {
    ...
    CHECK_EQ(__bn_d->property("name"), "TestSuite BayesNet");
    CHECK_EQ(__bn_d->property("author"), "Lionel");
  ...
  GUM_TEST(BayesNetCreation) {
    ...
    GUM_CHECK_ASSERT_THROWS_NOTHING(factory->setParentModality("4", "true"));
    values_5.push_back(static_cast< float >(0));   // 5 : true  given 2 : false, 3 : false, 4 : true
    values_5.push_back(static_cast< float >(1));   // 5 : false given 2 : false, 3 : false, 4 : true
    GUM_CHECK_ASSERT_THROWS_NOTHING(factory->setVariableValues(values_5));
    values_5.clear();
    GUM_CHECK_ASSERT_THROWS_NOTHING(factory->endFactorizedEntry());
