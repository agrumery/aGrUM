...
namespace gum_tests {
  ...
  GUM_TEST(FillG1) {
    ...
    CHECK_EQ(graph.size(), static_cast< gum::Size >(4));

    CHECK_EQ(graph.clique(1), A);
    CHECK_EQ(graph.clique(2), B);
    CHECK_EQ(graph.clique(3), C);
    CHECK_EQ(graph.clique(4), D);
  ...
  GUM_TEST(FillG3) {
    ...
    CHECK_EQ(graph.clique(2), B);
    CHECK_EQ(graph.clique(3), C);
    CHECK_NE(graph.clique(4), D);
    CHECK_EQ(graph.clique(4), E);
  }
  ...
  GUM_TEST(HasRunningIntersection) {
    gum::CliqueGraph g1, g2, g3;
    fillG1(g1);
    fillG2(g2);
    fillG3(g3);
  ...
  GUM_TEST(GetNbrEdges) {
    gum::CliqueGraph g1, g2, g3;
    fillG1(g1);
    fillG2(g2);
    fillG3(g3);

    CHECK_EQ(g1.sizeEdges(), static_cast< gum::Size >(3));
    CHECK_EQ(g2.sizeEdges(), static_cast< gum::Size >(5));
    CHECK_EQ(g3.sizeEdges(), static_cast< gum::Size >(3));
  }
  ...
  GUM_TEST(ClearNodes) {
    ...
    CHECK_EQ(g1.size(), static_cast< gum::Size >(0));
    CHECK_EQ(g1.sizeEdges(), static_cast< gum::Size >(0));

    GUM_CHECK_ASSERT_THROWS_NOTHING(g2.clear());
    CHECK_EQ(g2.size(), static_cast< gum::Size >(0));
    CHECK_EQ(g2.sizeEdges(), static_cast< gum::Size >(0));

    GUM_CHECK_ASSERT_THROWS_NOTHING(g3.clear());
    CHECK_EQ(g3.size(), static_cast< gum::Size >(0));
    CHECK_EQ(g3.sizeEdges(), static_cast< gum::Size >(0));
  }
  ...
  GUM_TEST(EraseNode) {
    ...
    CHECK_EQ(g1.size(), nodeG1Count - 1);
    GUM_CHECK_ASSERT_THROWS_NOTHING(g1.eraseNode(1));
    CHECK_EQ(g1.size(), nodeG1Count - 1);
    ...
    CHECK_EQ(g2.size(), nodeG2Count - 1);
    GUM_CHECK_ASSERT_THROWS_NOTHING(g2.eraseNode(1));
    CHECK_EQ(g2.size(), nodeG2Count - 1);
    ...
    CHECK_EQ(g3.size(), nodeG3Count - 1);
    GUM_CHECK_ASSERT_THROWS_NOTHING(g3.eraseNode(1));
    CHECK_EQ(g3.size(), nodeG3Count - 1);
  ...
  GUM_TEST(GetSeparatorByEdgeG1) {
    ...
    CHECK_EQ(separator.size(), static_cast< gum::Size >(1));
    CHECK(separator.contains(2));

    GUM_CHECK_ASSERT_THROWS_NOTHING(separator = g1.separator(2, 3));
    CHECK_EQ(separator.size(), static_cast< gum::Size >(2));
  ...
  GUM_TEST(EraseEdgeG1) {
    ...
    CHECK_EQ(g1.sizeEdges(), gum::Size(countEdgeG1 - 1));

    CHECK_THROWS(g1.separator(1, 2));
  }

  GUM_TEST(EraseEdgeG2) {
    gum::CliqueGraph g2;
    fillG2(g2);
    ...
    CHECK_EQ(g2.sizeEdges(), gum::Size(countEdgeG2 - 2));
  }

  GUM_TEST(EraseEdgeG3) {
    gum::CliqueGraph g3;
    fillG3(g3);
    ...
    CHECK_EQ(g3.sizeEdges(), gum::Size(countEdgeG3 - 1));
  }

  GUM_TEST(GetClique_1) {
    gum::CliqueGraph graph;
    fillG1(graph);
    ...
    CHECK_EQ(clique.size(), static_cast< gum::Size >(2));
  ...
  GUM_TEST(GetClique_2) {
    ...
    CHECK_EQ(clique, graph.clique(new_1));
  }

  GUM_TEST(GetContainerNode) {
    gum::NodeSet A, B, C, D, E;
    fillSets(A, B, C, D, E);
    ...
    CHECK(
        (firstClique != secondClique && firstClique != thirdClique && secondClique != thirdClique));
  }

  GUM_TEST(SetClique_1) {
    gum::CliqueGraph graph;
    fillG3(graph);
    ...
    CHECK_EQ(graph.clique(2).size(), static_cast< gum::Size >(4));
    CHECK(graph.hasRunningIntersection());
  }

  GUM_TEST(SetClique_2) {
    gum::CliqueGraph graph;
    fillG3(graph);
  ...
  GUM_TEST(RunningIntProp) {
    ...
    CHECK_EQ(g.hasRunningIntersection(), true);
  }
}   // namespace gum_tests
