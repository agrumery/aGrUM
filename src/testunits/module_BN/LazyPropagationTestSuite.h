...
namespace gum_tests {
  ...
  public:
    ...
    void /*test*/StaticEvidenceImpact() {
      ...
      TS_ASSERT_THROWS(ie_all.evidenceImpact(gum::NodeId(0), std::vector<gum::NodeId>{0, 1, 2}),
                       gum::InvalidArgument);

      auto res = ie_all.evidenceImpact(gum::NodeId(0), std::vector<gum::NodeId>{1, 2});
    ...
    void testEvidenceImpact() {
      ...
      */
      ...
      TS_ASSERT_EQUALS(res.nbrDim(), gum::Size(3));  // MarkovBlanket(E)=(A,D,C)
    }
