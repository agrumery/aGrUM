...
namespace gum_tests {
  ...
  GUM_TEST(InformationMethodsWithNames) {
    fill(*_bn);
  ...
  GUM_TEST(EvidenceProbabilityAsia) {
  ...
  GUM_TEST(EvidenceImpact) {
    ...
    */
    ...
    try {
      auto joint = bn.cpt("A") * bn.cpt("B") * bn.cpt("C") * bn.cpt("D") * bn.cpt("E") * bn.cpt("F")
                 * bn.cpt("H");
  ...
  GUM_TEST(JointEvidenceImpact) {
    ...
    */
    ...
    gum::LazyPropagation  ie(&bn);
    gum::Tensor< double > res;
    GUM_CHECK_ASSERT_THROWS_NOTHING(res = ie.evidenceJointImpact({"D", "E"}, {"A", "B", "C", "F"}));
    CHECK_EQ(res.nbrDim(), static_cast< gum::Size >(4));   // MarkovBlanket(E)=(A,D,C)
    try {
      auto joint = bn.cpt("A") * bn.cpt("B") * bn.cpt("C") * bn.cpt("D") * bn.cpt("E") * bn.cpt("F")
                 * bn.cpt("H");
  ...
  GUM_TEST(JointMutualInformation) {
    auto bn = gum::BayesNet< double >::fastPrototype("A->B->C->D;A->E->D;F->B;C->H;");
  ...
  GUM_TEST(MultipleJointMutualInformation) {
  ...
  GUM_TEST(OnChanged) {
...

    {
      ...
      const gum::Tensor< double > pot2bis = (p0 * p1 * p2 * p3 * p4 * ev4).sumOut(set2).normalize();
  ...
  GUM_TEST(MostProbableExplanation) {
    try {
      gum::BayesNet< double > bn;
  ...
  GUM_TEST(NonRegressionBugJoinTarget) {
    ...
    try {
      ...
      GUM_CHECK_TENSOR_ALMOST_EQUALS_SAMEVARS(ie.jointPosterior(bn.nodeset({"X00", "X01", "X06"})),
                                              bn_joint.sumIn(set).normalize());
    } catch (gum::Exception& e) { GUM_SHOWERROR(e); }
  }
}   // namespace gum_tests
