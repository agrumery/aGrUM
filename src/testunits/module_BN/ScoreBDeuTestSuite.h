...
#define GUM_CURRENT_SUITE  ScoreBDeu
#define GUM_CURRENT_MODULE BN

namespace gum_tests {

  struct ScoreBDeuTestSuite {
    private:
    ...
    double _score_(const std::vector< double >& N_ijk, const std::vector< double >& N_ij) const {
      ...
      if (!N_ij.empty()) {
        ...
        for (const auto n_ij: N_ij) {
          score += _gammalog2_(ess_qi) - _gammalog2_(n_ij + ess_qi);
        }
        for (const auto n_ijk: N_ijk) {
          score += _gammalog2_(n_ijk + ess_riqi) - _gammalog2_(ess_riqi);
        }
      } else {
        double N      = 0.0;
        double ess_ri = ess / N_ijk.size();
        for (const auto n_ijk: N_ijk) {
          score += _gammalog2_(n_ijk + ess_ri) - _gammalog2_(ess_ri);
          N += n_ijk;
        }
        score += _gammalog2_(ess) - _gammalog2_(N + ess);
      }
    ...
    public:
    void _test_no_range_no_nodeId2col() const {
      ...
      gum::learning::ScoreBDeu score(parser, prior);
      ...
      GUM_CHECK_ASSERT_THROWS_NOTHING(gum::learning::ScoreBDeu::isPriorCompatible(prior));
      GUM_CHECK_ASSERT_THROWS_NOTHING(
    ...
    void _test_no_range_has_nodeId2col() const {
      // create the translator set
      gum::LabelizedVariable var("X1", "", 0);
      var.addLabel("0");
      var.addLabel("1");
      ...
      gum::learning::ScoreBDeu score(parser, prior, nodeId2columns);
      score.setEffectiveSampleSize(2.0);
    ...
    void _test_has_range_no_nodeId2col() const {
      // create the translator set
      gum::LabelizedVariable var("X1", "", 0);
      var.addLabel("0");
      var.addLabel("1");
      ...
      gum::learning::NoPrior   prior(database);
      gum::learning::ScoreBDeu score(parser, prior, ranges);
      score.setEffectiveSampleSize(2.0);
    ...
    void _test_has_range_has_nodeId2col() const {
      // create the translator set
      gum::LabelizedVariable var("X1", "", 0);
      var.addLabel("0");
      var.addLabel("1");
      ...
      gum::learning::NoPrior   prior(database, nodeId2columns);
      gum::learning::ScoreBDeu score(parser, prior, ranges, nodeId2columns);
      score.setEffectiveSampleSize(2.0);
