  ...
  class PRMGeneratorTestSuite: public CxxTest::TestSuite {
    ...
    public:
      ...
      void testLayerGenerator() {
        ...
        // testing interfaces
        const Set<gum::prm::Interface*>& i_set = prm->interfaces();
        TS_ASSERT_EQUALS( i_set.size(), ( Size ) 10 );

        for( Set<gum::prm::Interface*>::const_iterator iter = i_set.begin(); iter != i_set.end(); ++iter ) {
          ...
          Size six = 0;
          Size two = 0;
          const Set<gum::prm::Attribute*>& attr = i.attributes();

          for( Set<gum::prm::Attribute*>::const_iterator a = attr.begin(); a != attr.end(); ++a ) {
        ...
        // testing classes
        const Set<gum::prm::Class*>& c_set = prm->classes();

        for( Set<gum::prm::Class*>::const_iterator c = c_set.begin(); c != c_set.end(); ++c ) {
          TS_ASSERT_EQUALS( ( **c ).attributes().size(), ( Size ) 30 );

          for( Set<gum::prm::Attribute*>::const_iterator a = ( **c ).attributes().begin(); a != ( **c ).attributes().end(); ++a ) {
