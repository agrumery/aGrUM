...
namespace gum_tests {
  ...
  class PRMInferenceTestSuite: public CxxTest::TestSuite {
    ...
    public:
      ...
      void testEvidence() {
        gum::prm::GroundedInference* g_ve = 0;
        ...
        const gum::prm::Instance& instance = prm->system( "aSys" ).get( "c1" );
        const gum::prm::Attribute& attribute = instance.get( "can_print" );
        gum::Potential<gum::prm::prm_float> e;
        e.add( attribute.type().variable() );
        e.fill( ( gum::prm::prm_float ) 0.0 );
        gum::Instantiation inst( e );
        inst.setFirst();
        e.set( inst, ( gum::prm::prm_float ) 1.0 );
      ...
      void testSmallGrdInference() {
        ...
        TS_GUM_ASSERT_THROWS_NOTHING( ve = new gum::VariableElimination<gum::prm::prm_float> ( bn ) );
        TS_GUM_ASSERT_THROWS_NOTHING( g_ve = new gum::prm::GroundedInference( *small, small->system( "microSys" ) ) );
        TS_GUM_ASSERT_THROWS_NOTHING( g_ve->setBNInference( ve ) );
        // Building query
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "c" );
          const gum::prm::Attribute& attribute = instance.get( "can_print" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
        ...
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "p" );
          const gum::prm::Attribute& attribute = instance.get( "equipState" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
          gum::Potential<gum::prm::prm_float> m;
          TS_GUM_ASSERT_THROWS_NOTHING( g_ve->marginal( chain, m ) );
          gum::Instantiation i( m );
        ...
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "pow" );
          const gum::prm::Attribute& attribute = instance.get( "powState" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
          gum::Potential<gum::prm::prm_float> m;
          TS_GUM_ASSERT_THROWS_NOTHING( g_ve->marginal( chain, m ) );
          gum::Instantiation i( m );
      ...
      void testSmallSVEInference() {
        gum::prm::SVE* sve = 0;
        TS_GUM_ASSERT_THROWS_NOTHING( sve = new gum::prm::SVE( *small, small->system( "microSys" ) ) );
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "c" );
          const gum::prm::Attribute& attribute = instance.get( "can_print" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
      ...
      void testSmallSVEDInference() {
        gum::prm::SVED* sved = 0;
        TS_GUM_ASSERT_THROWS_NOTHING( sved = new gum::prm::SVED( *small, small->system( "microSys" ) ) );
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "c" );
          const gum::prm::Attribute& attribute = instance.get( "can_print" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
      ...
      void testSmallStructInference() {
        gum::prm::StructuredInference* inf = 0;
        TS_GUM_ASSERT_THROWS_NOTHING( inf = new gum::prm::StructuredInference( *small, small->system( "microSys" ) ) );
        {
          const gum::prm::Instance& instance = small->system( "microSys" ).get( "c" );
          const gum::prm::Attribute& attribute = instance.get( "can_print" );
          gum::prm::PRMInference::Chain chain = std::make_pair( &instance, &attribute );
      ...
      void testInference() {
        ...
        for ( gum::DAG::NodeIterator node = bn.dag().beginNodes(); node != bn.dag().endNodes(); ++node ) {
          ...
          try {
            ...
            // GUM_TRACE("SVE done");
            gum::prm::SVED sved( *prm, prm->system( "aSys" ) );
            sved.marginal( chain, m_sved );
            // GUM_TRACE("SVED done");
            gum::prm::StructuredInference structinf( *prm, prm->system( "aSys" ) );
            ...
            for ( jnst.setFirst(); not jnst.end(); jnst.inc() ) {
              if ( m_sved.get( jnst ) != ( gum::prm::prm_float ) 0.0 ) {
                zero = false;
                break;
              }
            }
      ...
      void testStructuredBB() {
        ...
        TS_GUM_ASSERT_THROWS_NOTHING( bb = new gum::prm::StructuredBayesBall( sve ) );
        gum::prm::Instance& i = prm->system( "aSys" ).get( "pow" );
      ...
      void testEvidenceSioux() {
        ...
        // Adding evidence
        {
          const gum::prm::Instance& instance = prm->system( "aSys" ).get( "bw_printers[0]" );
          const gum::prm::Attribute& attribute = instance.get( "(t_degraded)equipState" );
          gum::Potential<gum::prm::prm_float> e;
          e.add( attribute.type().variable() );
          e.fill( ( gum::prm::prm_float ) 0.0 );
          gum::Instantiation inst( e );
        ...
        {
          const gum::prm::Instance& q_i = prm->system( "aSys" ).get( "pow" );
          const gum::prm::Attribute& q_a = q_i.get( "state" );
          gum::prm::PRMInference::Chain q_chain = std::make_pair( &q_i, &q_a );
          gum::Potential<gum::prm::prm_float> m;
      ...
      void testEvidenceSioux2() {
        gum::prm::SVE* g_ve = 0;
        TS_GUM_ASSERT_THROWS_NOTHING( g_ve = new gum::prm::SVE( *prm, prm->system( "aSys" ) ) );
        // Adding evidence
        {
          const gum::prm::Instance& instance = prm->system( "aSys" ).get( "bw_printers[0]" );
          const gum::prm::Attribute& attribute = instance.get( "(t_degraded)equipState" );
