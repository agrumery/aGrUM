...
class MixedGraphTestSuite: public CxxTest::TestSuite {
  ...
  public:
    ...
    void testInsert1() {
      ...
      TS_GUM_ASSERT_THROWS_NOTHING( graph.insertArc( 1,4 ) );
      TS_GUM_ASSERT_THROWS_NOTHING( graph.insertArc( 4,5 ) );
      TS_GUM_ASSERT_THROWS_NOTHING( graph.insertArc( 5,2 ) );
    }

    void testCopyConstructor() {
      gum::MixedGraph graph=buildGraph();

      gum::MixedGraph *copy=NULL;
    ...
    void testListMapEdges() {
      ...
      //          5_/             2 -- 4
      //                          5 -> 2
      TS_ASSERT_EQUALS( s, (gum::Size)(1+3+3+5+2+4) );
    }

    void testHashMapEdges() {
      gum::MixedGraph graph=buildGraph();

      gum::Property< gum::Size >::onEdges hashmap = graph.edgesProperty( &simpleEdgeMapFunction );
    ...
    void testListMapArcs() {
      ...
      //          5_/             2 -- 4
      //                          5 -> 2
      TS_ASSERT_EQUALS( s, (gum::Size)(1+4+4+5+5+2) );
    }
