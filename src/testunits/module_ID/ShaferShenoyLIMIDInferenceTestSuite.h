...
namespace gum_tests {
  ...
  GUM_TEST(InferenceWithOilWildCaterWithEvidenceOnDecisionNode) {
    ...
    GUM_CHECK_TENSOR_ALMOST_EQUALS_SAMEVARS(
        ...
        (gum::Tensor< double >() << net.variableFromName("OilContents")).fillWith({0.5, 0.3, 0.2}));
    GUM_CHECK_TENSOR_ALMOST_EQUALS_SAMEVARS(
  ...
  GUM_TEST(InferenceWithOilWildCaterWithEvidenceOnChanceNode) {
    ...
    GUM_CHECK_TENSOR_ALMOST_EQUALS_SAMEVARS(
        dIDI.posterior("OilContents"),
        (gum::Tensor< double >() << net.variableFromName("OilContents")).fillWith({0.0, 1.0, 0.0}));
    GUM_CHECK_TENSOR_ALMOST_EQUALS_SAMEVARS(
        dIDI.posteriorUtility("OilContents"),
  ...
  GUM_TEST(BugFromNeapolitan1) {
    ...
    CHECK_EQ(inf.optimalDecision("Buy").sum(), 9);
  }
  ...
  GUM_TEST(NewStructure) {
    {
      ...
      CHECK_EQ(res[0], gum::NodeSet({infdiag.idFromName("D2")}));
      CHECK_EQ(res[1], gum::NodeSet({infdiag.idFromName("D1")}));
    }
    {
      auto infdiag = gum::InfluenceDiagram< double >::fastPrototype("D1->Z->D2->X->$U");
      auto ieid    = gum::ShaferShenoyLIMIDInference< double >(&infdiag);
...

    {
      ...
      CHECK_EQ(res[0], gum::NodeSet({infdiag.idFromName("D4"), infdiag.idFromName("D7")}));
      CHECK_EQ(res[1], gum::NodeSet({infdiag.idFromName("D3"), infdiag.idFromName("D5")}));
      CHECK_EQ(res[2], gum::NodeSet({infdiag.idFromName("D2"), infdiag.idFromName("D6")}));
      CHECK_EQ(res[3], gum::NodeSet({infdiag.idFromName("D1")}));
    }
  }
  ...
  GUM_TEST(NoForgettingAssumption) {
    ...
    CHECK_EQ(dag.parents(infdiag.idFromName("D1")), infdiag.nodeset({}));
    CHECK_EQ(dag.parents(infdiag.idFromName("D2")), infdiag.nodeset({"D1"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("D3")), infdiag.nodeset({"D2"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("D4")), infdiag.nodeset({"D3", "R4"}));
    ...
    CHECK_EQ(dag.parents(infdiag.idFromName("D2")), infdiag.nodeset({"D1"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("D3")), infdiag.nodeset({"D2"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("D4")), infdiag.nodeset({"D2", "R4"}));


    CHECK_EQ(dag.parents(infdiag.idFromName("R1")), infdiag.nodeset({"D2"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("R2")), infdiag.nodeset({"R1"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("R3")), infdiag.nodeset({"R2", "D4"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("R4")), infdiag.nodeset({"R1"}));

    CHECK_EQ(dag.parents(infdiag.idFromName("U1")), infdiag.nodeset({"D3", "R2"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("U2")), infdiag.nodeset({"R3"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("U3")), infdiag.nodeset({"R1", "D1"}));
    CHECK_EQ(dag.parents(infdiag.idFromName("U4")), infdiag.nodeset({"R4", "D4"}));
  }
  ...
  GUM_TEST(NoForgettingAssumption2) {
    ...
    CHECK_EQ(revord1.size(), static_cast< gum::Size >(2));
    CHECK_EQ(revord1[0], limids.nodeset({"d4", "d2", "d3"}));
    CHECK_EQ(revord1[1], limids.nodeset({"d1"}));
    ...
    CHECK_EQ(revord2.size(), static_cast< gum::Size >(4));
    for (gum::Idx i = 0; i < static_cast< gum::Size >(4); i++) {
      CHECK_EQ(revord2[i].size(), static_cast< gum::Size >(1));
      CHECK_EQ(limids.variable(*(revord2[i].begin())).name(), order[3 - i]);
    }
    auto noForgetting = ieid.reducedLIMID();

    CHECK_EQ(noForgetting.parents("d1"), limids.nodeset({"b"}));
    CHECK_EQ(noForgetting.parents("d2"), limids.nodeset({"e"}));
    CHECK_EQ(noForgetting.parents("d3"), limids.nodeset({"f"}));
    CHECK_EQ(noForgetting.parents("d4"), limids.nodeset({"d2", "g"}));
  ...
  GUM_TEST(JunctionTree) {
    ...
    CHECK_EQ(jt->size(), static_cast< gum::Size >(5));
    ieid.makeInference();
  }
