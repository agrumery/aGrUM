...
namespace gum_tests {
  ...
  class CorrectedMutualInformationTestSuite : public CxxTest::TestSuite {
    ...
    void test_Ixy_Kmdl() {
      ...
      /*
            score.clear();
            TS_ASSERT_DELTA ( score.score( 0, 6, 5, std::vector<gum::Idx>{1, 4} )
         == 0.0 );
            TS_ASSERT_DELTA ( score.score( 6, 0, 5, std::vector<gum::Idx>{1, 4} )
         == 0.0 );
      */
    }
    ...
    void test_Ixy_NoCorr() {
      ...
      TS_ASSERT_DELTA(
        score.score(std::pair< gum::Idx, gum::Idx >{4, 7}), 0.0370, 0.001);
      TS_ASSERT_DELTA(
        score.score(1, 6, std::vector< gum::Idx >{0}), 0.0161, 0.001);
      ...
      TS_ASSERT_DELTA(
        score.score(2, 6, 5, std::vector< gum::Idx >{1}), -0.005, 0.001);
      score.clear();
