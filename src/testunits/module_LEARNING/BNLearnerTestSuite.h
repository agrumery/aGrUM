...
#undef createBoolVar
    ...
    void testNonRegressionZeroCount() {
      ...
      auto bn3 = learner.learnParameters(templ34.dag());

      gum::learning::BNLearner<double> learner4(
          GET_RESSOURCES_PATH("asia.csv"), templ34);
      learner4.useScoreAIC();
      auto bn4 = learner4.learnParameters(templ34.dag());
      TS_ASSERT_EQUALS(bn.cpt("lung_cancer?").toString(), bn3.cpt("lung_cancer?").toString());
      TS_ASSERT_EQUALS(bn.cpt("lung_cancer?").toString(), bn4.cpt("lung_cancer?").toString());
    }
  };
} /* namespace gum_tests */
