...
namespace gum {
  ...
  INLINE ShaferShenoyMNInference< GUM_SCALAR >::ShaferShenoyMNInference(
     const IMarkovNet< GUM_SCALAR >* MN, bool use_binary_join_tree) :
  ...
  INLINE ShaferShenoyMNInference< GUM_SCALAR >::~ShaferShenoyMNInference() {
    ...
    for (const auto& pots: __created_messages) {
    ...
    // hard evidence
    for (const auto& pot: __hard_ev_projected_factors) {
      delete pot.second;
    }

    // remove all the potentials in __clique_potentials that do not belong
    // to __clique_potentials_list : in this case, those potentials have been
    // created by combination of the corresponding list of potentials in
    // __clique_potentials_list. In other words, the size of this list is strictly
    // greater than 1.
    for (auto pot: __clique_potentials) {
      if (__clique_potentials_list[pot.first].size() > 1) { delete pot.second; }
    }
    ...
    if (__propagator != nullptr) delete __propagator;
  ...
  void ShaferShenoyMNInference< GUM_SCALAR >::setTriangulation(
    ...
    this->_setOutdatedMNStructureState();
  }
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::__setProjectionFunction(
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::__setCombinationFunction(
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::__invalidateAllMessages() {
    ...
    for (const auto& potset: __created_messages)
    ...
    if (this->isInferenceReady() || this->isDone())
      this->_setOutdatedMNPotentialsState();
  }

  /// fired when a new evidence is inserted
  template < typename GUM_SCALAR >
  INLINE void
     ShaferShenoyMNInference< GUM_SCALAR >::_onEvidenceAdded(const NodeId id,
                                                             bool isHardEvidence) {
    ...
    // of the undigraph
    if (isHardEvidence || !__reduced_graph.exists(id))
      __is_new_jt_needed = true;
    else {
      try {
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onEvidenceErased(
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::_onAllEvidenceErased(
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onEvidenceChanged(
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onMarginalTargetAdded(
     const NodeId id) {}


  /// fired before a target is removed
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onMarginalTargetErased(
     const NodeId id) {}


  /// fired after a new set target is inserted
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onJointTargetAdded(
     const NodeSet& set) {}


  /// fired before a set target is removed
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onJointTargetErased(
     const NodeSet& set) {}


  /// fired after all the nodes of the MN are added as single targets
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onAllMarginalTargetsAdded() {
  }
  ...
  template < typename GUM_SCALAR >
  INLINE void
     ShaferShenoyMNInference< GUM_SCALAR >::_onAllMarginalTargetsErased() {}

  /// fired after a new Markov net has been assigned to the engine
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onMarkovNetChanged(
     const IMarkovNet< GUM_SCALAR >* mn) {}

  /// fired before a all the joint_targets are removed
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onAllJointTargetsErased() {}


  /// fired before a all the single and joint_targets are removed
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_onAllTargetsErased() {}


  // check whether a new junction tree is really needed for the next inference
  template < typename GUM_SCALAR >
  bool ShaferShenoyMNInference< GUM_SCALAR >::__isNewJTNeeded() const {
    // if we do not have a JT or if __new_jt_needed is set to true, then
    // we know that we need to create a new join tree
    if ((__propagator == nullptr) || __is_new_jt_needed) return true;
    ...
    for (const auto node: this->targets()) {
      if (!__reduced_graph.exists(node) && !hard_ev_nodes.exists(node))
        return true;
    }
    for (const auto& joint_target: this->jointTargets()) {
      ...
      for (const auto node: joint_target) {
        bool found = true;
        try {
          const NodeSet& clique = __propagator->clique(__node_to_clique[node]);
    ...
    // to __reduced_graph, then we potentially have to reconstruct the join tree
    for (const auto& change: __evidence_changes) {
      if ((change.second == EvidenceChangeType::EVIDENCE_ADDED)
          && !__reduced_graph.exists(change.first))
        return true;
    }
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::__createNewJT() {
    ...
    for (const auto& nodeset: this->jointTargets())
      for (const auto n1: nodeset)
        for (const auto n2: nodeset)
          if (n1 < n2) __reduced_graph.addEdge(n1, n2);
    ...
    for (const auto node: __hard_ev_nodes) {
      __reduced_graph.eraseNode(node);
    }
    ...
    __triangulation->setGraph(&__reduced_graph, &(this->domainSizes()));
    ...
    if (__use_binary_join_tree) {
      ...
      NodeSet                        emptyset;
      __propagator = new CliqueGraph(
         bjt_converter.convert(triang_jt, this->domainSizes(), emptyset));
    } else {
      __propagator = new CliqueGraph(triang_jt);
    }
    ...
    // indicate, for each factor of the markov network a clique in __propagator
    // that can contain its conditional probability table
    __factor_to_clique.clear();
    ...
    for (const NodeId node: mn.nodes()) {
      if (__reduced_graph.exists(node)) {
        __node_to_clique.insert(
           node, __factor_to_clique[mn.smallestFactorFromNode(node)]);
      }
    }
    ...
    // compute the roots of __propagator's connected components
    __computeJoinTreeRoots();

    // remove all the Shafer-Shenoy potentials stored into the cliques
    for (const auto& xpot: __clique_potentials) {
      if (__clique_potentials_list[xpot.first].size() > 1) delete xpot.second;
    }
    __clique_potentials_list.clear();
    ...
    for (const auto clik: *__propagator) {
      __clique_potentials_list.insert(clik, empty_set);
    }

    // remove all the potentials created during the last inference
    for (const auto& potlist: __created_messages)
      for (auto pot: potlist.second)
        delete pot;
    __created_messages.clear();

    // remove all the potentials created to take into account hard evidence
    // during the last inference
    for (auto pot_pair: __hard_ev_projected_factors)
      delete pot_pair.second;
    __hard_ev_projected_factors.clear();
    ...
    for (const auto& edge: __propagator->edges()) {
    ...
    for (const auto& kv: mn.factors()) {
      ...
      for (const auto xnode: factor) {
        if (__hard_ev_nodes.contains(xnode)) hard_nodes_in_factor.insert(xnode);
      }
      ...
      if (hard_nodes_in_factor.empty()) {
        __clique_potentials_list[__factor_to_clique[factor]].insert(&pot);
      } else {
        ...
        if (hard_nodes_in_factor.size() != factor.size()) {
          ...
          __PotentialSet                 marg_factor_set;
          ...
          MultiDimCombineAndProjectDefault< GUM_SCALAR, Potential >
                         combine_and_project(__combination_op, SSNewMNprojPotential);
          __PotentialSet new_factor_list = combine_and_project.combineAndProject(
          ...
          __clique_potentials_list[__factor_to_clique[factor]].insert(
             projected_factor);
          __hard_ev_projected_factors.insert(factor, projected_factor);
        }
      }
    }
    ...
    for (const auto node: this->softEvidenceNodes()) {
      ...
      __clique_potentials_list[__node_to_clique[node]].insert(evidence[node]);
    }

    // now, in __clique_potentials_list, for each clique, we have the list of
    ...
    // store the result in __clique_potentials
    __clique_potentials.clear();
    MultiDimCombinationDefault< GUM_SCALAR, Potential > fast_combination(
       __combination_op);
    for (const auto& xpotset: __clique_potentials_list) {
      const auto& potset = xpotset.second;
      if (potset.size() > 0) {
        // here, there will be an entry in __clique_potentials
        // If there is only one element in potset, this element shall be
        // stored into __clique_potentials, else all the elements of potset
        // shall be combined and their result shall be stored
        if (potset.size() == 1) {
          __clique_potentials.insert(xpotset.first, *(potset.cbegin()));
        } else {
          auto joint = fast_combination.combine(potset);
          __clique_potentials.insert(xpotset.first, joint);
        }
      }
    }
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::_updateOutdatedMNStructure() {
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::__diffuseMessageInvalidations(
    ...
    if (message_computed) {
      ...
      if (__created_messages.exists(arc)) {
        auto& arc_created_potentials = __created_messages[arc];
      ...
      for (const auto node_id: __propagator->neighbours(to_id)) {
        if (node_id != from_id)
  ...
  /// messages
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::_updateOutdatedMNPotentials() {
    ...
    NodeProperty< bool > to_deallocate(__clique_potentials_list.size());
    for (auto pot_iter = __clique_potentials_list.cbegin();
         pot_iter != __clique_potentials_list.cend();
         ++pot_iter) {
        ...
        for (const auto& elt: mn.factors()) {
          ...
          if (chgFactor.contains(node)) {
            if (!hard_factors_changed.contains(chgFactor)) {
              ...
              const auto&  chgPot = __hard_ev_projected_factors[chgFactor];
              const NodeId chgClique = __factor_to_clique[chgFactor];
              __clique_potentials_list[chgClique].erase(chgPot);
              delete chgPot;
              __hard_ev_projected_factors.erase(chgFactor);
    ...
    NodeSet invalidated_cliques(__propagator->size());
    for (const auto& pair: __evidence_changes) {
      if (__node_to_clique.exists(pair.first)) {
        ...
        for (const auto neighbor: __propagator->neighbours(clique)) {
    ...
    for (const auto clique: invalidated_cliques)
      if (__clique_potentials.exists(clique) && to_deallocate[clique])
        delete __clique_potentials[clique];
    ...
    // and __clique_potentials_list and add the new soft ones
    for (auto& pot_pair: __node_to_soft_evidence)
      __clique_potentials_list[__node_to_clique[pot_pair.first]].erase(
         pot_pair.second);
    __node_to_soft_evidence.clear();
    ...
    // if we are performing _updateOutdatedMNPotentials, this means that the
    ...
    // performing the new projections. Idem for __hard_ev_projected_factors
    ...
    for (const auto& iter_factor: mn.factors()) {
      ...
      // perform the projection with a combine and project instance
      __PotentialSet                 marg_factor_set{iter_factor.second};
      Set< const DiscreteVariable* > hard_variables;
      ...
      MultiDimCombineAndProjectDefault< GUM_SCALAR, Potential >
                     combine_and_project(__combination_op, SSNewMNprojPotential);
      __PotentialSet new_potentials_list =
      ...
      __clique_potentials_list[__factor_to_clique[iter_factor.first]].insert(
         projected_factor);
      __hard_ev_projected_factors.insert(iter_factor.first, projected_factor);
    }
    ...
    MultiDimCombinationDefault< GUM_SCALAR, Potential > fast_combination(
       __combination_op);
    for (const auto clique: invalidated_cliques) {
      const auto& potset = __clique_potentials_list[clique];
      ...
      if (potset.size() > 0) {
        ...
        // shall be combined and their result shall be stored
        if (potset.size() == 1) {
          __clique_potentials[clique] = *(potset.cbegin());
        } else {
          auto joint = fast_combination.combine(potset);
          __clique_potentials[clique] = joint;
        }
      }
    }
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::__computeJoinTreeRoots() {
    const auto& mn = this->MN();
    ...
    for (const auto clique_id: clique_targets) {
      const auto& clique = __propagator->clique(clique_id);
      Size        dom_size = 1;
    ...
    NodeProperty< bool > marked = __propagator->nodesProperty(false);
    std::function< void(NodeId, NodeId) > diffuse_marks =
       [&marked, &diffuse_marks, this](NodeId node, NodeId from) {
         if (!marked[node]) {
           marked[node] = true;
           for (const auto neigh: __propagator->neighbours(node))
  ...
  template < typename GUM_SCALAR >
  INLINE void
     ShaferShenoyMNInference< GUM_SCALAR >::__collectMessage(NodeId id,
                                                             NodeId from) {
    for (const auto other: __propagator->neighbours(id)) {
  ...
  Set< const Potential< GUM_SCALAR >* >
     ShaferShenoyMNInference< GUM_SCALAR >::__marginalizeOut(
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >::__produceMessage(NodeId from_id,
                                                               NodeId to_id) {
    // get the potentials of the clique.
    __PotentialSet pot_list;
    if (__clique_potentials.exists(from_id))
      pot_list.insert(__clique_potentials[from_id]);

    // add the messages sent by adjacent nodes to from_id
    for (const auto other_id: __propagator->neighbours(from_id))
      if (other_id != to_id)
        pot_list += __separator_potentials[Arc(other_id, from_id)];

    // get the set of variables that need be removed from the potentials
    const NodeSet& from_clique = __propagator->clique(from_id);
    const NodeSet& separator = __propagator->separator(from_id, to_id);
    ...
    if (!new_pot_list.empty()) {
      if (new_pot_list.size() == 1) {   // there is only one potential
        ...
        if (!pot_list.exists(pot)) {
          if (!__created_messages.exists(arc))
            __created_messages.insert(arc, __PotentialSet());
          __created_messages[arc].insert(pot);
        }
      } else {
        ...
        if (!__created_messages.exists(arc))
          __created_messages.insert(arc, __PotentialSet());
        __created_messages[arc].insert(joint);
  ...
  template < typename GUM_SCALAR >
  INLINE void ShaferShenoyMNInference< GUM_SCALAR >::_makeInference() {
    ...
    // their associated joint_target do not belong to __reduced_graph)
  ...
  Potential< GUM_SCALAR >*
     ShaferShenoyMNInference< GUM_SCALAR >::_unnormalizedJointPosterior(
        NodeId id) {
    const auto& mn = this->MN();
    ...
    __PotentialSet pot_list;
    if (__clique_potentials.exists(clique_of_id))
      pot_list.insert(__clique_potentials[clique_of_id]);

    // add the messages sent by adjacent nodes to targetCliquxse
    for (const auto other: __propagator->neighbours(clique_of_id))
      pot_list += __separator_potentials[Arc(other, clique_of_id)];

    // get the set of variables that need be removed from the potentials
    const NodeSet&                 nodes = __propagator->clique(clique_of_id);
  ...
  template < typename GUM_SCALAR >
  const Potential< GUM_SCALAR >&
     ShaferShenoyMNInference< GUM_SCALAR >::_posterior(NodeId id) {
  ...
  Potential< GUM_SCALAR >*
    ...
    try {
      ...
      const NodeSet& clique_nodes = __propagator->clique(clique_of_set);
      for (const auto node: targets) {
    ...
    __PotentialSet pot_list;
    if (__clique_potentials.exists(clique_of_set))
      pot_list.insert(__clique_potentials[clique_of_set]);

    // add the messages sent by adjacent nodes to targetClique
    for (const auto other: __propagator->neighbours(clique_of_set))
      pot_list += __separator_potentials[Arc(other, clique_of_set)];

    // get the set of variables that need be removed from the potentials
    const NodeSet&                 nodes = __propagator->clique(clique_of_set);
    Set< const DiscreteVariable* > del_vars(nodes.size());
  ...
  template < typename GUM_SCALAR >
  const Potential< GUM_SCALAR >&
     ShaferShenoyMNInference< GUM_SCALAR >::_jointPosterior(const NodeSet& set) {
    // check if we have already computed the posterior
  ...
  template < typename GUM_SCALAR >
  const Potential< GUM_SCALAR >&
     ShaferShenoyMNInference< GUM_SCALAR >::_jointPosterior(
  ...
  GUM_SCALAR ShaferShenoyMNInference< GUM_SCALAR >::evidenceProbability() {
    ...
    for (const auto root: __roots) {
      ...
      NodeId                   node = *(__propagator->clique(root).begin());
