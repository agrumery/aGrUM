...
namespace gum {
  ...
  bool ShaferShenoyMNInference< GUM_SCALAR >:: _isNewJTNeeded_() const {
    ...
    for (const auto node: this->targets()) {
      if (! _reduced_graph_.exists(node) && !hard_ev_nodes.exists(node))
        return true;
    }
    ...
    for (const NodeId node: mn.nodes()) {
      if ( _reduced_graph_.exists(node)) {
         _node_to_clique_.insert(
           node,
            _factor_to_clique_[mn.smallestFactorFromNode(node)]);
      }
    }
      ...
      if (hard_nodes_in_factor.empty()) {
        ...
        if (hard_nodes_in_factor.size() != factor.size()) {
          ...
          MultiDimCombineAndProjectDefault< GUM_SCALAR, Potential >
             combine_and_project( _combination_op_, SSNewMNprojPotential);
          ...
          const Potential< GUM_SCALAR >* projected_factor
           ...
           _clique_potentials_list_[ _factor_to_clique_[factor]].insert(
             projected_factor);
  ...
  void ShaferShenoyMNInference< GUM_SCALAR >::updateOutdatedPotentials_() {
    ...
    for (auto& pot_pair:  _node_to_soft_evidence_)
       _clique_potentials_list_[ _node_to_clique_[pot_pair.first]].erase(
         pot_pair.second);
     _node_to_soft_evidence_.clear();
    ...
    for (const auto& iter_factor: mn.factors()) {
      ...
      const Potential< GUM_SCALAR >* projected_factor
       ...
       _clique_potentials_list_[ _factor_to_clique_[iter_factor.first]].insert(
         projected_factor);
  ...
  template < typename GUM_SCALAR >
  INLINE void
     ShaferShenoyMNInference< GUM_SCALAR >:: _collectMessage_(NodeId id,
                                                             NodeId from) {
  ...
  Set< const Potential< GUM_SCALAR >* >
     ShaferShenoyMNInference< GUM_SCALAR >:: _marginalizeOut_(
  ...
  template < typename GUM_SCALAR >
  void ShaferShenoyMNInference< GUM_SCALAR >:: _produceMessage_(NodeId from_id,
                                                               NodeId to_id) {
    // get the potentials of the clique.
     _PotentialSet_ pot_list;
    if ( _clique_potentials_.exists(from_id))
      pot_list.insert( _clique_potentials_[from_id]);
    ...
    if (!new_pot_list.empty()) {
      if (new_pot_list.size() == 1) {   // there is only one potential
        ...
        if (!pot_list.exists(pot)) {
          if (! _created_messages_.exists(arc))
             _created_messages_.insert(arc,  _PotentialSet_());
