  ...
  namespace prm {
    ...
    void PRMFactory<GUM_SCALAR>::addParameter(const std::string& type,
      ...
      if (type == "int") {
        p = new Parameter<GUM_SCALAR>(
            name, Parameter<GUM_SCALAR>::ParameterType::INT, (GUM_SCALAR)value);
      } else if (type == "real") {
        p = new Parameter<GUM_SCALAR>(
    ...
    bool PRMFactory<GUM_SCALAR>::__retrieveInputs(
      ...
      for (size_t i = 0; i < chains.size(); ++i) {
        try {
          ...
          retVal =
              retVal or ClassElement<GUM_SCALAR>::isSlotChain(*(inputs.back()));
        } catch (NotFound&) {
    ...
    PRMFactory<GUM_SCALAR>::startAttribute(const std::string& type,
      ...
      if (PRMObject::isClass(*c)) {
        a = new FormAttribute<GUM_SCALAR>(static_cast<Class<GUM_SCALAR>&>(*c),
                                          name, *__retrieveType(type));
      } else {
    ...
    template <typename GUM_SCALAR>
    INLINE void PRMFactory<GUM_SCALAR>::__addInstance(Class<GUM_SCALAR>* type,
                                                      const std::string& name) {
