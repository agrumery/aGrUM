      ...
      for( const auto attr : attr_set ) {
        try {
          for( auto iter = query->getRefAttr( attr ).begin(); iter != query->getRefAttr( attr ).end(); ++iter )
            if( ( not ignore.exists( iter->first ) )
                and ( __bb.exists( iter->first ) ) )
        ...
        if( __bb.requisiteNodes( *i ).exists( elt.first ) ) {
          if( ClassElement<GUM_SCALAR>::isAttribute( c.get( elt.first ) ) ) {
            ...
            for( const auto par : c.dag().parents( elt.first ) )
              if( ClassElement<GUM_SCALAR>::isAttribute( i->type().get( par ) )
                  and i->type().isInnerNode( i->type().get( par ) )
                  and __bb.requisiteNodes( i ).exists( par ) ) {
                inners.erase( par );
                outers.insert( par );
              }
          }
