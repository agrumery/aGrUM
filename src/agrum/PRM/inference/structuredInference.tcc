      ...
      if( data.queries().size() )
        for( const auto m : data.matches )
          if( not( m->exists( const_cast<Instance<GUM_SCALAR>*>( __query.first ) ) ) )
            eliminateNode( & ( m->atPos( __query_data.first )->get( __query_data.second ).type().variable() ), * ( __elim_map[m] ), __trash );
    }
    ...
    StructuredInference<GUM_SCALAR>::__buildPatternGraph( StructuredInference<GUM_SCALAR>::PData& data,
      ...
      for( const auto inst : match ) {
        for( const auto & elt : *inst ) {
          ...
          if( data.inners().exists( id ) and ( inst->type().dag().children( elt.second->id() ).size() == 0 ) and __allInstanceNoRefAttr( data, v ) )
