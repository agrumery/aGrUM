...
namespace gum {
  namespace prm {
    ...
    template<typename GUM_SCALAR>
    void
    GroundedInference<GUM_SCALAR>::_evidenceAdded( const typename PRMInference<GUM_SCALAR>::Chain& chain ) {
    ...
    template<typename GUM_SCALAR>
    void
    GroundedInference<GUM_SCALAR>::_evidenceRemoved( const typename PRMInference<GUM_SCALAR>::Chain& chain ) {
      std::stringstream var_name;
      var_name << chain.first->name() << "." << chain.second->safeName();
      const DiscreteVariable& var = __inf->bn().variableFromName( var_name.str() );

      for( auto iter = __obs.beginSafe(); iter != __obs.endSafe(); ++iter ) {// safe iterator needed here
    ...
    GroundedInference<GUM_SCALAR>::GroundedInference( const PRM<GUM_SCALAR>& prm, const System<GUM_SCALAR>& system ) :
      PRMInference<GUM_SCALAR> ( prm, system ), __inf( 0 ) {
      GUM_CONSTRUCTOR( GroundedInference );
    }

    template<typename GUM_SCALAR> INLINE
    GroundedInference<GUM_SCALAR>::GroundedInference( const GroundedInference<GUM_SCALAR>& source ) :
    ...
    template<typename GUM_SCALAR> INLINE
    void
    GroundedInference<GUM_SCALAR>::setBNInference( BayesNetInference<GUM_SCALAR>* bn_inf ) {
      if( __inf != 0 ) {
        delete __inf;
      }
    ...
    template<typename GUM_SCALAR> INLINE
    void
    GroundedInference<GUM_SCALAR>::_marginal( const typename PRMInference<GUM_SCALAR>::Chain& chain, Potential<GUM_SCALAR>& m ) {
      if( __inf == 0 ) {
    ...
    template<typename GUM_SCALAR> INLINE
    void
    GroundedInference<GUM_SCALAR>::_joint( const std::vector< typename PRMInference<GUM_SCALAR>::Chain >& queries, Potential<GUM_SCALAR>& j ) {
