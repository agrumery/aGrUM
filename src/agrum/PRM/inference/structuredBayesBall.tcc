    ...
    StructuredBayesBall<GUM_SCALAR>::__fillMaps( InstanceMap& marks ) {
      ...
      for( const auto & elt : req_map ) {
        ...
        if( __reqMap.exists( key ) ) {
          __keyMap.insert( elt.first, std::pair<std::string, Set<NodeId>* > ( key, __reqMap[key].first ) );
          __reqMap[key].second += 1;
          delete elt.second;
          req_map[elt.first] = 0;
        } else {
          __reqMap.insert( key, std::pair< Set<NodeId>*, Size> ( elt.second, 1 ) );
          __keyMap.insert( elt.first, std::pair<std::string, Set<NodeId>* > ( key, elt.second ) );
        }
      }
    }
