  ...
  namespace prm {
    ...
    SVE<GUM_SCALAR>::__eliminateNodesWithEvidence( const Instance<GUM_SCALAR>* i, BucketSet& pool, BucketSet& trash, Set<NodeId>* delayedVars ) {
      ...
      for( const auto & elt : this->evidence( i ) ) {
        inner = i->type().isInputNode( i->get( elt.first ) ) or i->type().isInnerNode( i->get( elt.first ) );
      ...
      for( const auto node : c.dag().nodes() )
        if( ClassElement<GUM_SCALAR>::isAttribute( c.get( node ) ) ) {
          ...
          for( const auto par : c.dag().parents( node ) )
            if( ClassElement<GUM_SCALAR>::isAttribute( c.get( par ) ) and c.isInnerNode( c.get( par ) ) ) {
              inners.erase( par );
              outers.insert( par );
            }
        }
