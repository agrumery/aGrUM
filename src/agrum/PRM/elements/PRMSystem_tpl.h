...
namespace gum {
    namespace prm {
        ...
        INLINE PRMSystem<GUM_SCALAR>::PRMSystem(const PRMSystem &from) :
                PRMObject(from), _skeleton(from._skeleton), _nodeIdMap(from._nodeIdMap),
                _nameMap(from._nameMap), _instanceMap(from._instanceMap),
                _arrayMap(from._arrayMap) {
            GUM_CONS_CPY(PRMSystem);
        }

        template<typename GUM_SCALAR>
        NodeId PRMSystem<GUM_SCALAR>::add(PRMInstance <GUM_SCALAR> *i) {
            if (_nameMap.exists(i->name())) {
                GUM_ERROR(
                        DuplicateElement,
                        "an Instance<GUM_SCALAR> with the same is already in this System");
            }

            NodeId id = _skeleton.addNode();
            _nodeIdMap.insert(id, i);
            _nameMap.insert(i->name(), i);

            try {
                _instanceMap[&(i->type())]->insert(i);
            } catch (NotFound &) {
                _instanceMap.insert(&(i->type()), new Set<PRMInstance<GUM_SCALAR> *>());
                _instanceMap[&(i->type())]->insert(i);
            }
        ...
        PRMSystem<GUM_SCALAR>::get(const PRMInstance <GUM_SCALAR> &i) const {
            try {
                return _nodeIdMap.keyByVal(const_cast< PRMInstance<GUM_SCALAR> * >(&i));
            } catch (NotFound &) {
                GUM_ERROR(NotFound, "found no Instance<GUM_SCALAR> matching the given id");
            }
        }
        ...
        PRMSystem<GUM_SCALAR>::get(const PRMClass <GUM_SCALAR> &type) const {
            try {
                return *(_instanceMap[const_cast< PRMClass<GUM_SCALAR> * >(&type)]);
            } catch (NotFound &) {
                GUM_ERROR(
                        NotFound,
        ...
        INLINE NodeId PRMSystem<GUM_SCALAR>::add(const std::string &array,
                                                 PRMInstance <GUM_SCALAR> *i) {
            try {
                if (i->type().isSubTypeOf(*(_arrayMap[array].first))) {
                    NodeId id = add(i);
                    _arrayMap[array].second->insert(i);
                    return id;
                } else {
                    GUM_ERROR(TypeError,
        ...
        INLINE void PRMSystem<GUM_SCALAR>::addArray(
            ...
            if (_arrayMap.exists(array)) {
                GUM_ERROR(DuplicateElement,
                          "an array '" << array << "' is already in this System");
            }

            _arrayMap.insert(array,
        ...
        PRMSystem<GUM_SCALAR>::begin(const std::string &a) {
            try {
                return _arrayMap[a].second->begin();
            } catch (NotFound &) {
                GUM_ERROR(NotFound, "found no array matching the given name");
            }
        }
        ...
        INLINE bool PRMSystem<GUM_SCALAR>::exists(const std::string &name) const {
            return _nameMap.exists(name) || _arrayMap.exists(name);
        }
