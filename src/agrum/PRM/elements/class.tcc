...
namespace gum {
  namespace prm {
    ...
    Class<GUM_SCALAR>::Class( const std::string& name, Class<GUM_SCALAR>&   super, const Set<Interface<GUM_SCALAR>*>& set ) :
      ClassElementContainer<GUM_SCALAR> ( name ), __dag( super.dag() ),
      __super( &super ), __implements( nullptr ), __instantiations( super.__instantiations ) {
      GUM_CONSTRUCTOR( Class );
      super.__addExtension( this );
      __inheritClass( super );
    ...
    Class<GUM_SCALAR>::__inheritClass( const Class<GUM_SCALAR>&   c ) {
      ...
      for( const auto c_agg : c.__aggregates ) {
        ...
        try {
          agg = new Aggregate<GUM_SCALAR> ( c_agg->name(), c_agg->agg_type(), c_agg->type(), c_agg->label() );
        } catch( OperationNotAllowed& ) {
          agg = new Aggregate<GUM_SCALAR> ( c_agg->name(), c_agg->agg_type(), c_agg->type() );
        }
      ...
      for( const auto c_slotchain : c.__slotChains ) {
        ...
        SlotChain<GUM_SCALAR>* sc = new SlotChain<GUM_SCALAR> ( c_slotchain->name(), chain );
      ...
      for( const auto slotchain : __slotChains ) {
        ...
        if( ( slotchain->chain().atPos( 0 ) == overloaded ) ) {
          ...
          if( seq.back()->type() != slotchain->lastElt().type() ) {
            seq.erase( seq.back() );
            seq.insert( & ( static_cast<ReferenceSlot<GUM_SCALAR>*>( seq.back() )->slotType().get( slotchain->lastElt().safeName() ) ) );
