...
namespace gum {
  namespace prm {
    ...
    System<GUM_SCALAR>::addArc( const std::string& u_name, const std::string& v_name,
      ...
      try {
        ...
        if( elt.elt_type() == ClassElement<GUM_SCALAR>::prm_refslot ) {
          ...
          GUM_ERROR( WrongClassElement, "the given name does not match a ReferenceSlot<GUM_SCALAR> in u's type" );
        }
      } catch( NotFound& ) {
        GUM_ERROR( NotFound, "found no ClassElement<GUM_SCALAR> matching the given name" );
      }
    ...
    template<typename GUM_SCALAR>
    NodeId
    System<GUM_SCALAR>::add( Instance<GUM_SCALAR>* i ) {
    ...
    template<typename GUM_SCALAR>
    void
    System<GUM_SCALAR>::groundedBN( BayesNetFactory<GUM_SCALAR>& factory ) const {
    ...
    template<typename GUM_SCALAR>
    void
    System<GUM_SCALAR>::__groundAgg( const ClassElement<GUM_SCALAR>& elt, const std::string& name, BayesNetFactory<GUM_SCALAR>& factory ) const {
      ...
      const Aggregate<GUM_SCALAR>& agg = static_cast<const Aggregate<GUM_SCALAR>&>( elt );
    ...
    System<GUM_SCALAR>::__groundRef( const Instance<GUM_SCALAR>& instance, BayesNetFactory<GUM_SCALAR>& factory ) const {
      for( const auto & elt : instance ) {
        ...
        for( const auto par : instance.type().dag().parents( elt.second->id() ) ) {
          switch( instance.type().get( par ).elt_type() ) {
            ...
            case ClassElement<GUM_SCALAR>::prm_slotchain: {
              std::string parent_name = static_cast<const SlotChain<GUM_SCALAR>&>( instance.type().get( par ) ).lastElt().safeName();
        ...
        if( ClassElement<GUM_SCALAR>::isAttribute( instance.type().get( elt.second->safeName() ) ) )
    ...
    template<typename GUM_SCALAR>
    void
    System<GUM_SCALAR>::__groundPotential( const Instance<GUM_SCALAR>& instance, const Attribute<GUM_SCALAR>& attr, BayesNetFactory<GUM_SCALAR>& factory ) const {
      ...
      for( const auto parent : instance.type().dag().parents( attr.id() ) ) {
        switch( instance.type().get( parent ).elt_type() ) {
          ...
          case ClassElement<GUM_SCALAR>::prm_slotchain: {
            std::stringstream parent_name;
            const SlotChain<GUM_SCALAR>& sc = static_cast<const SlotChain<GUM_SCALAR>&>( instance.type().get( parent ) );
            parent_name << instance.getInstance( sc.id() ).name() << "." << sc.lastElt().safeName();
            bijection.insert( & ( instance.getInstance( sc.id() ).get( sc.lastElt().safeName() ).type().variable() ),
    ...
    template<typename GUM_SCALAR> INLINE
    Instance<GUM_SCALAR>&
    System<GUM_SCALAR>::get( NodeId id ) {
      try {
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    System<GUM_SCALAR>::isInstance( const std::string& name ) const {
      return __nameMap.exists( name );
    }

    template<typename GUM_SCALAR> INLINE
    bool
    System<GUM_SCALAR>::isArray( const std::string& name ) const {
    ...
    System<GUM_SCALAR>::add( const std::string& array, Instance<GUM_SCALAR>* i ) {
      try {
        if( i->type().isSubTypeOf( * ( __arrayMap[array].first ) ) ) {
          ...
          GUM_ERROR( TypeError, "the given Instance<GUM_SCALAR> is of an incorrect Class<GUM_SCALAR> type" );
        }
      } catch( NotFound& ) {
        GUM_ERROR( NotFound, "found no array matching the given name" );
      }
    ...
    template<typename GUM_SCALAR> INLINE
    void
    System<GUM_SCALAR>::addArray( const std::string& array, ClassElementContainer<GUM_SCALAR>& type ) {
      if( __arrayMap.exists( array ) ) {
        GUM_ERROR( DuplicateElement, "an array with the same is already in this System" );
      }

      __arrayMap.insert( array, System<GUM_SCALAR>::model_pair( &type, new Sequence<Instance<GUM_SCALAR>*>() ) );
    }
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    System<GUM_SCALAR>::exists( const std::string& name ) const {
