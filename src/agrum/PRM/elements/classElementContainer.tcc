...
namespace gum {
  ...
  namespace prm {
    ...
    template<typename GUM_SCALAR> INLINE
    ClassElementContainer<GUM_SCALAR>::~ClassElementContainer() {
      GUM_DESTRUCTOR( ClassElementContainer );
    }

    template<typename GUM_SCALAR> INLINE
    ClassElementContainer<GUM_SCALAR>&
    ClassElementContainer<GUM_SCALAR>::operator= ( const ClassElementContainer<GUM_SCALAR>& source ) {
      GUM_ERROR( FatalError, "illegal call to ClassElementContainer copy operator" );
    }

    template<typename GUM_SCALAR> INLINE
    ClassElementContainer<GUM_SCALAR>::ClassElementContainer( const ClassElementContainer<GUM_SCALAR>& source ) :
      PRMObject( source ) {
...
//
    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::isInputNode( const ClassElement<GUM_SCALAR>& elt ) const {
      try {
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::isOutputNode( const ClassElement<GUM_SCALAR>& elt ) const {
      try {
    ...
    ClassElementContainer<GUM_SCALAR>::setOutputNode( const ClassElement<GUM_SCALAR>& elt, bool b ) {
      ...
      if( not exists( elt.safeName() ) ) {
        ...
        GUM_ERROR( WrongClassElement, "given ClassElement<GUM_SCALAR> is not an Attribute or an Aggregate" );
      }
    }

    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::isInnerNode( const ClassElement<GUM_SCALAR>& elt ) const {
      try {
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::isSuperTypeOf( const ClassElementContainer<GUM_SCALAR>& cec ) const {
      return cec.isSubTypeOf( *this );
    }

    template<typename GUM_SCALAR> INLINE
    std::pair<bool, bool>&
    ClassElementContainer<GUM_SCALAR>::_getIOFlag( const ClassElement<GUM_SCALAR>& elt ) {
      try {
    ...
    ClassElementContainer<GUM_SCALAR>::_getIOFlag( const ClassElement<GUM_SCALAR>& elt ) const {
      try {
        return __IOFlags[elt.safeName()];
      } catch( NotFound& ) {
        GUM_ERROR( NotFound, "this ClassElement<GUM_SCALAR> does not have any IO flags" );
    ...
    template<typename GUM_SCALAR> INLINE
    void
    ClassElementContainer<GUM_SCALAR>::_setIOFlag( const ClassElement<GUM_SCALAR>& elt, const std::pair<bool, bool>& flags ) {
      try {
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::exists( NodeId id ) const {
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    ClassElementContainer<GUM_SCALAR>::belongsTo( const ClassElement<GUM_SCALAR>& elt ) const {
      try {
    ...
    template<typename GUM_SCALAR> INLINE
    const DAG&
    ClassElementContainer<GUM_SCALAR>::dag() const {
      return _dag();
    }
...
std::ostream&
operator<< ( std::ostream& output, const gum::prm::ClassElementContainer<GUM_SCALAR>& container ) {
  std::string tab = "  ";
