...
namespace gum {
  namespace prm {
    ...
    template<typename GUM_SCALAR>
    Type<GUM_SCALAR>::Type( Type<GUM_SCALAR>& super_type, const std::vector<Idx>& label_map,
    ...
    template<typename GUM_SCALAR>
    bool
    Type<GUM_SCALAR>::isSubTypeOf( const Type<GUM_SCALAR>& super ) const {
      if ( ( *this ) == super ) {
        return true;
      } else if ( __super ) {
    ...
    template<typename GUM_SCALAR>
    bool
    Type<GUM_SCALAR>::__isValid() const {
      if ( not __super ) {
    ...
    Type<GUM_SCALAR>&
    Type<GUM_SCALAR>::super() { return *__super; }

    template<typename GUM_SCALAR> INLINE
    DiscreteVariable&
    Type<GUM_SCALAR>::variable() { return *__var; }

    template<typename GUM_SCALAR> INLINE
    const DiscreteVariable&
    Type<GUM_SCALAR>::variable() const { return *__var; }

    template<typename GUM_SCALAR> INLINE
    DiscreteVariable&
    Type<GUM_SCALAR>::operator*() { return *__var; }

    template<typename GUM_SCALAR> INLINE
    const DiscreteVariable&
    Type<GUM_SCALAR>::operator*() const { return *__var; }
    ...
    template<typename GUM_SCALAR> INLINE
    PRMObject::PRMType
    Type<GUM_SCALAR>::obj_type() const { return PRMObject::PRMType::TYPE; }

    template<typename GUM_SCALAR> INLINE
    const std::string&
    Type<GUM_SCALAR>::name() const { return PRMObject::name(); }

    template<typename GUM_SCALAR> INLINE
    const Type<GUM_SCALAR>&
    Type<GUM_SCALAR>::super() const {
      if ( __super ) {
        return *__super;
      } else {
    ...
    template<typename GUM_SCALAR> INLINE
    void
    Type<GUM_SCALAR>::setSuper( Type<GUM_SCALAR>& t ) {
      try {
        if ( t != super() ) {
    ...
    Type<GUM_SCALAR>::label_map() const {
      if ( __label_map ) {
        return *__label_map;
      } else {
        GUM_ERROR( NotFound, "No super type for this type." );
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    Type<GUM_SCALAR>::isSubType() const { return __super; }
    ...
    template<typename GUM_SCALAR> INLINE
    bool
    Type<GUM_SCALAR>::operator==( const PRMObject& obj ) const {
