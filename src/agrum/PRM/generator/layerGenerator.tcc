...
namespace gum {
  namespace prm {
    ...
    PRM<GUM_SCALAR>*
    LayerGenerator<GUM_SCALAR>::generate() {
      if ( __layers.size() == 0 ) {
    ...
    template<typename GUM_SCALAR>
    void
    LayerGenerator<GUM_SCALAR>::__generateInterfaces ( PRMFactory<GUM_SCALAR>& f, const std::string& type, std::vector<LayerGenerator<GUM_SCALAR>::MyData>& l ) {
    ...
    template<typename GUM_SCALAR>
    void
    LayerGenerator<GUM_SCALAR>::__generateClassDag ( Size lvl, DAG& dag,
        Bijection<std::string, NodeId>& names,
    ...
    template<typename GUM_SCALAR>
    void
    LayerGenerator<GUM_SCALAR>::__generateSystem ( PRMFactory<GUM_SCALAR>& factory, std::vector<LayerGenerator<GUM_SCALAR>::MyData>& l ) {
    ...
    template<typename GUM_SCALAR> INLINE
    Size
    LayerGenerator<GUM_SCALAR>::getDomainSize() const {
      return __domain_size;
    }

    template<typename GUM_SCALAR> INLINE
    void
    LayerGenerator<GUM_SCALAR>::setDomainSize ( Size s ) {
      __domain_size = s;
    }

    template<typename GUM_SCALAR> INLINE
    unsigned int
    LayerGenerator<GUM_SCALAR>::getMaxParents() const {
      return __max_parents;
    }

    template<typename GUM_SCALAR> INLINE
    void
    LayerGenerator<GUM_SCALAR>::setMaxParents ( Size s ) {
      __max_parents = s;
    }

    template<typename GUM_SCALAR> INLINE
    void
    LayerGenerator<GUM_SCALAR>::setLayers ( const std::vector<LayerGenerator<GUM_SCALAR>::LayerData>& v ) {
      __layers = v;
    }
