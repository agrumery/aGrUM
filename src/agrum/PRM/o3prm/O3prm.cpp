...
namespace gum {
  namespace prm {
    namespace o3prm {
      ...
      O3Position::O3Position() :  _file_(""),  _line_(0),  _column_(0) {
      ...
      O3Integer::O3Integer(const O3Position& pos, int value) :
           _pos_(pos),  _value_(value) {
        GUM_CONSTRUCTOR(O3Integer);
      }

      O3Integer::O3Integer(const O3Integer& src) :
           _pos_(src. _pos_),  _value_(src. _value_) {
      ...
      O3Formula::O3Formula() :
           _pos_(),  _formula_(std::unique_ptr< Formula >(new Formula(""))) {
      ...
      O3Label::O3Label(const O3Label& src) :
           _pos_(src. _pos_),  _label_(src. _label_) {
      ...
      O3IntType::O3IntType(const O3IntType& src) :
           _pos_(src. _pos_),  _name_(src. _name_),  _start_(src. _start_),
           _end_(src. _end_),  _dep_flag_(src. _dep_flag_) {
      ...
      O3RealType::O3RealType(const O3RealType& src) :
           _pos_(src. _pos_),  _name_(src. _name_),  _values_(src. _values_),
           _dep_flag_(src. _dep_flag_) {
      ...
      O3RawCPT::O3RawCPT(O3RawCPT&& src) :
          O3Attribute(src),  _values_(std::move(src. _values_)) {
      ...
      O3RuleCPT::O3RuleCPT(O3RuleCPT&& src) :
          O3Attribute(src),  _rules_(std::move(src. _rules_)) {
      ...
      O3Parameter::O3Parameter(const O3Position& pos,
           ...
           _type_(O3Parameter::PRMType::INT),
           _pos_(pos),  _name_(name),
      ...
      O3Parameter::O3Parameter(const O3Position& pos,
                               const O3Label&    name,
                               const O3Float&    value) :
           _type_(O3Parameter::PRMType::FLOAT),
           _pos_(pos),  _name_(name),  _value_(value) {
        GUM_CONSTRUCTOR(O3Parameter);
      }

      O3Parameter::O3Parameter(const O3Parameter& src) :
           _type_(src. _type_),  _pos_(src. _pos_),  _name_(src. _name_),
           _value_(src. _value_) {
        GUM_CONS_CPY(O3Parameter);
      }

      O3Parameter::O3Parameter(O3Parameter&& src) :
           _type_(std::move(src. _type_)),  _pos_(std::move(src. _pos_)),
      ...
      O3Aggregate::O3Aggregate() :
           _variableType_(),  _aggregateType_(),  _name_(),  _parents_(),
           _parameters_() {
      ...
      O3Aggregate::O3Aggregate(const O3Aggregate& src) :
           ...
           _name_(src. _name_),  _parents_(src. _parents_),
      ...
      O3Aggregate::O3Aggregate(O3Aggregate&& src) :
           ...
           _name_(std::move(src. _name_)),  _parents_(std::move(src. _parents_)),
      ...
      O3Assignment::O3Assignment(const O3Assignment& src) :
           ...
           _rightInstance_(src. _rightInstance_),  _rightIndex_(src. _rightIndex_) {
      ...
      O3System::O3System(const O3System& src) :
           _name_(src. _name_),  _instances_(src. _instances_),
           _assigments_(src. _assigments_),  _increments_(src. _increments_) {
