...
/// Retrieve prm object.
      const PRM<double>* O3prmrInterpreter::prm() const {
        return m_reader->prm();
      }
...
/// a std::vector of struct SingleResult, with pair label/value.
      ...
      bool O3prmrInterpreter::checkObserve ( ObserveCommand<double>* command ) {
        try {
          ...
          const Instance<double>& instance = sys.get ( findInstanceName ( left_val, sys ) );
          const Attribute<double>& attr = instance.get ( findAttributeName ( left_val, instance ) );
      ...
      bool O3prmrInterpreter::checkUnobserve ( UnobserveCommand<double>* command ) {
        try {
          ...
          const Attribute<double>& attr = instance.get ( findAttributeName ( name, instance ) );
...
// Return false if any error.
      ...
      std::string O3prmrInterpreter::findInstanceName ( std::string& s, const System<double>& sys ) {
      ...
      std::string O3prmrInterpreter::findAttributeName ( const std::string& s, const Instance<double>& instance ) {
