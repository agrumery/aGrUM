...
namespace gum {
  ...
  void StaticTriangulation::__computeRecursiveThinning() {
    ...
    for ( unsigned int i = __added_fill_ins.size() - 1;
      ...
      if ( __added_fill_ins[i].size() ) {
        ...
        for ( unsigned int j = 0; require_mint; ++j ) {
          ...
          for ( auto iter_edge = T.beginSafe(); iter_edge != T.endSafe();
                ++iter_edge ) {
            ...
            for ( auto iter_adj = nei.beginSafe();
          ...
          // triangulated graph
          for ( auto iter_delete = T_prime.beginSafe();
    ...
    for ( unsigned int i = __elim_order.size() - 1;
      ...
      for ( auto iter = nei.beginSafe(); iter != nei.endSafe(); ++iter ) {
  ...
  void StaticTriangulation::__computeEliminationTree() {
    ...
    for ( unsigned int i = 0; i < __elim_order.size(); ++i ) {
      ...
      for ( auto it = list_of_nodes.beginSafe();
  ...
  ( const NodeId node, const NodeId from,
    ...
    for ( auto iter_sep = nei.beginSafe();
  ...
  StaticTriangulation::triangulatedGraph() {
    ...
    if ( ! __has_triangulated_graph ) {
      ...
      for ( auto iter_node = __junction_tree->nodes().beginSafe();
        ...
        for ( auto iter_clique = clique.beginSafe();
  ...
  void StaticTriangulation::__triangulate() {
    ...
    for ( unsigned int nb_elim = 0; tmp_graph.size() != 0; ++nb_elim ) {
      ...
      if ( !__minimality_required ) {
        ...
        for ( auto iter_edges = nei.beginSafe();
        ...
        for ( auto iter_edges1 = nei.beginSafe();
