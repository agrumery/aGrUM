...
namespace gum {
  ...
  template < GUM_Numeric GUM_SCALAR >
  std::pair< std::string, int > IKTBNLearner< GUM_SCALAR >::_determineNode_(const std::string& name) const {
    ...
    if (_atemporalVarNames_().contains(name))
  ...
  void IKTBNLearner< GUM_SCALAR >::_checkArcTemporallyFeasible_(std::string_view tail,
    ...
    if (tailSlice != KTBN< GUM_SCALAR >::ATEMPORAL
  ...
  std::unordered_set< std::string >
      IKTBNLearner< GUM_SCALAR >::_scanConstantColumns_(std::string_view                  dirPath,
    ...
    const std::unordered_set< std::string > missing(missingSymbols.begin(),
                                                     missingSymbols.end());
    ...
    std::vector< bool >        haveFirstSeen;     // per column: whether firstSeen[c] is set THIS trajectory
    ...
    for (Size i = 0; i < nbSamples; ++i) {
      ...
      if (i == 0) {
        ...
        for (std::size_t c = 0; same && c < header.size(); ++c) same = (raw[c] == header[c]);
        if (!same)
          GUM_ERROR(InvalidArgument,
    ...
    for (const std::size_t c: candidates) atemporalVars.insert(header[c]);
  ...
  void IKTBNLearner< GUM_SCALAR >::_checkBaseIsTemporal_(std::string_view base,
    ...
    if (!_isKnownBase_(b))
      GUM_ERROR(InvalidArgument,
                "unknown base variable '" << base << "': it is not one of this learner's variables")
