...
long lrs_getnextbasis(
    lrs_dic** dict_p, lrs_dat* Q, long prune ); /* gets next lrs tree basis,
...
long lrs_leaf( lrs_dic* P, lrs_dat* Q ); /* true if current dictionary is leaf
...
/***************************************************/
long checkredund( lrs_dic* P, lrs_dat* Q ); /* solve primal lp to check redund
                                               of obj fun. returns TRUE if
                                               redundant, else FALSE          */
long checkcobasic(
    lrs_dic* P, lrs_dat* Q, long index ); /* TRUE if index is cobasic and
...
long checkindex(
    lrs_dic* P,
    lrs_dat* Q,
    long index ); /* index=0 non-red.,1 red., 2 input linearity NOTE: row is
...
void reorder( long a[],
              long range ); /* reorder array in increasing order with one
                               misplaced element   */
void reorder1( long a[],
               long b[],
               long newone,
               long
...
void lrs_set_obj( lrs_dic* P,
                  ...
                  long num[],
                  long den[],
                  long max ); /* set up objective function with coeffs
