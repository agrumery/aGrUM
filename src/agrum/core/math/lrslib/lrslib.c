...
lrs_mp_vector lrs_alloc_mp_vector_wrapper ( long n ) {
  return lrs_alloc_mp_vector ( n );
}
/* allocate lrs_mp_matrix for m+1 x n+1 lrs_mp   */
lrs_mp_matrix lrs_alloc_mp_matrix_wrapper ( long m, long n ) {
  return lrs_alloc_mp_matrix ( m, n );
}

void lrs_clear_mp_vector_wrapper ( lrs_mp_vector a, long n ) {
  lrs_clear_mp_vector ( a, n );
}
...
lrs_printoutput ( lrs_dat* Q, lrs_mp_vector output ) {
  ...
  if ( Q->hull || IS_ZERO ( output[0] ) ) { /*non vertex */
    for ( i = 0; i < Q->n; i++ )
      pmp ( "", output[i] );
...
/* if input is V-rep. vertices are scaled by 1/output[1] */
{
  ...
  if ( IS_ZERO ( output[1] ) ) { /*non-vertex */
    for ( i = 1; i <= rowd; i++ )
      pmp ( "", output[i] );
...

{
  ...
  for ( i = 1; i <= m; i++ ) { /* read in input matrix row by row                 */
    ...
    if ( hull ) {
      ...
      if ( !IS_ONE ( A[i][1] ) || !IS_ONE ( A[0][1] ) ) /* all rows must have a one in column one */
        Q->polytope = FALSE;
    }

    if ( !IS_ZERO ( A[i][hull] ) ) /* for H-rep, are zero in column 0     */
  ...
  while ( fscanf ( lrs_ifp, "%s", name ) != EOF ) {
    ...
    if ( strcmp ( name, "cache" ) == 0 ) {
      if ( fscanf ( lrs_ifp, "%lu", &dict_limit ) == EOF )
        dict_limit = 1;
...
/* col is output column being printed */
{
  ...
  for ( i = lastdv + 1; i <= m; i++ )
    if ( IS_ZERO ( A[Row[i]][0] ) )
      if ( ( col == ZERO ) || IS_ZERO ( A[Row[i]] [col] ) ) {
        nincidence++;
...

{
  ...
  for ( j = 0; j < m; j++ ) {
    ...
    if ( i <= m ) {
      ...
      while ( C[k] <= d && IS_ZERO ( A[Row[i]][Col[k]] ) )
        k++;
...
/* inequality contains the relabelled inequalities after initialization     */
{
  ...
  /*
    ...
    k = d - 1;
    while ((k >= 0) &&
           (IS_ZERO (A[Row[i]][Col[k]]) || Cobasic[C[k]]))
      k--;
    if (k >= 0)
      {
        pivot (P, Q, i, k);
  ...
  */
  ...
  while ( i > d ) {
    while ( Cobasic[B[i]] ) {
      ...
      while ( ( k >= 0 ) && ( IS_ZERO ( A[Row[i]][Col[k]] ) || Cobasic[C[k]] ) ) {
        k--;
      }
      ...
      /* perform ratio test on rhs or column of basis inverse */
    {
      ...
      for ( j = start; j < start + degencount; j++ ) {
        ...
        if ( firstime )
          ...
          else if ( IS_ZERO ( Nmin ) && IS_ZERO ( A[i][ratiocol] ) )
            comp = 0;
...
/* FALSE if a_r,g=0, a_rs !=0, r > s          */
{
  ...
  for ( i = Q->lastdv + 1; i <= m; i++ ) {
    ...
    if ( IS_ZERO ( A[r][col] ) ) /* necessary for lexmin to fail */
      for ( j = 0; j < d; j++ ) {
        s = Col[j];

        if ( B[i] > C[j] ) { /* possible pivot to reduce basis */
          if ( IS_ZERO ( A[r][0] ) ) { /* no need for ratio test, any pivot feasible */
            if ( !IS_ZERO ( A[r][s] ) )
              return ( FALSE );
...

{
  ...
  while ( ( i <= m ) &&
          ( IS_ZERO ( A[Row[i]][s] ) || !IS_ZERO ( A[Row[i]][0] ) ) )
    i++;
...
/*      = 0 (EQ)   - linearity     */
{
  ...
  if ( hull ) {
    ...
    if ( !IS_ONE ( A[i][1] ) || !IS_ONE ( oD[1] ) )   /* all rows must have a one in column one */
      Q->polytope = FALSE;
  }
