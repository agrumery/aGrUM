 ...
 * they are "safe", i.e., deleting elements which are pointed to by iterators
 *does
 * never produce any segmentation fault nor unexpected results when the
 *iterators
 ...
 * safety: dereferencing an unsafe iterator pointing to an erased element will
 *most
...
namespace gum {
   ...
   * when elements of the list are deleted while some safe iterators are
   *pointing
  ...
  class List {
     ...
     * copied.
     * Of course if these values are pointers, the new values point toward the
     * same
    ...
    /// return a safe const iterator pointing just before the beginning of the
    /// List
    /** Safe const iterators are const iterators whose state is updated by the
     * list
     * when the element they point to is erased. As such, in this case, they
    ...
    /// returns an unsafe iterator pointing just before the beginning of the
    /// List
    /** Unsafe iterators are a little bit faster than safe iterators and
     * they consume less memory. However, if the element they point to is
     * erased, their dereference or their increment/decrement will produce a
    ...
    /// returns an unsafe const iterator pointing to the last element of the
    /// List
    /** Unsafe iterators are a little bit faster than safe iterators and
     * they consume less memory. However, if the element they point to is
     * erased, their dereference or their increment/decrement will produce a
     ...
     * this
     * is a copy of this value that is inserted. The method runs in constant
     * time.
     ...
     * the size of the list and in the number of iterators referenced in the
     * list.
     * Comparisons between Val instances are performed through == operators.
     ...
     * if
     * any, that is returned. If the element cannot be found, 0 is returned.
     * This
   ...
   * typical use is to iterate over a const List). Whenever you are not sure
   *that
  ...
  class ListConstIterator {
     ...
     * After
     * being detached, the iterator does not point to any element, i.e., trying
     * to
   ...
   *a
   * segmentation fault but rather throw an exception. Similarly, incrementing
   *or
   * decrementing an iterator pointing to a deleted element is guaranteed to
   *make
  ...
  class ListConstIteratorSafe {
     ...
     * iterator is still alive. After being detached, the iterator does not
     * point
     * to any element, i.e., trying to access its content will raise an
     * exception.
     */
    void clear();
    ...
    /// makes the iterator point to the preceding element in the List
    /** for (iter=rbeginSafe(); iter!=rendSafe(); --iter) loops are guaranteed
     * to
     * parse the whole List as long as no element is added to or deleted from
     * the List while being in the loop. Deleting elements during the loop is
     * guaranteed to never produce a segmentation fault. Runs in constant time.
