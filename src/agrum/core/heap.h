 ...
 ***************************************************************************/
/** @file
 * @brief heaps and priority queues
 *
 * @author Christophe GONZALES and Pierre-Henri WUILLEMIN
 *
 * This file provides class Heap that implements a classic heap, and
 * PriorityQueue that is essentially a heap in which elements are sorted
 * according to a dynamically modifiable priority. Elements are sorted according
 * to a weak order which is < by default, i.e., the top element of the heap is the
 * smallest element and the more to the bottom the higher the element. In addition,
 * PriorityQueue has a special feature that enables it to prevent multiple
 * identical elements to be stored into it.
 * @par Usage example:
 * @code
 ...
 * heap1.eraseByVal (8);
 *
 * // erase element at position 3 (see function erase for the meaning of position 3)
 * heap1.erase (3);
 *
...
namespace gum {
  ...
  #define GUM_HEAP_DEFAULT_SIZE 10
  ...
  // ==============================================================================
  /// a << operator for the basic heaps
  // ==============================================================================
   ...
   * we do have access in O(1) to the "smallest" element contained by the structure.
   * Insertions and deletions of elements are processed in O(ln n), where n is the
   * number of elements in the heap. In addition to the classical pop, top and push
   * functions, Heap provide direct accessors to the elements in the heap
   * (see operator[]). However, these can only be accessed in read-only mode.
   * @par Usage example:
   * @code
   * // create a heap of integers, the top element is the smallest element
   * Heap<int> heap1;
   ...
   *
   * // create a priority queue of floats, the priorities of which are pairs of int
  ...
  public:
    ...
    /** @param compare a function taking two elements in argument, say e1 and e2, and
     * returning a boolean indicating wether e1 < e2, i.e., whether e1 should be
     * nearer than e2 to the top of the heap.
     * @param size the size of the internal data structures containing the elements
     * (could be for instance vectors or hashtables) */
    // ============================================================================
    explicit Heap( Cmp compare = Cmp(), unsigned int size = GUM_HEAP_DEFAULT_SIZE );
    ...
    // ============================================================================
    unsigned int insert( const Val& val );
     ...
     * computed as follows: suppose that the heap is a complete binary tree, that is,
     * a binary tree where all levels are completely filled except, maybe, the last
     * one and, in this case, the elements of this level are all to the left of the
     * tree. Then parsing the tree from top to bottom and, for each level, from left
     * to right, and assigning index 0 to the root of the tree and, incrementing the
     * index by 1 each time we jump to another node, we get a unique index for each
     * element. This is precisely what the index passed in argument of the function
     * represents. */
    // ============================================================================
    ...
    // ============================================================================
    /// copy operator
    /** When a problem occurs during the copy (for instance when not enough memory is
     * available), the operator guarantees that the heap stays in a coherent state.
     * Actually, the heap becomes empty. An exception is then thrown. */
    // ============================================================================
     ...
     * at position "index". */
    // ============================================================================
    const Val& operator[]( unsigned int index ) const;
    ...
    // ============================================================================
    /// return the size of the internal structure storing the heap
    // ============================================================================
    unsigned int capacity() const ;

    // ============================================================================
    /// changes the size of the the internal structure storing the heap
    /** If the new size does not enable the heap to contain the elements it currently
     * contains, then the resizing does not occur. */
    // ============================================================================
  ...
  private:
    ...
    /// for friendly displaying the content of a heap
    friend std::ostream& operator<< <> ( std::ostream&, const Heap<Val,Cmp>& );
  };
  ...
  /* ============================================================================ */
  /** @class PriorityQueue
   * @brief Heap in which each element has a mutable priority
   * @ingroup basicstruct_group
   *
   * A priority queue differs from a heap by the fact that a priority (a score) is
   * assigned to each element in the structure. The elements are sorted according to
   * a weak order on the scores. The priority of any element can be changed
   * at any moment by the user. The priority queue then restores a heap property
   * accordingly.
   * @par Usage example:
   * @code
   * // create a priority queue of strings, the priorities of which are integers
  ...
  public:
     ...
     * and returning a boolean indicating wether e1 < e2, i.e., whether e1 should be
     * nearer than e2 to the top of the heap.
     ...
     * (could be for instance vectors or hashtables) */
    // ============================================================================
    explicit PriorityQueue( Cmp compare = Cmp(), bool uniqueness = false,
                            unsigned int size = GUM_HEAP_DEFAULT_SIZE );
    ...
    /** @throw NotFound exception is thrown if the queue is empty */
    // ============================================================================
    const Priority& getTopPriority() const;
    ...
    // ============================================================================
    unsigned int insert( const Priority& priority, const Val& val );
     ...
     * exception. */
    // ============================================================================
    void setPriority( unsigned int index, const Priority& new_priority );
    ...
    /// modifies the priority of each instance of a given element
    // ============================================================================
    const Priority& getPriorityByVal( const Val& elt ) const ;
    ...
    // ============================================================================
    const HashTable< Id,std::vector<int> >& getAllValues() const ;
    ...
    /** When a problem occurs during the copy (for instance when not enough memory is
     * available), the operator guarantees that the heap stays in a coherent state.
     * Actually, the priority queue becomes empty. An exception is then thrown. */
    // ============================================================================
    PriorityQueue<Val,Priority,Cmp>&
    operator= ( const PriorityQueue<Val,Priority,Cmp>& );

    // ============================================================================
    /// returns the element at index elt from the priority queue
    /**
     * @throw NotFound
     */
    // ============================================================================
    ...
    // ============================================================================
    bool getUniquenessPolicy() const ;
  ...
  private:
    ...
    std::vector<std::pair<Priority, Val*>*> heap;

    /// a hashtable for quickly finding the elements by their value
    HashTable< Val,std::vector<int> > indices;
    ...
    ( std::ostream& stream, const PriorityQueue<Val,Priority,Cmp>& );
  };
