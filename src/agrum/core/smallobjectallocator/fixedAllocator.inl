...
namespace gum {
  ...
  INLINE void* FixedAllocator::__Chunk::__allocate(const std::size_t& blockSize){
    ...
    // Remember that the first byte of each block gives us the index of next available slot.
    // The new first availble block will be at the index indicating in this block.
  ...
  INLINE void FixedAllocator::__Chunk::__deallocate(void* pDeallocatedBlock, const std::size_t& blockSize){
    ...
    // First byte of toRelease has now to give the index of current first available block
    *toRelease = __firstAvailableBlock;

    // So that first available block points to it
    __firstAvailableBlock = static_cast<unsigned char>( (toRelease - __pData) / blockSize );
        ...
        while(true){
          ...
          if((__deallocChunk + offset) < __chunks.end()){
            if((__deallocChunk + offset)->__pData <= pDeallocatedBlock &&
                pDeallocatedBlock < ((__deallocChunk + offset)->__pData + (__numBlocks * __blockSize))){
              // If pointed chunk contains this pointer, deallocation find the place
