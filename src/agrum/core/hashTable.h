 ...
 * to point to a delete element, the user can use "unsafe" iterators
 * HashTableIterator and HashTableConstIterator (a.k.a. HashTable<>::iterator and
 * HashTable<>::const_iterator). Those iterators are slightly faster than their
...
namespace gum {
  ...
  template <typename Key, typename Val, typename Alloc> std::ostream& operator<<
  ( std::ostream&, const HashTableList<Key,Val,Alloc>& );

  /// a \c << operator for HashTable
  template <typename Key, typename Val, typename Alloc>
  std::ostream& operator<<
  ( std::ostream&, const HashTable<Key,Val,Alloc>& );
  ...
  /* =========================================================================== */
  /* ===       singly chained lists for storing elements in hash tables      === */
  /* =========================================================================== */
   ...
   * the list is called a bucket. Lists are singly linked because the iterators
   * are only one-directional (forward). */
  template <typename Key, typename Val, typename Alloc> class HashTableList {
  public:
    ...
    /// the buckets of the singly list chain
    struct Bucket {
      Bucket* next { nullptr };
      value_type pair;

      Bucket() {}
      Bucket( const Bucket& from ) : pair ( from.pair ) {}
      Bucket( const Key& k, const Val& v ) : pair ( k,v ) {}
      Bucket( Key&& k, Val&& v ) : pair ( std::move ( k ), std::move ( v ) ) {}
      Bucket( const value_type& p ) : pair ( p ) {}
      Bucket( value_type&& p ) : pair ( std::move ( p ) ) {}
      ~Bucket () {}
      value_type&  elt () { return pair; }
      key_type&    key () { return const_cast<key_type&> ( pair.first ); }
      mapped_type& val () { return pair.second; }
    };
   ...
   /// move operator
    ...
    Bucket* bucket( const Key& key ) const noexcept;
   ...
   * point to a delete element, the user can use "unsafe" iterators
   * HashTableIterator and HashTableConstIterator (a.k.a. HashTable<>::iterator and
   * HashTable<>::const_iterator). Those iterators are slightly faster than their
   * safe counterparts. However, as in the standard library, accessing through them
   * a deleted element usually results in a mess (most probably a segfault).
  ...
  public:
    ...
    using Bucket = typename HashTableList<Key,Val,Alloc>::Bucket;
    ...
    explicit
    HashTable ( Size size_param = HashTableConst::default_size,
                bool resize_pol = HashTableConst::default_resize_policy,
                bool key_uniqueness_pol =
                HashTableConst::default_uniqueness_policy );
    ...
    /// move operator
    HashTable<Key,Val,Alloc>&
    operator= ( HashTable<Key,Val,Alloc>&& from );
    ...
    value_type& getWithDefault( const Key& key, const Val& default_value );
    
    /// returns a reference on the element the key of which is passed in argument
    /** In case of multiple identical keys in the hash table, the first value
     * encountered is returned. The method runs in constant time.
     * In case of not found key, (key,default_value) is inserted in *this. */
    value_type& getWithDefault( Key&& key, Val&& default_value );
    ...
    value_type& set( const Key& key, const Val& default_value );
     ...
     * original hashtable. */
    template <typename Mount, typename OtherAlloc = Alloc>
  ...
  public:
    ...
    HashTableConstIterator<Key,Val> operator+ ( unsigned int ) const;
  ...
  protected:
     ...
     * slot of the hash table's 'node' vector. */
    typename HashTable<Key,Val>::Bucket* __next_bucket { nullptr };
  ...
  public:
    ...
    /// returns a new iterator
    HashTableIterator<Key,Val> operator+ ( unsigned int ) const;
