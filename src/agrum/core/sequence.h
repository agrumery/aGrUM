 ...
 *ordered
 * set of elements. The main difference between these two data structures lies
 *in
...
namespace gum {
   ...
   * where the key lies in O(1). As a result, it is not possible to insert a
   *given
   ...
   * enables to optimize the code of the sequences. Determining whether gen
   *should
   * be set to true or false is not left to the developper but is determined by
   *the
  ...
  class SequenceImplementation {
    ...
    /** If the element cannot be found, the function does nothing. In
     * particular,
     * it throws no exception. Complexity \f$o(n)\f$ (need to change the
     * position
   ...
   * is similar to a const safe iterator because the elements that can be
   *accessed
  ...
  template <typename Key> class SequenceIteratorSafe {
    ...
    /// returns the value pointed to by the iterator (works only for
    /// non-scalars)
    const Key* operator->() const;
