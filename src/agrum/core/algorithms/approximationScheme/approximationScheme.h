    ...
    public:
      ...
      */
      ...
      bool continueApproximationScheme( double error ) {
        ...
        _current_epsilon = error; // eps rate isEnabled needs it so affectation was moved from eps isEnabled below
        ...
        if( _last_epsilon >= 0. ) {
          if( _current_epsilon > .0 )  // ! _current_epsilon can be 0. AND epsilon isEnabled can be disabled !
            _current_rate = fabs( ( _current_epsilon - _last_epsilon ) / _current_epsilon );
          // limit with current eps ---> 0 is | 1 - ( last_eps / 0 ) | ---> infinity
          // the else means a return false if we isEnabled the rate below, as we would have returned false if epsilon isEnabled was enabled
          else
        ...
        if( stateApproximationScheme() == ApproximationSchemeSTATE::Continue ) {
          if( onProgress.hasListener() ) GUM_EMIT3( onProgress, ( _current_step * 100 ) / _max_iter, _current_epsilon, timer_step );
