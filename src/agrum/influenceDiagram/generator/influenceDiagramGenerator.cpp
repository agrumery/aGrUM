...
namespace gum {
  ...
  InfluenceDiagramGenerator::InfluenceDiagramGenerator() {
  ...
  InfluenceDiagramGenerator::InfluenceDiagramGenerator ( CPTGenerator* cptGenerator ) {
    GUM_CONSTRUCTOR ( InfluenceDiagramGenerator );
  ...
  InfluenceDiagramGenerator::InfluenceDiagramGenerator ( UTGenerator* utGenerator ) {
    GUM_CONSTRUCTOR ( InfluenceDiagramGenerator );
    __cptGenerator = new SimpleCPTGenerator();
  ...
  // @param cptGenerator The policy used to generate CPT.
  // @param utGenerator The policy used to generate UT.
  InfluenceDiagramGenerator::InfluenceDiagramGenerator ( CPTGenerator* cptGenerator, UTGenerator* utGenerator ) {
    GUM_CONSTRUCTOR ( InfluenceDiagramGenerator );
    __cptGenerator = cptGenerator;
    __utGenerator = utGenerator;
  }
  ...
  InfluenceDiagram<float>*
  InfluenceDiagramGenerator::generateIDF ( Size nbrNodes, float arcDensity, float chanceNodeDensity, float utilityNodeDensity, int max_modality ) {
    InfluenceDiagram<float>* influenceDiagram = new InfluenceDiagram<float>();
    // First we add nodes
    ...
    __checkTemporalOrderf ( influenceDiagram );
  ...
  // @param max_modality Each DRV has from 2 to max_modality modalities
  // @return A IDs randomly generated.
  InfluenceDiagram<double>*
  InfluenceDiagramGenerator::generateIDD ( Size nbrNodes, float arcDensity, float chanceNodeDensity, float utilityNodeDensity, int max_modality ) {
    InfluenceDiagram<double>* influenceDiagram = new InfluenceDiagram<double>();
    // First we add nodes
    HashTable<Size, NodeId> map;
    std::stringstream strBuff;
    int nb_mod;
    ...
    __checkTemporalOrderd ( influenceDiagram );
