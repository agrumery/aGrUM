...
#define NANODBC_CALL_RC(FUNC, RC, ...)                                           \
  do {                                                                           \
    std::cerr << __FILE__ ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE( \
                   FUNC) "(" #__VA_ARGS__ ")"                                    \
              << std::endl;                                                      \
    RC = FUNC(__VA_ARGS__);                                                      \
  } while (false) /**/
...
namespace {
  ...
  inline std::string recent_error(SQLHANDLE    handle,
    ...
    do {
      ...
      if (success(rc) && total_bytes > 0) sql_message.resize(total_bytes + 1);
...
namespace nanodbc {
  ...
  class statement::statement_impl {
    ...
    result procedure_columns(const string_type& catalog,
      ...
      RETCODE rc;
      NANODBC_CALL_RC(NANODBC_UNICODE(SQLProcedureColumns),
                      rc,
                      stmt_,
    ...
    unsigned long parameter_size(short param) const {
      ...
      NANODBC_CALL_RC(
        SQLDescribeParam, rc, stmt_, param + 1, &data_type, &parameter_size, 0, 0);
      if (!success(rc)) NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
      NANODBC_ASSERT(
        parameter_size <=
    ...
    void bind_null(short param, std::size_t elements) {
      ...
      SQLSMALLINT scale;
      prepare_bind(
        param, elements, PARAM_IN, data_type, param_type, parameter_size, scale);
...
namespace nanodbc {
  ...
  catalog::tables catalog::find_tables(const string_type& table,
    ...
    RETCODE   rc;
    NANODBC_CALL_RC(NANODBC_UNICODE(SQLTables),
                    rc,
  ...
  catalog::columns catalog::find_columns(const string_type& column,
    ...
    statement stmt(conn_);
    RETCODE   rc;
    NANODBC_CALL_RC(NANODBC_UNICODE(SQLColumns),
                    rc,
                    stmt.native_statement_handle(),
                    (NANODBC_SQLCHAR*)(catalog.empty() ? NULL : catalog.c_str()),
                    (catalog.empty() ? 0 : SQL_NTS),
  ...
  catalog::primary_keys catalog::find_primary_keys(const string_type& table,
    ...
    statement stmt(conn_);
    RETCODE   rc;
    NANODBC_CALL_RC(NANODBC_UNICODE(SQLPrimaryKeys),
                    rc,
                    stmt.native_statement_handle(),
                    (NANODBC_SQLCHAR*)(catalog.empty() ? NULL : catalog.c_str()),
                    (catalog.empty() ? 0 : SQL_NTS),
