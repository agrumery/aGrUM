...
/*******************************************************/
int64_t lrs_main( int argc, char* argv[] )
...
/*********************************************************************************/
  ...
  plrs_read_dat( Q, input_file ); /* read first part of problem data to get
                                     dimensions and problem type: H- or V- input
                                     representation     */
  ...
  if ( !lrs_read_dat(
           Q,
           argc,
           argv ) ) /* read first part of problem data to get dimensions   */
  ...
  if ( !lrs_getfirstbasis( &P, Q, &Lin, FALSE ) ) return 1;
  ...
  do {
    ...
    for ( col = 0; col <= P->d; col++ ) /* print output vertex/ray if any */
      if ( lrs_getsolution( P, Q, output, col ) ) lrs_printoutput( Q, output );
...
void plrs_read_dic( lrs_dic* P, lrs_dat* Q, std::ifstream& input_file ) {
  ...
  for ( i = 1; i <= m; i++ ) /* read in input matrix row by row                 */
  {
    ...
    if ( hull ) {
      ...
      if ( !one( A[i][1] ) ||
           !one( A[0][1] ) ) /* all rows must have a one in column one */
        Q->polytope = FALSE;
    }
  ...
  /* 2010.4.26 patch */
  if ( Q->nonnegative ) /* set up Gcd and Lcm for nonexistent nongative
                           inequalities */
    for ( i = m + 1; i <= m + d; i++ ) {
      itomp( ONE, Lcm[i] );
      itomp( ONE, Gcd[i] );
  ...
  if ( Q->homogeneous && Q->verbose ) {
    out_stream << "*Input is homogeneous, column 1 not treated as redundant"
               << endl;
  }
  ...
  while ( input_file.good() ) {
    ...
    if ( line.find( "*" ) == 0 ) {
      ...
      if ( Q->nonnegative ) {
        out_stream
            << "*Starting cobasis incompatible with nonegative option:skipped"
            << endl;
      } else {
      ...
      if ( hull && !Q->voronoi )
        out_stream
            << "*Geometric option for H-representation or voronoi only, skipped"
            << endl;
      else
        Q->geometric = TRUE;
      ...
      string str;
      ss >> str;
      if ( !( ss >> Q->frequency ) ) Q->frequency = 0;
      Q->printcobasis = TRUE;
      ...
      if ( !hull )
        ...
        out_stream << "*Truncate option for H-representation only, skipped"
                   << endl;
      ...
      out_stream << "*Nonnegative option must come before begin line, skipped"
                 << endl;
    ...
    } else if ( line.find( "voronoi" ) != string::npos ||
                line.find( "Voronoi" ) != string::npos ) {
      if ( !hull )
        out_stream << "*voronoi requires V-representation - option skipped"
                   << endl;
      else {
        Q->voronoi = TRUE;
        Q->polytope = FALSE;
...
void plrs_readfacets( lrs_dat* Q, int64_t facet[], string facets ) {
  ...
  for ( j = Q->nlinearity; j < d;
        j++ ) /* note we place these after the linearity indices */
  {
    if ( !( ss >> facet[j] ) ) {
      return;
    }
...
/* showing function calls intended for public use      */
/*******************************************************/
int64_t redund_main( int argc, char* argv[] )

{
  ...
  ***************************************************/
  ...
  for ( i = 1; i <= m; i++ ) {
    ...
    if ( debug ) lrs_printrow( "*", Q, Ain[i], d );
  }
  ...
  if ( !lrs_getfirstbasis( &P, Q, &Lin, TRUE ) ) return 1;
  ...
  // if ( debug ) {
  //   fprintf( lrs_ofp,
  //            "\ncheckindex m=%lld, n=%lld, nlinearity=%lld lastdv=%lld",
  //            (long long int)m,
  //            (long long int)d,
  //            (long long int)nlinearity,
  //            (long long int)lastdv );
  // }
  ...
  for ( index = lastdv + 1; index <= m + d; index++ ) {
    ineq = Q->inequality[index - lastdv]; /* the input inequality number corr. to
                                             this index */

    redineq[ineq] = checkindex( P, Q, index );
    // if ( debug ) {
    //   fprintf( lrs_ofp,
    //            "\ncheck index=%lld, inequality=%lld, redineq=%lld",
    //            (long long int)index,
    //            (long long int)ineq,
    //            (long long int)redineq[ineq] );
    // }
    if ( redineq[ineq] == ONE ) {
      // fprintf (lrs_ofp, "\n*row %lld was redundant and removed", (long long
      // int)ineq);
      // fflush  (lrs_ofp);
    }
  ...
  for ( i = 1; i <= m; i++ )
    if ( redineq[i] == 0 ) nredund++;
  // fprintf( lrs_ofp, "\nbegin" );
  // fprintf( lrs_ofp,
  //          "\n%lld %lld rational",
  //          (long long int)nredund,
  //          (long long int)( Q->n ) );
  ...
  for ( i = 1; i <= m; i++ )
    if ( redineq[i] == 0 ) lrs_printrow( "", Q, Ain[i], Q->inputd );
  // fprintf( lrs_ofp, "\nend" );
  // fprintf( lrs_ofp,
  //          "\n*Input had %lld rows and %lld columns",
  //          (long long int)m,
  //          (long long int)( Q->n ) );
...
void lrs_lpoutput( lrs_dic* P, lrs_dat* Q, lrs_mp_vector output ) {
  ...
  for ( i = 0; i < P->d; i++ ) {
    // fprintf( lrs_ofp,
    //          "y_%lld=",
    //          (long long int)( Q->inequality[P->C[i] - Q->lastdv] ) );
...
/* end of lrs_lpoutput */
/***********************/
void lrs_printrow( char name[], lrs_dat* Q, lrs_mp_vector output, int64_t rowd )
...
/* col=0 for vertex 1....d for ray/facet             */
{
  ...
  if ( Q->lponly ) {
    if ( !positive( A[0][col] ) ) return FALSE;
  }
...
int64_t lrs_init( char* name ) /* returns TRUE if successful, else FALSE */
{
...
#ifdef MP
// fprintf (lrs_ofp, " max digits=%lld/%lld", (long long int)DIG2DEC
// (lrs_record_digits), (long long int)DIG2DEC (lrs_digits));
#endif
  ...
  // fprintf (lrs_ofp, "\n");
  fclose( lrs_ifp );
  // if ( lrs_ofp != stdout ) fclose( lrs_ofp );
}
...
lrs_dat* lrs_alloc_dat( const char* name ) {
  ...
  if ( Q == NULL ) return Q; /* failure to allocate */
...
/*  lrs_read_dat               */
/*******************************/
int64_t lrs_read_dat( lrs_dat* Q, int argc, char* argv[] ) {
  char    name[100];
          ...
          0 ) /*skip until "begin" found processing options */
  {
    if ( strncmp( name, "*", 1 ) == 0 ) /* skip any line beginning with * */
    {
      ...
      if ( !lrs_set_digits( dec_digits ) ) return ( FALSE );
    } else if ( strcmp( name, "linearity" ) == 0 ) {
      if ( !readlinearity( Q ) ) return FALSE;
...
/****************************/
int64_t lrs_read_dic( lrs_dic* P, lrs_dat* Q )
/* read constraint matrix and set up problem and dictionary  */

{
  ...
  while ( fscanf( lrs_ifp, "%s", name ) != EOF ) {
    ...
    if ( strcmp( name, "debug" ) == 0 ) {
      ...
      if ( fscanf( lrs_ifp,
                   "%lld %lld",
                   (long long int*)( &Q->strace ),
                   (long long int*)( &Q->etrace ) ) == EOF )
        Q->strace = 0;
      // fprintf( lrs_ofp,
      //          "\n*%s from B#%lld to B#%lld",
      //          name,
      //          (long long int)( Q->strace ),
      //          (long long int)( Q->etrace ) );
      Q->verbose = TRUE;
      if ( Q->strace <= 1 ) Q->debug = TRUE;
    }
    ...
    if ( strcmp( name, "restart" ) == 0 ) {
      ...
      if ( Q->voronoi ) {
        if ( fscanf( lrs_ifp,
                     ...
                     (long long int*)( &Q->count[2] ),
                     (long long int*)( &P->depth ) ) == EOF ) {
          return FALSE;
        }
        // fprintf( lrs_ofp,
        //          "\n*%s V#%lld R#%lld B#%lld h=%lld data points",
        //          name,
        //          (long long int)( Q->count[1] ),
        //          (long long int)( Q->count[0] ),
        //          (long long int)( Q->count[2] ),
        //          (long long int)( P->depth ) );
      } else if ( hull ) {
        if ( fscanf( lrs_ifp,
                     "%lld %lld %lld",
                     (long long int*)( &Q->count[0] ),
                     ...
                     (long long int*)( &P->depth ) ) == EOF ) {
          // fprintf( lrs_ofp,
          //          "\n*%s F#%lld B#%lld h=%lld vertices/rays",
          //          name,
          //          (long long int)Q->count[0],
          //          (long long int)Q->count[2],
          //          (long long int)P->depth );
        ...
        // fprintf( lrs_ofp,
        //          "\n*%s V#%lld R#%lld B#%lld h=%lld facets",
        //          name,
        //          (long long int)( Q->count[1] ),
        //          (long long int)( Q->count[0] ),
        //          (long long int)( Q->count[2] ),
      ...
      if ( !readfacets( Q, Q->facet ) ) return FALSE;
    } /* end of restart */
    ...
    if ( strcmp( name, "maximize" ) == 0 || strcmp( name, "minimize" ) == 0 ) {
      if ( Q->hull ) {
        ...
        for ( j = 0; j <= d; j++ ) {
          ...
          prat( "", A[0][j], oD[j] );
          if ( !Q->maximize ) changesign( A[0][j] );
        }
    ...
    if ( strcmp( name, "geometric" ) == 0 ) {
      ...
      if ( hull & !Q->voronoi ) {
        // fprintf( lrs_ofp,
        //          " - option for H-representation or voronoi only, skipped" );
      } else {
        Q->geometric = TRUE;
      }
    }
    ...
    if ( strcmp( name, "#incidence" ) ==
         0 ) /* number of incident inequalities only */
    {
      Q->printcobasis = TRUE;
    }
    ...
    if ( strcmp( name, "printcobasis" ) == 0 ) {
      if ( fscanf( lrs_ifp, "%lld", (long long int*)( &Q->frequency ) ) == EOF ) {
        /*2010.7.7  set default to zero = print only when outputting
         * vertex/ray/facet */
        Q->frequency = 0;
      }
      // fprintf( lrs_ofp, "\n*%s", name );
    ...
    if ( strcmp( name, "verbose" ) == 0 ) Q->verbose = TRUE;
...
int64_t lrs_getfirstbasis( lrs_dic**      D_p,
                           lrs_dat*       Q,
                           lrs_mp_matrix* Lin,
                           int64_t        no_output )
...
/* no_output is TRUE supresses output headers   */
{
  ...
  if ( Q->nonnegative ) {
    ...
    if ( !getabasis( D, Q, inequality ) ) return FALSE;
    /* bug fix 2009.12.2 */
  ...
  if ( !no_output || Q->debug ) {
    ...
    if ( nredundcol > k ) {
      ...
      //     lrs_ofp,
      //     "\nlinearity %lld ",
      //     (long long int)( nredundcol - k ) ); /*adjust nredundcol for homog. */
#else
      stringstream ss;
      char*        type = "header";
...
/* end if linearity */
  ...
  if ( Q->lponly || Q->nash ) {
    if ( Q->verbose ) {
      // fprintf( lrs_ofp,
      //          "\nNumber of pivots for starting dictionary: %lld",
      //          (long long int)( Q->count[3] ) );
      if ( Q->lponly ) printA( D, Q );
    }
  }
  ...
  if ( !primalfeasible( D, Q ) ) {
    ...
    if ( Q->nash && Q->verbose ) {
      // fprintf( lrs_ofp,
      //          "\nNumber of pivots for feasible solution: %lld",
      //          (long long int)( Q->count[3] ) );
  ...
  if ( Q->maximize || Q->minimize ) {
    ...
    if ( Q->lponly ) {
      ...
      if ( Q->verbose ) {
        // fprintf( lrs_ofp,
        //          "\nNumber of pivots for optimum solution: %lld",
        //          (long long int)( Q->count[3] ) );
        printA( D, Q );
      }
#endif
...
/* we use the fact that cobases are sorted by index value */
  ...
  if ( Q->debug ) {
    // fprintf( lrs_ofp,
    //          "\n*Inequality numbers for indices %lld .. %lld : ",
    //          (long long int)( lastdv + 1 ),
    //          (long long int)( m + d ) );
    // for ( i = 1; i <= m - nlinearity; i++ )
    //   fprintf( lrs_ofp, " %lld ", (long long int)( inequality[i] ) );
    printA( D, Q );
...
#endif
    if ( !restartpivots( D, Q ) ) return FALSE;
...
int64_t lrs_getnextbasis( lrs_dic** D_p, lrs_dat* Q, int64_t backtrack )
...

{
  ...
  while ( ( j < d ) || ( D->B[m] != m ) ) /*main while loop for getnextbasis */
  {
    ...
    if ( backtrack ) /* go back to prev. dictionary, restore i,j */
    {
      ...
      if ( Q->debug ) {
        // fprintf( lrs_ofp,
        //          "\n Backtrack Pivot: indices i=%lld j=%lld depth=%lld",
        //          (long long int)i,
        //          (long long int)j,
        //          (long long int)( D->depth ) );
        printA( D, Q );
    ...
    if ( D->depth < Q->mindepth ) break;

    /* try to go down tree */

    /* 2011.7.14 patch */
    while ( ( j < d ) && ( !reverse( D, Q, &i, j ) ||
                           ( Q->truncate && Q->minratio[D->m] == 1 ) ) )
      j++;
    if ( j == d )
      backtrack = TRUE;
    else
    /*reverse pivot found */
    {
      ...
      D->depth++;
      if ( D->depth > Q->deepest ) Q->deepest++;
      ...
      save_basis( *D_p, Q );
      if ( Q->strace == Q->count[2] ) Q->debug = TRUE;
      if ( Q->etrace == Q->count[2] ) Q->debug = FALSE;
...
/* return FALSE if no output generated  */
{
  ...
  hull = Q->hull;
  lexflag = P->lexflag;
  if ( lexflag || Q->allbases ) ++( Q->count[1] );
#ifdef PLRS
  ...
  if ( hull ) return FALSE; /* skip printing the origin */
  ...
  if ( lexflag && one( output[0] ) ) ++Q->count[4]; /* integer vertex */
  ...
  */
  ...
  if ( Q->printslack ) {
    ...
    for ( i = lastdv + 1; i <= P->m; i++ ) {
      if ( !iszero( A[Row[i]][0] ) ) {
        // fprintf(
        //     lrs_ofp, " %lld ", (long long int)( Q->inequality[B[i] - lastdv] ) );
        }
    }
  }
...
int64_t lrs_getray(
    lrs_dic* P, lrs_dat* Q, int64_t col, int64_t redcol, lrs_mp_vector output )
...
void getnextoutput( lrs_dic* P, lrs_dat* Q, int64_t i, int64_t col, lrs_mp out )
/* get A[B[i][col] and copy to out */
{
  ...
  if ( i == d && Q->voronoi ) return; /* skip last column if voronoi set */
...
void lrs_printcobasis( lrs_dic* P, lrs_dat* Q, int64_t col )
/* col is output column being printed */
{
  ...
  if ( hull )
    ss << "F#" << count[0] << " B#" << count[2] << " h=" << P->depth
       << " vertices/rays ";
  else if ( Q->voronoi )
    ss << "V#" << count[1] << " R#" << count[0] << " B#" << count[2]
       << " h=" << P->depth << " data points ";
  else
    ss << "V#" << count[1] << " R#" << count[0] << " B#" << count[2]
       << " h=" << P->depth << " facets ";
  ...
  // if ( hull ) {
  //   fprintf( lrs_ofp,
  //            "\nF#%lld B#%lld h=%lld vertices/rays ",
  //            (long long int)( count[0] ),
  //            (long long int)( count[2] ),
  //            (long long int)( P->depth ) );
  // } else if ( Q->voronoi ) {
  //   fprintf( lrs_ofp,
  //            "\nV#%lld R#%lld B#%lld h=%lld data points ",
  ...
  // } else {
  //   fprintf( lrs_ofp,
  //            "\nV#%lld R#%lld B#%lld h=%lld facets ",
  //            (long long int)( count[1] ),
  //            (long long int)( count[0] ),
  //            (long long int)( count[2] ),
  //            (long long int)( P->depth ) );
    ...
    if ( iszero( A[Row[i]][0] ) )
      if ( ( col == ZERO ) || iszero( A[Row[i]][col] ) ) {
        ...
        if ( Q->incidence ) {
          ...
          // fprintf(
          //     lrs_ofp, " %lld", (long long int)( inequality[B[i] - lastdv] ) );
        }
      }
...
// for some reason next line fails for mp library !   2014.12.3 so no volume is
// reported!
    ...
    /* inaccurate for plrs as restart command does not contain number of integer
     * vertices : an overcount is produced */
  ...
  //   if ( Q->maximize )
  //     fprintf( lrs_ofp, "\n*Recommendation: Add dualperturb option before "
  //                       "maximize in input file\n" );
  //   else
  //     fprintf( lrs_ofp, "\n*Recommendation: Add dualperturb option before "
  //                       "minimize in input file\n" );
  // }
  ...
  // if ( Q->maxdepth < MAXD )
  //   fprintf( lrs_ofp,
  //            "\n*Tree truncated at depth %lld",
  //            (long long int)( Q->maxdepth ) );
  // if ( Q->maxoutput > 0L )
  //   fprintf( lrs_ofp,
  //            "\n*Maximum number of output lines = %lld",
  //            (long long int)( Q->maxoutput ) );
  ...
  {
    // fprintf( lrs_ofp,
    //          "\n*Totals: facets=%lld bases=%lld",
    //          (long long int)( count[0] ),
    //          (long long int)( count[2] ) );

    if ( nredundcol >
         homogeneous ) /* don't count column 1 as redundant if homogeneous */
    {
      // fprintf( lrs_ofp,
      //          " linearities=%lld",
      //          (long long int)( nredundcol - homogeneous ) );
      // fprintf( lrs_ofp,
      //          " facets+linearities=%lld",
      //          (long long int)( nredundcol - homogeneous + count[0] ) );
    }
    if ( lrs_ofp != stdout ) {
      // printf( "\n*Totals: facets=%lld bases=%lld",
      //         (long long int)( count[0] ),
    ...
    if ( ( cest[2] > 0 ) || ( cest[0] > 0 ) ) {
      ...
      // fprintf( lrs_ofp,
      //          "\n*Total number of tree nodes evaluated: %lld",
      //          (long long int)( Q->totalnodes ) );
#ifdef TIMES
      // fprintf( lrs_ofp,
    ...
    /*
          if ( Q-> restart || Q->allbases || (count[0] > 1 && !Q->homogeneous &&
       !Q->polytope))
  ...
  {
    // fprintf( lrs_ofp,
    //          "\n*Totals: vertices=%lld rays=%lld bases=%lld",
    //          (long long int)( count[1] ),
    //          (long long int)( count[0] ),
    //          (long long int)( count[2] ) );
    ...
    //   if ( nredundcol > 0 ) fprintf( lrs_ofp, "+linearities" );
    //   fprintf(
    //       lrs_ofp, "=%lld", (long long int)( nredundcol + count[0] + count[1] ) );
    // }
    ...
    //     if ( nredundcol > 0 ) printf( "+linearities" );
  ...
  // fprintf(
  //     lrs_ofp,
  //     "\n*Dictionary Cache: max size= %lld misses= %lld/%lld   Tree Depth= %lld",
  //     (long long int)( dict_count ),
  //     (long long int)( cache_misses ),
  //     (long long int)( cache_tries ),
  //     (long long int)( Q->deepest ) );
  // if ( lrs_ofp != stdout )
  //   printf(
  //       "\n*Dictionary Cache: max size= %lld misses= %lld/%lld   Tree Depth= %lld",
  //       (long long int)( dict_count ),
  ...
  // fprintf( lrs_ofp,
  //          "\n*Input size m=%lld rows n=%lld columns",
  //          (long long int)( P->m ),
  //          (long long int)( Q->n ) );
  // if ( hull )
  //   fprintf( lrs_ofp,
  //            " working dimension=%lld",
  //            (long long int)( d - 1 + homogeneous ) );
  // else
...
/*  Estimation function */
/************************/
int64_t lrs_estimate( lrs_dic* P, lrs_dat* Q )
...
/*    [4] integer vertices                       */
{
  ...
  for ( runcount = 1; runcount <= Q->runs;
    ...
    while ( nchild != 0 ) /* while not finished yet */
    {
      ...
      if ( Q->debug ) {
        ...
        //   fprintf( lrs_ofp,
        //            "%lld,%lld ",
        //            (long long int)( isave[k] ),
        //            (long long int)( jsave[k] ) );
      }
      ...
      if ( nchild > 0 ) /*reverse pivot found choose random child */
      {
        ...
        // if ( Q->debug )
        //   fprintf( lrs_ofp,
        //            "  selected pivot k=%lld seed=%lld ",
        //            (long long int)( k ),
        //            (long long int)( Q->seed ) );
        estdepth++;
        ...
        if ( Q->getvolume ) {
          rescaledet(
              P, Q, Nvol, Dvol ); /* scales determinant in case input rational */
    ...
    while ( estdepth > 0 ) {
      ...
      if ( Q->debug ) {
        // fprintf( lrs_ofp,
        //          "\n Backtrack Pivot: indices i,j %lld %lld ",
        //          (long long int)i,
        //          (long long int)j );
        printA( P, Q );
      }
  ...
  // 2015.2.9   Do not update totals if we do iterative estimating and subtree is
  // too big
...
int64_t reverse( lrs_dic* P, lrs_dat* Q, int64_t* r, int64_t s )
...
/* TRUE if B[*r] C[s] is a reverse lexicographic pivot */
{
  ...
  if ( Q->debug ) {
    // fprintf( lrs_ofp,
    //          "\n+reverse: col index %lld C %lld Col %lld ",
    //          (long long int)( s ),
    //          (long long int)( enter ),
    //          (long long int)( col ) );
    // fflush( lrs_ofp );
  ...
  if ( !negative( A[0][col] ) ) {
    // if ( Q->debug ) fprintf( lrs_ofp, " Pos/Zero Cost Coeff" );
  ...
  *r = lrs_ratio( P, Q, col );
  ...
  {
    // if ( Q->debug ) fprintf( lrs_ofp, " Pivot col non-negative:  ray found" );
    Q->minratio[P->m] = 0; /* 2011.7.14 */
    return ( FALSE );
  }
    ...
    if ( i != s ) {
      ...
      if ( positive( A[0][j] ) ||
        ...
        if ( ( !negative( A[0][j] ) && !positive( A[row][j] ) ) ||
          ...
          if ( Q->debug ) {
            // fprintf( lrs_ofp,
            //          "\nPositive cost found: index %lld C %lld Col %lld",
            //          (long long int)( i ),
            //          (long long int)( C[i] ),
            //          (long long int)( j ) );
            // fflush( lrs_ofp );
...
int64_t selectpivot( lrs_dic* P, lrs_dat* Q, int64_t* r, int64_t* s )
...
/* pivot variables are B[*r] C[*s] in locations Row[*r] Col[*s] */
{
  ...
  *s = d;
  ...
  if ( j < d ) /* pivot column found! */
  {
    ...
    *r = lrs_ratio( P, Q, col );
    if ( *r != 0 ) return ( TRUE ); /* unbounded */
  }
  return ( FALSE );
} /* end of selectpivot        */
/******************************************************* */

void pivot( lrs_dic* P, lrs_dat* Q, int64_t bas, int64_t cob )
...
/* Col[cob]   respectively                       */
{
  ...
  if ( sign( Ars ) == POS ) {
    for ( j = 0; j <= d; j++ ) /* no need to change sign if Ars neg */
      /*   A[r][j]=-A[r][j];              */
      if ( !iszero( A[r][j] ) ) changesign( A[r][j] );
  } /* watch out for above "if" when removing this "}" ! */
  else
    for ( i = 0; i <= m_A; i++ )
      if ( !iszero( A[i][s] ) ) changesign( A[i][s] );
  ...
  if ( !Q->maximize ) changesign( P->objnum );
  if ( iszero( P->objnum ) )
...
int64_t primalfeasible( lrs_dic* P, lrs_dat* Q )
/* Do dual pivots to get primal feasibility */
  ...
  while ( primalinfeasible ) {
    ...
    if ( i <= m ) {
      ...
      while ( j < d && !positive( A[Row[i]][Col[j]] ) )
        j++;
      if ( j >= d ) return ( FALSE ); /* no positive entry */
      pivot( P, Q, i, j );
      update( P, Q, &i, &j );
    } else
...
int64_t lrs_solvelp( lrs_dic* P, lrs_dat* Q, int64_t maximize )
...
int64_t getabasis( lrs_dic* P, lrs_dat* Q, int64_t order[] )
...

{
...
#ifndef LRS_QUIET
          // fprintf( lrs_ofp,
          //          "\n*Input linearity in row %lld is redundant--converted to "
          //          "inequality",
          //          (long long int)( order[j] ) );
#endif
          linearity[j] = 0l;
        } else {
          if ( Q->debug ) printA( P, Q );
#ifndef LRS_QUIET
          // fprintf( lrs_ofp,
          //          "\n*Input linearity in row %lld is inconsistent with earlier "
          //          "linearities",
          //          (long long int)( order[j] ) );
          // fprintf( lrs_ofp, "\n*No feasible solution" );
#endif
  ...
  while ( k < nlinearity ) {
    while ( k < nlinearity && linearity[k] == 0 )
      k++;
    if ( k < nlinearity ) linearity[i++] = linearity[k++];
  }
  ...
  if ( Q->debug ) {
    ...
    // fprintf( lrs_ofp,
    //          "lastdv=%lld nredundcol=%lld",
    //          (long long int)( Q->lastdv ),
    //          (long long int)( Q->nredundcol ) );
    // fprintf( lrs_ofp, "\nredundant cobases:" );
    // for ( i = 0; i < nredundcol; i++ )
    //   fprintf( lrs_ofp, " %lld", (long long int)( redundcol[i] ) );
    // printA( P, Q );
  ...
  for ( i = 0; i < nlinearity; i++ ) { /* find cobasic index */
    ...
    if ( !removecobasicindex( P, Q, k ) ) return FALSE;
    d = P->d;
  }
  if ( Q->debug && nlinearity > 0 ) printA( P, Q );
...
int64_t removecobasicindex( lrs_dic* P, lrs_dat* Q, int64_t k )
...
/* used after detecting column dependency    */
{
  ...
  // if ( Q->debug )
  //   fprintf( lrs_ofp,
  //            "\nremoving cobasic index k=%lld C[k]=%lld",
  //            (long long int)( k ),
  //            (long long int)( C[k] ) );
...
/* a new lrs_dic record is created with reduced size, and items copied over */
{
  ...
  if ( Q->debug ) {
    // fprintf( lrs_ofp,
    //          "\nDictionary resized from d=%lld to d=%lld",
    //          (long long int)( Q->inputd ),
    //          (long long int)( P->d ) );
    printA( P1, Q );
  }
...
int64_t restartpivots( lrs_dic* P, lrs_dat* Q )
...
/* inequality contains the relabelled inequalities after initialization     */
{
  ...
  for ( i = 0; i < d; i++ ) /* find index corresponding to facet[i] */
  {
    ...
    //   fprintf( lrs_ofp,
    //            " %lld %lld;",
    //            (long long int)( facet[i + nlinearity] ),
    //            (long long int)( j + lastdv ) );
  }
  ...
  */
  ...
  if ( lexmin( P, Q, ZERO ) ) --Q->count[1]; /* decrement vertex count if lexmin */
...
/* recoded Dec 1997                     */
{
  ...
  if ( degencount == 0 ) return ( degencount ); /* non-negative pivot column */
  ...
  while ( degencount > 1 ) /*keep going until unique min ratio found */
  {
    ...
    /* perform ratio test on rhs or column of basis inverse */
    {
      ...
      if ( basicindex != d ) ratiocol = Col[cindex++];
      for ( j = start; j < start + degencount; j++ ) {
        ...
        if ( firstime )
          ...
          if ( ratiocol == ZERO ) comp = -comp; /* all signs reversed for rhs */
        }
    ...
    if ( Q->debug ) {
      // fprintf( lrs_ofp,
      //          " ratiocol=%lld degencount=%lld ",
      //          (long long int)( ratiocol ),
      //          (long long int)( degencount ) );
      // fprintf( lrs_ofp, "  Min ratios: " );
...
int64_t lexmin( lrs_dic* P, lrs_dat* Q, int64_t col )
...
int64_t ismin( lrs_dic* P, lrs_dat* Q, int64_t r, int64_t s )
...
void update( lrs_dic* P, lrs_dat* Q, int64_t* i, int64_t* j )
...
int64_t lrs_degenerate( lrs_dic* P, lrs_dat* Q )
...
/* not thoroughly tested   2000/02/15                  */
{
  ...
  for ( i = d + 1; i <= m; i++ )
    if ( iszero( A[Row[i]][0] ) ) return TRUE;
...
int64_t checkredund( lrs_dic* P, lrs_dat* Q )
...
int64_t checkcobasic( lrs_dic* P, lrs_dat* Q, int64_t index )
...

{
  ...
  if ( j == d ) return FALSE; /* not cobasic index */
  ...
  while ( ( i <= m ) && ( iszero( A[Row[i]][s] ) || !iszero( A[Row[i]][0] ) ) )
    i++;

  if ( i > m ) {
    // if ( debug ) fprintf( lrs_ofp, " is non-redundant" );
    return TRUE;
  }
  // if ( debug )
...
int64_t checkindex( lrs_dic* P, lrs_dat* Q, int64_t index )
...
/*NOTE: row is returned all zero if redundant!! */
{
  ...
  if ( checkcobasic( P, Q, index ) ) return ZERO;
...
void lprat( const char* name, int64_t Nt, int64_t Dt )
/*print the long precision rational Nt/Dt without reducing  */
{
  // if ( Nt > 0 ) fprintf( lrs_ofp, " " );
  // fprintf( lrs_ofp, "%s%lld", name, (long long int)( Nt ) );
  // if ( Dt != 1 ) fprintf( lrs_ofp, "/%lld", (long long int)( Dt ) );
  // fprintf( lrs_ofp, " " );
} /* lprat */

int64_t lreadrat( int64_t* Num, int64_t* Den )
...
/* returns true if denominator is not one        */
{
  ...
  if ( fscanf( lrs_ifp, "%s", in ) == EOF ) return ( FALSE );
...
void lrs_getinput(
    lrs_dic* P, lrs_dat* Q, int64_t* num, int64_t* den, int64_t m, int64_t d )
...
int64_t readlinearity( lrs_dat* Q ) /* read in and check linearity list */
{
  int64_t i, j;
  int64_t nlinearity;
...
int64_t readfacets( lrs_dat* Q, int64_t facet[] )
/* read and check facet list for obvious errors during start/restart */
/* this must be done after linearity option is processed!! */
{
  int64_t i, j;
...
void pimat( lrs_dic* P, int64_t r, int64_t s, lrs_mp Nt, char name[] )
...
*/
  /* I wish I understood the above remark. For the time being, do it the easy way
   * for Nash */
  if ( global->nash ) {
    for ( r = 0; r <= m_A; r++ )
      for ( s = 0; s <= d; s++ )
...
static void lrs_dump_state() {
  ...
  // fprintf( stderr,
  //          "lrs_lib: Current digits at %lld out of %lld\n",
...
void print_basis( FILE* fp, lrs_dat* global ) {
  ...
  if ( global->saved_flag ) {
    ...
    //     fprintf( fp,
    //              "%lld ",
    //              (long long int)( global->inequality[global->saved_C[i] -
    //                                                  global->lastdv] ) );
...
static void ptimes() {
  ...
  //     rusage.ru_oublock );
  // if ( lrs_ofp != stdout )
  //   printf( "\n*%0.3fu %0.3fs %lldKb %lld flts %lld swaps %lld blks-in %lld "
  //           "blks-out \n",
  //           double_time( rusage.ru_utime ),
  //           double_time( rusage.ru_stime ),
  //           rusage.ru_maxrss,
...
/*      = 0 (EQ)   - linearity     */
{
  ...
  for ( j = hull; j <= d; j++ ) /* hull data copied to cols 1..d */
  {
    ...
    if ( !one( oD[j] ) ) lcm( Lcm[i], oD[j] ); /* update lcm of denominators */
    copy( Temp, A[i][j] );
    gcd( Gcd[i], Temp ); /* update gcd of numerators   */
  }
...
void lrs_set_obj(
    lrs_dic* P, lrs_dat* Q, int64_t num[], int64_t den[], int64_t max ) {
  int64_t i;
...
void lrs_set_obj_mp(
    lrs_dic* P, lrs_dat* Q, lrs_mp_vector num, lrs_mp_vector den, int64_t max ) {
  int64_t i;
...
int64_t lrs_solve_lp( lrs_dic* P, lrs_dat* Q )
/* user callable function to solve lp only */
{
  ...
  if ( !lrs_getfirstbasis( &P, Q, &Lin, FALSE ) ) return FALSE;
...
int64_t dan_selectpivot( lrs_dic* P, lrs_dat* Q, int64_t* r, int64_t* s )
...
int64_t phaseone( lrs_dic* P, lrs_dat* Q )
...
int64_t lrs_set_digits( int64_t dec_digits ) {
  /* convert user specified decimal digits to mp digits */

  // fprintf( lrs_ofp, "\n*digits %lld", (long long int)( dec_digits ) );
  if ( dec_digits > 0 ) lrs_digits = DEC2DIG( dec_digits );
...
int64_t lrs_checkbound( lrs_dic* P, lrs_dat* Q ) {
...
int64_t lrs_leaf( lrs_dic* P, lrs_dat* Q ) {
