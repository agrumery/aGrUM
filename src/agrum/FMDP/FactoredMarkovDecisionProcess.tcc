...
namespace gum {
	...
	/* **************************************************************************************************** **/
		...
		FactoredMarkovDecisionProcess<T_DATA>::primeCopy( MultiDimDecisionDiagramBase<T_DATA>* source ){
				...
				typename Property< HashTable< Idx, NodeId >* >::onNodes arcMap;
				...
				for( NodeGraphPartIterator nodeIter = source->nodesMap().beginNodes(); nodeIter != source->nodesMap().endNodes(); ++nodeIter ){
					...
					if( !source->isTerminalNode( *nodeIter ) ){
						...
						arcMap.insert( *nodeIter, new HashTable< Idx, NodeId >( *(source->nodeSons( *nodeIter ) ) ) );
		...
		FactoredMarkovDecisionProcess<T_DATA>::show( ) const{
			for( HashTableConstIterator< Idx, HashTable< const DiscreteVariable*, const MultiDimImplementation< T_DATA >* >* > actionIter = __actionTransitionTable.begin(); actionIter != __actionTransitionTable.end(); ++actionIter ) {
				std::cout << std::endl << std::endl << std::endl << " ACTION : " << actionName( actionIter.key() ) << std::endl;
				for( HashTableConstIterator< const DiscreteVariable*, const MultiDimImplementation< T_DATA >* > tableIter = (*actionIter)->begin(); tableIter != (*actionIter)->end(); ++tableIter )
					std::cout << std::endl << " VARIABLE : " << tableIter.key()->name() << std::endl << reinterpret_cast<const MultiDimDecisionDiagramBase<T_DATA>*>( *tableIter )->toDot();
			}
			
			std::cout << std::endl << "REWARD : " << std::endl << reinterpret_cast<const MultiDimDecisionDiagramBase<T_DATA>*>( __defaultRewardTable )->toDot();
		}
	/* **************************************************************************************************** **/
	/* **																									**/
	/* **									Protected methods.												**/
