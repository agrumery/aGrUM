...
namespace gum {
  ...
  //
  // =================================================================================================
  void StatesCounter::__incState(const Instantiation& state, NodeId parentId, Idx parentModa, Idx nbVisits ){

    Idx varIter = 0;
    if( parentId )
      varIter = state.variablesSequence().pos( __counter->node(parentId)->nodeVar() ) + 1;
    ...
    for( ; varIter < state.variablesSequence().size(); ++varIter ){
      ...
      for ( Idx moda = 0; moda < curVar->domainSize(); ++moda ){
        if( moda == state.valFromPtr(curVar) )
          ...
          __counter->manager()->setSon(varId, moda, __counter->terminalNodeId( nbVisits ) );
      }
      parentId = varId;
    }
    __counter->manager()->setSon(parentId, parentModa, __counter->manager()->addTerminalNode( nbVisits + 1 ) );
  }
