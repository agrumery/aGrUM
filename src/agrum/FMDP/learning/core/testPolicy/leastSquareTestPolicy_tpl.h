...
namespace gum {
  ...
  LeastSquareTestPolicy< GUM_SCALAR >::~LeastSquareTestPolicy() {
    for (auto obsIter = this-> _obsTable_.cbeginSafe();
          _obsTable_.cendSafe() != obsIter;
         ++obsIter)
      delete obsIter.val();
  ...
  void LeastSquareTestPolicy< GUM_SCALAR >::addObservation(Idx        attr,
    ...
    if (! _obsTable_.exists(attr))
       _obsTable_.insert(attr, new LinkedList< double >());
  ...
  void LeastSquareTestPolicy< GUM_SCALAR >::computeScore() {
    ...
    for (auto attrIter =  _sumAttrTable_.cbeginSafe();
         attrIter !=  _sumAttrTable_.cendSafe();
         ++attrIter) {
      ...
      sumErrorAttr += ((double) _nbObsTable_[key] / (double)this->nbObservation())
                    * errorAttr;
    }
