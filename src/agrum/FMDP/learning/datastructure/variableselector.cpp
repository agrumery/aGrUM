...
namespace gum {
  ...
  // ==========================================================================
    ...
    // Default constructor
    // ###################################################################
    VariableSelector::VariableSelector(Set<const DiscreteVariable*>& startingSet) : __remainingVars(startingSet){
      GUM_CONSTRUCTOR(VariableSelector);
      __remainingScores.insert(0.0,0.0);
      __remainingVarsByScore.insert(0.0, new Set<const DiscreteVariable*>(__remainingVars));

      for( auto varIter = __remainingVars.cbeginSafe(); varIter != __remainingVars.cendSafe(); ++varIter ){
    ...
    //
    // ###################################################################
    void VariableSelector::updateScore( const DiscreteVariable* var, double score, double secondaryscore){
      __removeVar(var);
    ...
    //
    // ###################################################################
    void VariableSelector::downdateScore( const DiscreteVariable* var, double score, double secondaryscore){
      __removeVar(var);
      __remainingVarsScore[var] -= score;
      __addVar(var);
      __remainingVarsOtherScore[var] -= secondaryscore;
    ...
    void VariableSelector::__addVar(const DiscreteVariable *var ){
      ...
      if(!__remainingVarsByScore.exists(varScore)){
        __remainingVarsByScore.insert(varScore, new Set<const DiscreteVariable*>());
