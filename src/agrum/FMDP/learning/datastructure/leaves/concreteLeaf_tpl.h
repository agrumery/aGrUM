...
namespace gum {
  ...
  ConcreteLeaf< AttributeSelection, isScalar >::ConcreteLeaf(
      NodeId                                        leafId,
  ...
  template < TESTNAME AttributeSelection, bool isScalar >
  void* ConcreteLeaf< AttributeSelection, isScalar >::operator new(size_t s) {
    return SmallObjectAllocator::instance().allocate(s);
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  void ConcreteLeaf< AttributeSelection, isScalar >::operator delete(void* p) {
    SmallObjectAllocator::instance().deallocate(p, sizeof(ConcreteLeaf));
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  double ConcreteLeaf< AttributeSelection, isScalar >::effectif(Idx moda) const {
  ...
  template < TESTNAME AttributeSelection, bool isScalar >
  double ConcreteLeaf< AttributeSelection, isScalar >::total() const {
    return double(_n1_->nbObservation());
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  Idx ConcreteLeaf< AttributeSelection, isScalar >::nbModa() const {
    return _nbModa_(Int2Type< isScalar >());
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  Idx ConcreteLeaf< AttributeSelection, isScalar >::_nbModa_(Int2Type< true >) const {
    return _valueDomain_->size();
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  Idx ConcreteLeaf< AttributeSelection, isScalar >::_nbModa_(Int2Type< false >) const {
    return _n1_->valueDomain();
  }

  template < TESTNAME AttributeSelection, bool isScalar >
  std::string ConcreteLeaf< AttributeSelection, isScalar >::toString() {
