...
// ===========================================================================
  ...
  SPUDDPlanning<GUM_SCALAR>::__displayOptimalPolicy( MultiDimDecisionDiagramBase< Idx >* op ) {
        ...
        if( op->isTerminalNode( node ) ) {
          ...
          if( op->nodeSons( node ) != nullptr ) {
            ...
            for( const auto son :  *op->nodeSons( node ) ) {
              if( son != 0 )
                arcstream << tab <<  node << " -> " << son << " [label=\"" << op->nodeVariable( node )->label( lab ) << "\",color=\"#0000ff\"]" << ";" << std::endl;
    ...
    output << terminalStream.str() << std::endl
           ...
           << defaultarcstream.str() << std::endl << std::endl << "}" << std::endl;
  }
...
// std::cout << " ------------- Maximisation " << std::endl;
      ...
      for( const auto & Vaction : VactionCollector ) {
        ...
        std::pair<Idx, Idx> resEvalRetro = __evalNbRetrogradeEvaluation( Vnew, Vaction );
        ...
        if( sensDirect ) {
          time.reset();
          MultiDimDecisionDiagramBase< GUM_SCALAR >* Vind = maximize2MultiDimDecisionDiagrams( Vaction, Vnew );
          ...
          time.reset();
          MultiDimDecisionDiagramBase< GUM_SCALAR >* Vind = maximize2MultiDimDecisionDiagrams( Vnew, Vaction );
