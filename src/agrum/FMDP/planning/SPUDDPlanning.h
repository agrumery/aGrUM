...
namespace gum {
  ...
  class SPUDDPlanning {
    ...
    */
    ...
    /**
     * Evals final value function by multiplying by discount reward and adding
     * reward
     */
    MultiDimDecisionDiagramBase<GUM_SCALAR>*
     ...
     * Evals the policy corresponding to the given value function
     */
    std::pair<Idx, Idx> __evalNbRetrogradeEvaluation(
        const MultiDimDecisionDiagramBase<GUM_SCALAR>* t1,
     ...
     * nodes
     * to converge toward a cannonical policy, a factory and the current node
     * are
     * needed to build
     * resulting diagram. Also we need an exploration table to avoid exploration
     * of
    ...
    /// The Factored Markov Decision Process on which we do our planning (NB :
    /// this
