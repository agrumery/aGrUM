...
namespace gum {
    ...
    template <typename GUM_SCALAR,
              ...
              template <typename> class PROJECTOPERATOR,
              template <typename> class TerminalNodePolicy>
    void Regress<GUM_SCALAR, COMBINEOPERATOR, PROJECTOPERATOR, TerminalNodePolicy>::__establishVarOrder(){
      ...
      while ( fite != __DG1->variablesSequence().endSafe() && site != __DG2->variablesSequence().endSafe() ) {
        ...
        if( ! __DG2->variablesSequence().exists( *fite ) && !__primedVars->exists(*fite) ){
          __rd->add( **fite );
          ++fite;
          continue;
        }
        ...
        // if not we add it to final order
        if( ! __DG1->variablesSequence().exists( *site ) && !__primedVars->exists(*site) ){
          __rd->add( **site );
          ++site;
          continue;
        }
...
//        }
    ...
    template <typename GUM_SCALAR,
              ...
              template <typename> class TerminalNodePolicy>
    NodeId
    Regress<GUM_SCALAR, COMBINEOPERATOR, PROJECTOPERATOR, TerminalNodePolicy>::__compute(O4DGContext & currentSituation,
                                                                                         Idx lastInstVarPos) {
