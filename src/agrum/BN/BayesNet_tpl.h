...
namespace gum {
  ...
  NodeId build_node(gum::BayesNet< GUM_SCALAR >& bn,
    ...
    NodeId res;
    try { res = bn.idFromName(v->name()); } catch (gum::NotFound const&) { res = bn.add(*v); }
    return res;
  }
  ...
  BayesNet< GUM_SCALAR >::~BayesNet() {
    ...
    for (const auto& p: _probaMap_) { delete p.second; }
  }
  ...
  INLINE NodeId BayesNet< GUM_SCALAR >::add(const DiscreteVariable& var) {
    ...
    try { return add(var, ptr); } catch (Exception const&) {
      delete ptr;
      throw;
    }
  }
  ...
  INLINE NodeId BayesNet< GUM_SCALAR >::add(const DiscreteVariable& var, NodeId id) {
    ...
    try { return add(var, ptr, id); } catch (Exception const&) {
      delete ptr;
      throw;
    }
  }
  ...
  template <typename GUM_SCALAR>
  INLINE const VariableNodeMap& BayesNet< GUM_SCALAR >::variableNodeMap() const { return _varMap_; }
  ...
  void BayesNet< GUM_SCALAR >::clear() {
    if (!this->empty()) {
      auto l = this->nodes();
      for (const auto no: l) { this->erase(no); }
    }
  }
  ...
  INLINE void BayesNet< GUM_SCALAR >::addArc(const std::string& tail, const std::string& head) {
    try { addArc(this->idFromName(tail), this->idFromName(head)); } catch (DuplicateElement const&
    ) { GUM_ERROR(DuplicateElement, "The arc " << tail << "->" << head << " already exists.") }
  }
  ...
  void BayesNet< GUM_SCALAR >::_clearTensors_() {
    ...
    for (const auto& elt: _probaMap_) { delete elt.second; }
  ...
  BayesNet< GUM_SCALAR >
    ...
    for (gum::Idx i = 0; i < observations.nbrDim(); i++) { all.add(interventions.variable(i)); }
    ...
    for (gum::Idx i = 0; i < observations.nbrDim(); i++) {
      ...
      for (gum::NodeId child: this->children(nod)) {
        bn.eraseArc(bn.idFromName(nam),
                    bn.idFromName(this->variable(child).name()));
      }
    }
    for (gum::Idx i = 0; i < interventions.nbrDim(); i++) {
      ...
      for (gum::NodeId par: this->parents(nod)) {
        ...
        if (bn.existsArc(v1, v2))
          bn.eraseArc(v1, v2);
      }
      cpt_changed.insert(bn.idFromName(nam));
      bn.cpt(bn.idFromName(nam)).fillWith(
          Tensor< GUM_SCALAR >::deterministicTensor(
              bn.variable(bn.idFromName(nam)),
              interventions.val(i)));
    }
    ...
    for (gum::Idx i = 0; i < all.nbrDim(); i++) {
      ...
      for (gum::NodeId child: this->children(nod)) {
        if (!cpt_changed.contains(child)) {
          ...
          bn.cpt(bn.idFromName(this->variable(child).name())).fillWith(
              this->cpt(child).extract(all));
        }
      }
    }
