...
//                              IBaseBayesNet
  ...
  IBaseBayesNet<GUM_SCALAR>::IBaseBayesNet() :
    __propertiesMap( 0 ) {
    GUM_CONSTRUCTOR( IBaseBayesNet );
  }

  template <typename GUM_SCALAR> INLINE
  IBaseBayesNet<GUM_SCALAR>::IBaseBayesNet( const IBaseBayesNet<GUM_SCALAR>& from ) :
    __propertiesMap( 0 ) {
    GUM_CONS_CPY( IBaseBayesNet );
  ...
  template<typename GUM_SCALAR> INLINE
  const std::string&
  IBaseBayesNet<GUM_SCALAR>::property( const std::string& name ) const {
    try {
  ...
  template<typename GUM_SCALAR> INLINE
  void
  IBaseBayesNet<GUM_SCALAR>::setProperty( const std::string& name, const std::string& value ) {
    try {
  ...
  IBaseBayesNet<GUM_SCALAR>::beginNodes() const {
    return dag().beginNodes();
  }

  template<typename GUM_SCALAR> INLINE
  const DAG::NodeIterator
  IBaseBayesNet<GUM_SCALAR>::endNodes() const {
    return dag().endNodes();
  }

  template<typename GUM_SCALAR> INLINE
  const DAG::ArcIterator
  IBaseBayesNet<GUM_SCALAR>::beginArcs() const {
    return dag().beginArcs();
  }

  template<typename GUM_SCALAR> INLINE
  const DAG::ArcIterator&
  IBaseBayesNet<GUM_SCALAR>::endArcs() const {
    return dag().endArcs();
  }

  template <typename GUM_SCALAR>
  void
  IBaseBayesNet<GUM_SCALAR>::_moralGraph( UndiGraph& graph ) const {
  ...
  template <typename GUM_SCALAR>
  void
  IBaseBayesNet<GUM_SCALAR>::_topologicalOrder( Sequence<NodeId>& topo ) const {
    DAG dag = this->dag();
  ...
  IBaseBayesNet<GUM_SCALAR>::__properties() const {
    if ( __propertiesMap == 0 ) {
  ...
  template<typename GUM_SCALAR> INLINE
  double
  IBaseBayesNet<GUM_SCALAR>::log10DomainSize( void ) const {
    double dSize = 0.0;
  ...
  template<typename GUM_SCALAR> INLINE
  std::string
  IBaseBayesNet<GUM_SCALAR>::toString( void ) const {
    Size param = 0;
  ...
  template <typename GUM_SCALAR>
  bool
  IBaseBayesNet<GUM_SCALAR>::operator== ( const IBaseBayesNet<GUM_SCALAR>& from ) const {
    if ( dag() == from.dag() ) {
  ...
  template<typename GUM_SCALAR>
  void
  IBaseBayesNet<GUM_SCALAR>::completeInstantiation( Instantiation& I ) const {
    I.clear();
