...
// ensure binary read on windows
    ...
    bool Buffer::CanSeek() { return (stream != NULL) && (ftell(stream) != -1); }
    ...
    Token *Scanner::NextToken() {
      while (ch == ' ' || (ch >= 9 && ch <= 10) || ch == 13 || ch == L'=')
        NextCh();
      ...
      switch (state) {
        ...
        case_15:
          recEnd = pos;
          recKind = 2;
          if (ch == L'=' || (ch >= L'A' && ch <= L'D') ||
              (ch >= L'F' && ch <= L'Z') || ch == L'_' ||
              (ch >= L'a' && ch <= L'd') || (ch >= L'f' && ch <= L'z')) {
            AddCh();
            goto case_2;
