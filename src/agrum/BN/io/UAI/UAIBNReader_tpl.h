...
namespace gum {
  ...
  void UAIBNReader< GUM_SCALAR >::buildFromQuartets(
    ...
    for (NodeId i = 0; i < nbrPot; i++) {
      ...
      for (NodeId j = 1; j < nbrPar; j++) {
        ...
        if (papa >= nbrNode)
           _addError_(lig(), col(), "Not enough variables in the BayesNet");
  ...
  template < typename GUM_SCALAR >
  INLINE void UAIBNReader< GUM_SCALAR >:: _addFatalError_(Idx                lig,
                                                         Idx                col,
                                                         const std::string& s) {
  ...
  template < typename GUM_SCALAR >
  INLINE void UAIBNReader< GUM_SCALAR >:: _addError_(Idx                lig,
                                                    Idx                col,
                                                    const std::string& s) {
     _parser_->errors().addError(s,  _streamName_, lig, col);
  }
  template < typename GUM_SCALAR >
  INLINE void UAIBNReader< GUM_SCALAR >:: _addWarning_(Idx                lig,
                                                      Idx                col,
                                                      const std::string& s) {
