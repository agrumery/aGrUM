...
namespace gum {
  ...
  INLINE JunctionTreeInference<GUM_SCALAR>::~JunctionTreeInference() {
    ...
    GUM_DESTRUCTOR( JunctionTreeInference );
  ...
  template <typename GUM_SCALAR>
  INLINE void JunctionTreeInference<GUM_SCALAR>::eraseAllEvidence() {
    this->_invalidatePosteriors();
  ...
  INLINE void JunctionTreeInference<GUM_SCALAR>::__marginalizeOut(
    ...
    while ( !product_size.empty() ) {
      ...
      if ( pot_to_mult.size() == 1 ) {
        ...
        if ( pot_to_mult.size() == 1 ) {
          joint =
              const_cast<Potential<GUM_SCALAR>*>( *( pot_to_mult.begin() ) );
          joint_to_delete = false;
        } else {
      ...
      for ( const auto pot : pot_to_mult ) {
        ...
        for ( Idx i = 0; i < pot_vars.size(); ++i ) {
          if ( del_vars.contains( pot_vars[i] ) ) {
            ...
            if ( div_size != 1 ) {
              product_size.setPriority( pot_vars[i],
                                        product_size.priority( pot_vars[i] ) /
                                            div_size );
            }
          }
        }
  ...
  template <typename GUM_SCALAR>
  INLINE void JunctionTreeInference<GUM_SCALAR>::makeInference() {
    makeInference( false );
  }
  ...
  template <typename GUM_SCALAR>
  INLINE GUM_SCALAR JunctionTreeInference<GUM_SCALAR>::evidenceProbability() {
  ...
  template <typename GUM_SCALAR>
  INLINE GUM_SCALAR
  JunctionTreeInference<GUM_SCALAR>::VI( NodeId X, NodeId Y ) {
