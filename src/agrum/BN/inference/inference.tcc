...
namespace gum {
  ...
  template <typename GUM_SCALAR>
  INLINE const NodeSet& Inference<GUM_SCALAR>::targets() const {
    return __targets;
  }
  ...
  template <typename GUM_SCALAR>
  INLINE const Set<NodeSet>& Inference<GUM_SCALAR>::targetSets() const {
    return __settargets;
  }
  ...
  /// EVIDENCE ///
  template <typename GUM_SCALAR>
  void Inference<GUM_SCALAR>::__createHardEvidence(
      const NodeId id, const Idx val, Potential<GUM_SCALAR>& pot ) const {
    ...
    // creating the deterministic potential
    pot.beginMultipleChanges();

    while ( pot.nbrDim() > 0 ) {
      pot.erase( pot.variable( 0 ) );
    }
  ...
  INLINE void Inference<GUM_SCALAR>::addEvidence( const NodeId id,
    ...
    __createHardEvidence( id, val, pot );
    addEvidence( pot );
  }
  ...
  Inference<GUM_SCALAR>::addEvidence( const NodeId id,
    ...
    if ( _bn.variable( id ).domainSize() != vals.size() ) {
      GUM_ERROR( InvalidArgument,
                 _bn.variable( id ) << " and size of data do not correspond." );
    }
  ...
  Inference<GUM_SCALAR>::addEvidence( const Potential<GUM_SCALAR>& pot ) {
    ...
    NodeId id = _bn.variableNodeMap().get( pot.variable( 0 ) );
    ...
    if ( __isHardEvidence( pot, val ) ) {  // pot is deterministic
  ...
  template <typename GUM_SCALAR>
  INLINE void Inference<GUM_SCALAR>::clearEvidence( const NodeId id ) {
    if ( hasEvidence( id ) ) {
      if ( hasHardEvidence( id ) ) {
        __hardEvidence.erase( id );
      }
