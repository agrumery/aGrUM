...
// Use the SimpleCPTGenerator for generating the BNs CPT.
  ...
  MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::MCBayesNetGenerator ( Size nbrNodes, Size maxArcs, Idx maxModality, Size iteration , Idx p, Idx q ) :
    IBayesNetGenerator<GUM_SCALAR, ICPTGenerator> ( nbrNodes, maxArcs , maxModality ), _bayesNettemp(), _hashMarginal() {
  ...
  MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::MCBayesNetGenerator ( BayesNet<GUM_SCALAR> bayesNet, Size iteration , Idx p, Idx q ) :
    IBayesNetGenerator<GUM_SCALAR, ICPTGenerator> ( bayesNet.size(), ( Size ) ( bayesNet.sizeArcs() * 1.1 ) , getMaxModality ( bayesNet ) ), _bayesNettemp(), _hashMarginal() {
...
// density represent de
  ...
  void MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::disturbBN ( BayesNet<GUM_SCALAR>& bayesNetinit, Size iteration ) {  // insert option for the variation
    ...
    if ( __checkConditions() ) {
      ...
      while ( _hashMarginal.size() ) {
        delete ( * ( _hashMarginal.beginSafe() ) );
        _hashMarginal.erase ( _hashMarginal.beginSafe() );

      }

    } else {
      std::cout << IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.toDot()  << std::endl;
  ...
  bool MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__checkConditions() {
    ...
    return IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_maxArcs >= IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.dag().sizeArcs();
...
//main algorithme for moving between state of the IBayesNet according on the nature of the topology polytree or multi-connected
    ...
    if ( per < _p ) {
      ...
      if ( per < _p + _q ) {
        ...
        if ( !__checkConditions() ) {
          ...
          IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet = _bayesNettemp;
        } else _bayesNettemp = IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet;
  ...
  void MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__insertArc ( NodeId i, NodeId j ) {
    ...
    if ( _disturbing ) {
      ...
      this->disturbAugmCPT ( i, j, IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet, potj, ( GUM_SCALAR ) 0.5 );
    } else IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.addArc ( i, j );
  }
  ...
  void MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__eraseArc ( NodeId i, NodeId j, bool mustbeconnex ) {
    if ( _disturbing ) {
      const BayesNet<GUM_SCALAR> bayesNet ( IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet );
      Potential<GUM_SCALAR> potj;
      ...
      if ( __connect ( i, j ) || ! mustbeconnex ) {
        ...
        this->disturbReducCPT ( i, j, IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet, potj, marg );
      } else IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.addArc ( i, j );
    } else {
      IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.eraseArc ( i, j );
  ...
  void MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__chooseCloseNodes ( NodeId& i, NodeId& j ) {
    Idx temp = rand() % IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.size();
    Size co = 0;
  ...
  void MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__createTree ( Size BNSize ) {
    Idx n = 0;
    Idx nb_mod = 2 + rand() % ( IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_maxModality - 1 );
    std::stringstream strBuff;
    strBuff << "n_" << n++;
    NodeId root = IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.add ( LabelizedVariable ( strBuff.str(), "" , nb_mod ) );
...
// Allow to invert maximum nbiter arc to use from polytree only
  ...
  bool MCBayesNetGenerator<GUM_SCALAR, ICPTGenerator, ICPTDisturber>::__isPolytree() {
    const DAG __dag = IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.dag();
    return IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.size() - 1 == IBayesNetGenerator<GUM_SCALAR, ICPTGenerator>::_bayesNet.sizeArcs();
  }
