...
namespace gum {
  ...
  namespace learning {
    ...
    template < template < typename > class ALLOC >
    ScoreBDeu< ALLOC >*
       ScoreBDeu< ALLOC >::clone(const typename ScoreBDeu< ALLOC >::allocator_type& alloc) const {
    ...
    /// copy operator
    template < template < typename > class ALLOC >
    ScoreBDeu< ALLOC >& ScoreBDeu< ALLOC >::operator=(const ScoreBDeu< ALLOC >& from) {
      if (this != &from) {
    ...
    template < template < typename > class ALLOC >
    std::string ScoreBDeu< ALLOC >::isAprioriCompatible(const std::string& apriori_type,
                                                        double             weight) {
    ...
    /// indicates whether the apriori is compatible (meaningful) with the score
    template < template < typename > class ALLOC >
    INLINE std::string ScoreBDeu< ALLOC >::isAprioriCompatible(const Apriori< ALLOC >& apriori) {
