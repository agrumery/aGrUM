...
namespace gum {
  ...
  namespace learning {
    ...
    template < template < typename > class ALLOC >
    ScoreBIC< ALLOC >*
       ScoreBIC< ALLOC >::clone(const typename ScoreBIC< ALLOC >::allocator_type& alloc) const {
    ...
    /// copy operator
    template < template < typename > class ALLOC >
    ScoreBIC< ALLOC >& ScoreBIC< ALLOC >::operator=(const ScoreBIC< ALLOC >& from) {
      if (this != &from) {
    ...
    template < template < typename > class ALLOC >
    std::string ScoreBIC< ALLOC >::isAprioriCompatible(const std::string& apriori_type,
                                                       double             weight) {
      ...
      msg << "The apriori '" << apriori_type << "' is not yet compatible with the score 'BIC'.";
      return msg.str();
    }
    ...
    /// indicates whether the apriori is compatible (meaningful) with the score
    template < template < typename > class ALLOC >
    INLINE std::string ScoreBIC< ALLOC >::isAprioriCompatible(const Apriori< ALLOC >& apriori) {
    ...
    double ScoreBIC< ALLOC >::N(const IdCondSet< ALLOC >& idset) {
      // get the counts for all the nodes in the idset and add the apriori
      std::vector< double, ALLOC< double > > N_ijk(this->counter_.counts(idset, true));
      if (this->apriori_->isInformative()) this->apriori_->addAllApriori(idset, N_ijk);
