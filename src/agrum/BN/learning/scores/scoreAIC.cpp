...
#  ifdef GUM_NO_INLINE
#    include <agrum/BN/learning/scores/scoreAIC_inl.h>
#  endif /* GUM_NO_INLINE */
...
namespace gum {
  ...
  namespace learning {
    ...
    /// copy operator
    ScoreAIC& ScoreAIC::operator=(const ScoreAIC& from) {
      if (this != &from) {
        Score::operator=(from);
    ...
    /// move operator
    ScoreAIC& ScoreAIC::operator=(ScoreAIC&& from) {
      if (this != &from) {
    ...
    std::string ScoreAIC::isPriorCompatible(PriorType prior_type, double weight) {
      ...
      return std::format("The prior '{}' is not yet compatible with the score 'AIC'.",
                         priorTypeToString(prior_type));
    }

    /// returns the score corresponding to a given nodeset
    double ScoreAIC::score_(const IdCondSet& idset) {
