...
namespace gum {
    ...
    if ( _functionGraph_->isTerminalNode(eraseId)) {
      for (auto iterVar =  _functionGraph_->variablesSequence().begin();
        ...
        Link< NodeId >* nodeIter
           =  _functionGraph_-> _var2NodeIdMap_[*iterVar]->list();
        while (nodeIter != nullptr) {
          for (Idx modality = 0; modality < (*iterVar)->domainSize(); ++modality)
            if ( _functionGraph_->node(nodeIter->element())->son(modality)
                == eraseId)
      ...
      if (updateParents) {
       ...
       _functionGraph_
         -> _var2NodeIdMap_[ _functionGraph_-> _internalNodeMap_[eraseId]->nodeVar()]
  ...
  void MultiDimFunctionGraphManager< GUM_SCALAR,
    ...
    for (SequenceIteratorSafe< const DiscreteVariable* > varIter
      ...
      const Link< NodeId >* curElem
         =  _functionGraph_-> _var2NodeIdMap_[*varIter]->list();
      Idx nbElem = 0;
    ...
    if ( _functionGraph_->variablesSequence().pos(movedVar) > desiredPos)
      for (Idx currentPos =  _functionGraph_->variablesSequence().pos(movedVar);
        ...
        const DiscreteVariable* preVar
           =  _functionGraph_->variablesSequence().atPos(currentPos - 1);
  ...
  void MultiDimFunctionGraphManager< GUM_SCALAR, TerminalNodePolicy >::
      _adjacentSwap_(const DiscreteVariable* x, const DiscreteVariable* y) {
    ...
    while (oldxNodes->list()) {
      ...
      currentOldXNode
         =  _functionGraph_-> _internalNodeMap_[oldxNodes->list()->element()];
        ...
        if (currentNewYNodeId != 0) {
          ...
          for (Idx i = 0; i < currentOldXNode->nodeVar()->domainSize(); ++i) {
            if ( _functionGraph_-> _internalNodeMap_.exists(
               ...
               _functionGraph_-> _internalNodeMap_[currentOldXNode->son(i)]
          ...
          for (Idx i = 0; i < currentOldXNode->nodeVar()->domainSize(); ++i) {
            if ( _functionGraph_-> _internalNodeMap_.exists(
               ...
               _functionGraph_-> _internalNodeMap_[currentNewYNodeSons[i]]
                 ->addParent(oldxNodes->list()->element(), i);
            }
          }

           _functionGraph_-> _var2NodeIdMap_[y]->addLink(
    ...
    while (oldyNodes->list()) {
      ...
      if ( _functionGraph_-> _internalNodeMap_[curId]->parents() == nullptr) {
        for (Idx i = 0;
             i
             <  _functionGraph_-> _internalNodeMap_[curId]->nodeVar()->domainSize();
             ++i)
          if ( _functionGraph_-> _internalNodeMap_.exists(
                  _functionGraph_-> _internalNodeMap_[curId]->son(i)))
             _functionGraph_
               -> _internalNodeMap_[ _functionGraph_-> _internalNodeMap_[curId]->son(
                  i)]
  ...
  template < typename GUM_SCALAR, template < class > class TerminalNodePolicy >
  INLINE NodeId MultiDimFunctionGraphManager< GUM_SCALAR, TerminalNodePolicy >::
      _checkIsomorphism_(const DiscreteVariable* var, NodeId* sons) {
  ...
  template < typename GUM_SCALAR, template < class > class TerminalNodePolicy >
  INLINE bool
     MultiDimFunctionGraphManager< GUM_SCALAR, TerminalNodePolicy >:: _isRedundant_(
      ...
      while (currentNodeId != nullptr) {
        ...
        if (nextNodeId) {
          ...
          while (anotherNodeId->nextLink() != nullptr) {
            ...
            anotherNode
               =  _functionGraph_-> _internalNodeMap_[anotherNodeId->element()];
  ...
  INLINE void
    ...
    Sequence< const DiscreteVariable* > oldSequence(
        _functionGraph_->variablesSequence());
    for (SequenceIterator< const DiscreteVariable* > varIter = oldSequence.begin();
         varIter != oldSequence.end();
         ++varIter)
      if (! _functionGraph_->varNodeListe(*varIter)->list())
         _functionGraph_->erase(**varIter);
  }
