...
#  pragma omp parallel num_threads(int(nb_threads))
        {
          ...
          if (this_thread + i < nb_ranges) {
            DBRowGeneratorParser& parser = _parsers_[this_thread].data;
    ...
    void RecordCounter::_threadedCount_(
       const std::size_t                                           begin,
       const std::size_t                                           end,
       DBRowGeneratorParser&                                       parser,
       const std::vector< std::pair< std::size_t, std::size_t > >& cols_offsets,
       std::vector< double >&                                      countings) {
