...
namespace gum {
  ...
  INLINE ListConstIterator< Val >::ListConstIterator(const List< Val >& theList,
    ...
    if (ind_elt < (theList. _nb_elements_ >> 1)) {
      ...
      for ( _bucket_ = theList. _deb_list_; ind_elt;
           --ind_elt,  _bucket_ =  _bucket_-> _next_) {}
    } else {
      // find the element we shall point to src the end of the list
      for ( _bucket_ = theList. _end_list_,
          ind_elt   = theList. _nb_elements_ - ind_elt - 1;
           ind_elt;
  ...
  template < typename Val >
  INLINE ListBucket< Val >*
         ListConstIterator< Val >:: _getBucket_() const noexcept {
    return  _bucket_;
  }
  ...
  INLINE ListConstIteratorSafe< Val >::ListConstIteratorSafe(
     const ListConstIteratorSafe< Val >& src) :
       ...
       _prev_current_bucket_{src. _prev_current_bucket_},  _null_pointing_{
                                                           src. _null_pointing_} {
    // for debugging purposes
  ...
  ListConstIteratorSafe< Val >::ListConstIteratorSafe(
    ...
    if (ind_elt < ( _list_-> _nb_elements_ >> 1)) {
      // find the element we shall point to src the start of the list
      for ( _bucket_ =  _list_-> _deb_list_; ind_elt;
           --ind_elt,  _bucket_ =  _bucket_-> _next_) {}
    } else {
      // find the element we shall point to src the end of the list
      for ( _bucket_ =  _list_-> _end_list_,
          ind_elt   =  _list_-> _nb_elements_ - ind_elt - 1;
           ind_elt;
           --ind_elt,  _bucket_ =  _bucket_-> _prev_) {}
    }
  ...
  template < typename Val >
  INLINE ListBucket< Val >*
         ListConstIteratorSafe< Val >:: _getBucket_() const noexcept {
    return  _bucket_;
  }
  ...
  INLINE bool ListConstIteratorSafe< Val >::isEnd() const {
    return  _null_pointing_ ? ( _next_current_bucket_ == nullptr)
                                && ( _prev_current_bucket_ == nullptr)
                           : ( _bucket_ == nullptr);
  }
  ...
  void List< Val, Alloc >:: _copy_elements_(const List< Val, OtherAlloc >& src) {
    ...
    try {
      ...
      for (;  _deb_list_ != nullptr;
            _deb_list_ = const_cast< ListBucket< Val >* >(ptr)) {
  ...
  void List< Val, Alloc >::clear() {
    ...
    for (ListBucket< Val >*ptr =  _deb_list_, *next_ptr = nullptr; ptr != nullptr;
         ptr = next_ptr) {
      next_ptr = ptr-> _next_;
  ...
  INLINE List< Val, Alloc >::List(List< Val, Alloc >&& src) :
       ...
       _safe_iterators_{std::move(src. _safe_iterators_)},  _alloc_bucket_{std::move(
                                                            src. _alloc_bucket_)} {
    // for debugging purposes
    GUM_CONS_MOV(List);
  ...
  INLINE const ListConstIteratorSafe< Val >&
    ...
    return *(
       reinterpret_cast< const ListConstIteratorSafe< Val >* >( _list_end_safe_));
  }
  ...
  template < typename Val, typename Alloc >
  INLINE ListBucket< Val >*
         List< Val, Alloc >:: _createBucket_(const Val& val) const {
  ...
  template < typename... Args >
  INLINE ListBucket< Val >*
         List< Val, Alloc >:: _createEmplaceBucket_(Args&&... args) const {
    // create a new bucket (catch allocation and construction exceptions)
    ListBucket< Val >* new_elt =  _alloc_bucket_.allocate(1);
  ...
  template < typename Val, typename Alloc >
  INLINE ListBucket< Val >*
         List< Val, Alloc >:: _getIthBucket_(Size i) const noexcept {
    ListBucket< Val >* ptr;

    if (i <  _nb_elements_ / 2) {
      for (ptr =  _deb_list_; i; --i, ptr = ptr-> _next_) {}
    } else {
      for (ptr =  _end_list_, i =  _nb_elements_ - i - 1; i;
           --i, ptr            = ptr-> _prev_) {}
    }
  ...
  INLINE Val& List< Val, Alloc >::emplace(const const_iterator& iter,
                                          Args&&... args) {
    return  _insert_(iter,
                     _createEmplaceBucket_(std::forward< Args >(args)...),
                    location::BEFORE);
  }
  ...
  template < typename Val, typename Alloc >
  INLINE ListBucket< Val >*
         List< Val, Alloc >:: _getBucket_(const Val& val) const noexcept {
    for (ListBucket< Val >* ptr =  _deb_list_; ptr != nullptr; ptr = ptr-> _next_)
  ...
  INLINE void List< Val, Alloc >::eraseAllVal(const Val& val) {
    for (ListBucket< Val >*iter =  _deb_list_, *next_bucket = nullptr;
         iter != nullptr;
         iter = next_bucket) {
  ...
  std::string List< Val, Alloc >::toString() const {
    ...
    for (ListBucket< Val >* ptr =  _deb_list_; ptr != nullptr;
         ptr = ptr-> _next_, deja = true) {
      if (deja) stream << " --> ";
  ...
  INLINE bool
    ...
    // parse the two lists
    for (ListBucket< Val >*iter1 =  _deb_list_, *iter2 = src. _deb_list_;
         iter1 != nullptr;
