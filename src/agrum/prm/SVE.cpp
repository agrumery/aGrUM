...
//   for (ElimIter iter = __elim_orders.begin(); iter != __elim_orders.end();
//        ++iter)
//   {
//     delete *iter;
//   }
//   typedef HashTable<const Class*, SVE::ArraySet*>::iterator LiftedIter;
//   for (LiftedIter iter = __lifted_pools.begin();
//        iter != __lifted_pools.end(); ++iter) {
//     for (SVE::ArraySetIterator jter = (**iter).begin();
//          jter != (**iter).end(); ++jter) {
//       delete *jter;
//     }
//     delete *iter;
...
//   }
//   for (SVE::BucketSetIterator iter = __lifted_trash.begin();
//        iter != __lifted_trash.end(); ++iter) {
//     delete *iter;
//   }
// }
// 
// void
// SVE::__eliminateNodes(const Instance* query, NodeId node,
...
//   for (Instance::PSISCIterator node = i->beginPSISC();
//       node != i->endPSISC(); ++node) {
//     for (Set< Instance::InverseSC* >::iterator inv_sc = (*node)->begin();
//         inv_sc != (*node)->end(); ++inv_sc) {
//       if (not ignore.exists((*inv_sc)->first)) {
//         __eliminateNodesDownward(i, (*inv_sc)->first, pool,
//                                  trash, my_list, ignore);
//       }
//     }
//   }
...
//   for (List<const Instance*>::iterator iter = my_list.begin();
...
//   for (Set<SlotChain*>::iterator sc = i->slotChains().begin();
//       sc != i->slotChains().end(); ++sc) {
//     for (Set<Instance*>::iterator iter =
//         i->getInstances((**sc).id()).begin();
//         iter != i->getInstances((**sc).id()); ++iter)
//     {
//       if (*iter != from) {
//         elim_list.insert(*iter);
//       }
...
//   // Downward elimination
//   ignore.insert(i);
//   for (Instance::PSISCIterator child = i->beginPSISC();
//       child != i->endPSISC(); ++child) {
...
//         i->getInstances((**sc).id()).begin();
//         parent != i->getInstances((**sc).id()).end(); ++parent)
//     {
//       if (not ignore.exists(*parent)) {
//         __eliminateNodesUpward(*parent, pool, trash, tmp_list, ignore);
//       }
...
// 
// void
// SVE::__eliminatNodesWithEvidence(const Instance* i, BucketSet& pool, BucketSet& trash) {
...
// 
// void
// SVE::_marginal(const Instance* i, const Attribute* elt, Potential<prm_float>& m)
// {
...
// 
// void
// SVE::_joint(const Set<InstAttrPair>& queries, Potential<prm_float>& j) {
