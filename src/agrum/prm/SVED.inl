...
INLINE
void
SVED::_evidenceAdded(const Instance* i, NodeId elt_id) {
  // Do nothing
}

INLINE
void
SVED::_evidenceRemoved(const Instance* i, NodeId elt_id) {
  // Do nothing
}
                                ...
                                Set<const Instance*>& ignore)
{
  try {
    ...
    for (InvIter inv_sc = i->inverseSC(attr).begin();
         inv_sc != i->inverseSC(attr).end(); ++inv_sc) {
      if ( (not ignore.exists((*inv_sc)->first)) and
           (__bb.exists((*inv_sc)->first)) )
      {
        __eliminateNodesDownward(i, (*inv_sc)->first, pool, trash,
                                 elim_list, ignore);
      }
    }
  } catch (NotFound&) {
  ...
  while (not elim_list.empty()) {
    if (__checkElimOrder(i, elim_list.front())) {
      if ( (not ignore.exists(elim_list.front())) and
        ...
        __eliminateNodesDownward(i, elim_list.front(), pool,
