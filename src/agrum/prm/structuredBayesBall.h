  ...
  namespace prm {
    ...
    class StructuredBayesBall {
      ...
      private:
        ...
        /// Code alias
        std::pair<bool, bool>& __getMark( InstanceMap& marks, const Instance<GUM_SCALAR>* i, NodeId n );
        /// Code alias
        ...
        std::string __buildHashKey( const Instance<GUM_SCALAR>* i, Set<NodeId>& req_nodes );

        /// The PRM at which __model belongs.
        const PRMInference<GUM_SCALAR>* __inf;

        /// Associate an Instance<GUM_SCALAR> with a unique key w.r.t. d-separation and the
        /// set of requisite nodes deduced from d-separation analysis.
        HashTable<const Instance<GUM_SCALAR>*, std::pair<std::string, Set<NodeId>*> > __keyMap;
...
#include <agrum/prm/structuredBayesBall.tcc>
