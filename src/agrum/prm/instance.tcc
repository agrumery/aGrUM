...
namespace gum {
  namespace prm {
    ...
    Size
    Instance<GUM_SCALAR>::size() const {
    ...
    template<typename GUM_SCALAR> INLINE
    PRMObject::PRMType
    Instance<GUM_SCALAR>::obj_type() const { return PRMObject::PRMType::INSTANCE; }

    template<typename GUM_SCALAR> INLINE
    Class<GUM_SCALAR>&
    Instance<GUM_SCALAR>::type() { return *__type; }

    template<typename GUM_SCALAR> INLINE
    const Class<GUM_SCALAR>&
    Instance<GUM_SCALAR>::type() const { return *__type; }
    ...
    template<typename GUM_SCALAR> INLINE
    Instance<GUM_SCALAR>&
    Instance<GUM_SCALAR>::RefIterator::operator*() const {
      return **__iter;
    }
    ...
    template<typename GUM_SCALAR> INLINE
    const Instance<GUM_SCALAR>&
    Instance<GUM_SCALAR>::RefConstIterator::operator*() const {
      return **__iter;
    }
    ...
    Instance<GUM_SCALAR>::__copyAttributeCPF( Attribute<GUM_SCALAR>* attr ) {
      ...
      // } catch (NotFound& e) {
      //   if (dynamic_cast<const MultiDimAggregator<GUM_SCALAR>*>(type().get(attr->safeName()).cpf().getContent())) {
      //     delete attr->__cpf;
