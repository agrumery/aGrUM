...
NameGenerator::nextName(PRMObject::ObjectType type) {
  ...
  switch (type) {
    ...
    case PRMObject::prm_system:
    case PRMObject::prm_interface:
    case PRMObject::prm_type:     { s << ++(__counters[0]); break; }
    case PRMObject::prm_class_elt:{ s << ++(__counters[1]); break; }
    case PRMObject::prm_instance: { s << ++(__counters[2]); break; }
