...
// std::vector<NodeId>*
// StructuredInference::__getClassOutputs(const Class* c) {
//   try {
//     return __outputs[c];
//   } catch (NotFound&) {
//     std::vector<NodeId>* outputs = new std::vector<NodeId>();
//     for (Class::const_ClassEltIterator elt = c->begin(); elt != c->end(); ++c) {
//       if (c->isOutputNode(**elt)) {
//         outputs->push_back((**elt).id());
//       }
//     }
//     __outputs.insert(c, outputs);
//     return outputs;
//   }
// }

// void
// StructuredInference::__addNodesInReducedGraph(StructuredInference::RGData& data) {
//   // First we arrange queries in a tractable way
//   std::vector<PRMInference::Chain>* v = 0;
//   HashTable<const Instance*, std::vector<PRMInference::Chain>*> queries;
//   typedef HashTable< const gspan::Pattern*, std::vector<PRMInference::Chain>*>::iterator QIter;
//   for (QIter chains = __q.begin(); chains != __q.end(); ++chains) {
//     v = *chains;
//     for (std::vector<PRMInference::Chain>::iterator chain = v->begin(); chain != v->end(); ++chain) {
//       try {
//         queries[chain->first]->push_back(*chain);
//       } catch (NotFound&) {
//         queries.insert(chain->first, new std::vector<PRMInference::Chain>());
//         queries[chain->first]->push_back(*chain);
//       }
//     }
//   }
//   // Second we add nodes while building the partial order for triangulation.
//   const Sequence<Instance*>* instance_set = 0;
//   const Attribute* a = 0;
//   std::vector<NodeId>* outputs;
//   NodeId n = 0;
//   // Adding nodes
//   for (std::vector<gspan::Pattern*>::iterator p = __gspan->patterns().begin(); p != __gspan->patterns().end(); ++p) {
//     for (GSpan::MatchedInstances::iterator match = __gspan->matches(**p).begin(); match != __gspan-->matches(**p).end(); ++match) {
//       instance_set = *match;
//       for (Set<Instance*>::const_iterator i = instance_set->begin(); i != instance_set->end(); ++i) {
//         // Lets add the instances outputs nodes
//         for (std::vector<NodeId>::iterator node = outputs->begin(); node != outputs->end(); ++node) {
//           if ((**i).hasRefAttr(*node)) {
//             n = data.reducedGraph.insertNode();
//             a = &((**i).get(*node));
//             data.mods.insert(n, a->type().variable().domainSize());
//             data.node2attr.insert(n, a);
//             data.var2node.insert(&(a->type().variable()), n);
//             data.outputs().insert(n);
//           }
//         }
//         // Checking queries
//         try {
//           v = queries[*i];
//           for (std::vector<PRMInference::Chain>::iterator chain = v->begin(); chain != v->end(); ++chain) {
//             if ((**i).type().isOutputNode(*(chain->second))) {
//               n = data.var2node[&(chain->second->type().variable())];
//               data.outputs().erase(n);
//               data.queries().insert(n);
//             } else {
//               n = data.reducedGraph.insertNode();
//               a = chain->second;
//               data.mods.insert(n, a->type().variable().domainSize());
...
//             }
//           }
//         } catch (NotFound&) { } // No query over i
//       }
//     }
//   }
//   // Deleting the stuff we needed to handle queries
//   typedef HashTable<const Instance*, std::vector<PRMInference::Chain>*>::iterator QJter;
//   for (QJter j = queries.begin(); j != queries.end(); ++j)
//     delete *j;
// }

// void
// StructuredInference::__buildQuerySet(StructuredInference::PData& data) {
//   if (__q.exists(&(data.pattern))) {
//     std::vector<PRMInference::Chain>* chains = __q[&(data.pattern)];
//     for (std::vector<PRMInference::Chain>::iterator chain = chains->begin(); chain != chains->end(); ++chain) {
//       std::string inst_name = chain->first->name();
//       std::string attr_name = chain->second->safeName();
//       for (GSpan::MatchedInstances::const_iterator match = data.matches.begin(); match != data.matches.end(); ++match) {
//         for (Sequence<Instance*>::const_iterator iter = (**match).begin(); iter != (**match).end(); ++iter) {
//           if ((**iter).name() == inst_name) {
//             GUM_ASSERT((**iter).exists(attr_name));
//             GUM_ASSERT(data.match().atPos(iter.pos())->exists(attr_name));
//             inst_name = data.match().atPos(iter.pos())->name();
//             NodeId node = data.node2attr.first(inst_name + __dot + attr_name);
//             data.inners().erase(node);
//             data.outputs().erase(node);
//             data.queries().insert(node);
//             match = data.matches.end();
//             break;
//           }
//         }
//       }
