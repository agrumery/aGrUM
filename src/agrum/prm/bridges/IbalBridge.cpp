...
namespace gum {
                          ...
                          const Class& c, const Aggregate& agg)
  {
    ...
    if (c.parents(agg).size() > 0) {
      const ClassElement& parent = c.get(c.parents(agg).begin()->tail());
  ...
  {
    if (c.parents(attr).size() == 0) {
      __print_att_no_parents(output, prm, c, attr);
    } else if (c.parents(attr).size() == 1) {
                                      ...
                                      const Class& c, const Attribute& attr)
  {
    ...
    for (DAG::ArcIterator iter = c.parents(attr).begin();
         iter != c.parents(attr).end(); ++iter) {
  ...
  IBALBridge::__print_agg_parents_list(const Model& m, const Instance& i, NodeId id)
  {
    ...
    typedef Set<Instance*>::const_iterator inst_iter;
    bool first = true;
    for (DAG::ArcIterator iter = i.parents(id).begin(); iter != i.parents(id).end(); ++iter) {
