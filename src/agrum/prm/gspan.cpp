...
//   typedef Bijection<Idx, LabelData*>::iterator BijIterator;
//   int count = 0;
...
//         __nodes.push_back(const_cast<LabelData*>(iter.second()));
//       }
//     } catch (NotFound&) {
//       ++count;
//       // It's a label over edges
//       EdgeData* e = *(__graph->edges(iter.second()).begin());
...
//     }
//   }
//   Bijection<Idx, LabelData*>* new_labels = new Bijection<Idx, LabelData*>();
//   GSpan::LabelSort my_sort(this);
//   std::sort(__nodes.begin(), __nodes.end(), my_sort);
//   std::sort(__edges.begin(), __edges.end(), my_sort);
//   Size idx = 0;
//   for (std::vector<LabelData*>::iterator iter = __nodes.begin(); iter != __nodes.end(); ++iter) {
//     (*iter)->id = ++idx;
//     new_labels->insert(idx, *iter);
//   }
//   for (std::vector<LabelData*>::iterator iter = __edges.begin(); iter != __edges.end(); ++iter) {
//     (*iter)->id = ++idx;
//     new_labels->insert(idx, *iter);
//     __tree.addRoot(**iter);
//   }
...
// 
// void
// GSpan::__subgraph_mining(InterfaceGraph& ig, Pattern& pat) {
...
//       std::vector< HashTable<std::string, DFSTree::EdgeGrowth*>* > count_vector;
//       for (std::list<NodeId>::iterator plop = r_path.begin(); plop != r_path.end(); ++plop) {
//         count_vector.push_back(new HashTable<std::string, DFSTree::EdgeGrowth*>());
//       }
...
//           HashTable<std::string, DFSTree::EdgeGrowth*>& edge_count = *(count_vector[idx]);
...
//               EdgeData& edge_data = ig.edge(current_id, neighbor_id);
//               LabelData* neighbor_label = (neighbor == edge_data.u)?edge_data.l_u:edge_data.l_v;
//               NodeId neighbor_node = (seq->exists(neighbor))?seq->pos(neighbor) + 1:0;
//               // The temp_growth
//               DFSTree::EdgeGrowth temp_growth(*node, edge_data.l, neighbor_label, neighbor_node);
//               try {
//                 DFSTree::EdgeGrowth* edge_growth = edge_count[temp_growth.toString()];
//                 edge_growth->insert(current, neighbor);
//               } catch (NotFound&) {
//                 DFSTree::EdgeGrowth* edge_growth = new DFSTree::EdgeGrowth(*node, edge_data.l, neighbor_label, neighbor_node);
//                 edge_growth->insert(current, neighbor);
...
//       typedef HashTable<std::string, DFSTree::EdgeGrowth*>::iterator EdgeCountIter;
//       for (size_t node = 0; node < count_vector.size(); ++node) {
//         HashTable<std::string, DFSTree::EdgeGrowth*>* edge_count = count_vector[node];
...
//   for (std::vector<Pattern*>::iterator p = __patterns.begin() + 1; p != __patterns.end(); ++p) {
//     UndiGraph iso_graph;
...
//       }
//       if (not found) {
//         // We add stuff to compute the max independent set over the remaining matches of p
...
//     DFSTree::NeighborDegreeSort my_sort(iso_graph);
...
// GSpan::patterns_cost() {
//   double cost = 0;
//   typedef HashTable<Pattern*, MatchedInstances*>::iterator MatchIter;
...
// 
// double
// GSpan::__instance_cost(Instance* i) {
//   unsigned cost = 1;
//   Set<ClassElement*> inputs;
//   for (Set<SlotChain*>::iterator slot_chain = i->slotChains().begin(); slot_chain != i->slotChains().end(); ++slot_chain) {
...
//     if (i->type().isOutputNode(*node) and i->hasInverseSC(*node)) {
...
//     ValueElimination<prm_float> ve(bn);
//     ve.makeInference();
//     const std::vector<NodeId>& elim = ve.eliminationOrder();
//     Set<AbstractCPF<prm_float>*> pool;
//     for (DAG::NodeIterator node = bn.dag().beginNodes(); node != bn.dag().endNodes(); ++node) {
//       pool.insert(const_cast<AbstractCPF<prm_float>*>(&(bn.cpt(*node))));
//     }
//     Set<AbstractCPF<prm_float>*> trash;
...
//             for (ArcSet::iterator parent = (*i)->dag().parents(elt.id()).begin(); parent != (*i)->dag().parents(elt.id()).end(); ++parent) {
