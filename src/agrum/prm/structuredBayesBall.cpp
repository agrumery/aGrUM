...
//   for (HashTable<std::string, std::pair< Set<NodeId>*, Size> >::iterator iter = __reqMap.begin();
//        iter != __reqMap.end(); ++iter) {
//     delete iter->first;
//   }
// }
// 
...
//   if (i->get(n).elt_type() != ClassElement::prm_attribute) {
//     return false;
//   }
//   if (__inf->hasEvidence(*i, static_cast<const Attribute&>(i->get(n)))) {
...
//   // GUM_TRACE_VAR(marks.size());
//   __fillMaps(marks);
//   for (StructuredBayesBall::InstanceMap::iterator iter = marks.begin();
//        iter != marks.end(); ++iter) {
//     delete *iter;
//   }
// }
...
//                                         if ( (not __getMark(marks, i, n).first) and
//                                             (not __isHardEvidence(i, n)) ) {
//                                           __getMark(marks, i, n).first = true;
//                                           for (ArcSetIterator prnt = i->dag().parents(n).begin();
//                                               prnt != i->dag().parents(n).end(); ++prnt) {
...
//                                           // Out of i.
//                                           try {
//                                             for (Set< Instance::InverseSC* >::iterator iter = i->inverseSC(n).begin();
//                                                 iter != i->inverseSC(n).end(); ++iter) {
//                                               __fromParent((**iter).first, (**iter).second, marks);
//                                             }
//                                           } catch (NotFound&) {
//                                             // Not an inverse sc
//                                           }
...
//                                       }
//     default: {
//                // We shouldn't reach any other ClassElement
//                // than Attribute or SlotChain.
...
//     marks[i]->insert(n, std::pair<bool, bool>(false, false));
//   }
//   // Concerns only Attribute
//   if ( (__isHardEvidence(i, n)) and
//        (not __getMark(marks, i, n).first) ) {
//     __getMark(marks, i, n).first = true;
//     for (ArcSetIterator iter = i->dag().parents(n).begin();
//          iter != i->dag().parents(n).end(); ++iter) {
//       __fromChild(i, iter->tail(), marks);
//     }
//   } else if (not __getMark(marks, i, n).second) {
//     __getMark(marks, i, n).second = true;
//     // In i.
//     for (ArcSetIterator iter = i->dag().children(n).begin();
//          iter != i->dag().children(n).end(); ++iter) {
...
//     for (StructuredBayesBall::MarkMap::iterator jter = (**iter).begin();
//          jter != (**iter).end(); ++jter) {
//       if (jter->first) {
...
//   for (HashTable<const Instance*, Set<NodeId>*>::iterator iter = req_map.begin();
//        iter != req_map.end(); ++iter) {
//     if ((**iter).size() == 0) {
//       to_remove.insert(iter.key());
//     }
//   }
//   for (Set<const Instance*>::iterator iter = to_remove.begin(); iter != to_remove.end();
//        ++iter) {
//     delete req_map[*iter];
//     req_map.erase(*iter);
//   }
//   // Fill __reqMap and __keyMap
//   //GUM_TRACE_VAR(req_map.size());
//   for (HashTable<const Instance*, Set<NodeId>*>::iterator iter = req_map.begin();
//        iter != req_map.end(); ++iter) {
...
//     }
//   }
//   // //GUM_TRACE_VAR(__reqMap.size());
//   // for (HashTable<std::string, std::pair< Set<NodeId>*, Size> >::iterator iter = __reqMap.begin();
//   //      iter != __reqMap.end(); ++iter) {
//   //   std::stringstream sBuff;
//   //   sBuff << iter.key() << ": " << "[";
//   //   for (Set<NodeId>::iterator node = iter->first->begin(); node != iter->first->end(); ++node) {
//   //     sBuff << " " << *node;
//   //   }
//   //   sBuff << " ] - repeat: " << iter->second;
//   //   GUM_TRACE(sBuff.str());
//   // }
//   // GUM_TRACE_VAR(__keyMap.size());
//   // for (HashTable<const Instance*, std::pair<std::string, Set<NodeId>*> >::iterator iter = __keyMap.begin();
//   //      iter != __keyMap.end(); ++iter) {
//   //   std::stringstream sBuff;
//   //   sBuff << iter.key()->name() << " - " << iter->first << " - " << iter->second->size();
//   //   GUM_TRACE(sBuff.str());
//   // }
// }
// 
...
//   std::stringstream sBuff;
//   sBuff << i->type().name();
//   for (DAG::NodeIterator node = i->type().dag().beginNodes();
//        node != i->type().dag().endNodes(); ++node) {
