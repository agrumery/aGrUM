...
// // ============================================================================
// #ifdef GUM_NO_INLINE
// #include <agrum/prm/interfaceGraph.inl>
// #endif // GUM_NO_INLINE
// // ============================================================================
// namespace gum {
// 
// InterfaceGraph::InterfaceGraph(Model& model):
//   __model(&model), __labels(new Bijection<Idx, LabelData*>()),
...
//   for (Model::iterator iter = model.begin(); not iter.isEnd(); ++iter) {
...
//         list.pop_front();
//         // We add input edges
//         for (Set<SlotChain*>::iterator iter = data->n->slotChains().begin();
//              iter != data->n->slotChains().end(); ++iter) {
...
//         }
//         // We add output edges
//         for (Instance::PSISCIterator iter = data->n->beginPSISC();
//              iter != data->n->endPSISC(); ++iter) {
//           for (Set< Instance::InverseSC* >::iterator jter = (**iter).begin();
//                jter != (**iter).end(); ++jter) {
...
//   __model(source.__model), __graph(source.__graph), __nodes(source.__nodes),
...
//   for (DAG::NodeIterator iter = node->n->type().dag().beginNodes();
//        iter != node->n->type().dag().endNodes(); ++iter) {
//     if (node->n->type().isOutputNode(*iter)) {
//       try {
//         sBuff << "-" << node->n->inverseSC(*iter).size() << node->n->get(*iter).name();
