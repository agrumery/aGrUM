...
//       for (Set<SlotChain*>::iterator input = (*iter)->slotChains().begin();
//            input != (*iter)->slotChains().end(); ++input)
//       {
//         for (Set<Instance*>::iterator jter = (*iter)->getInstances((*input)->id()).begin();
//              jter != (*iter)->getInstances((*input)->id()).end(); ++jter) {
//           if ((not seq.exists(*jter)) and
//               (not input_set.exists(&((*jter)->get((*input)->lastElt().id())))) ) {
...
//         if ( (*iter)->type().isOutputNode(*node) and
//              (*iter)->hasInverseSC(*node) )
//         {
//           for (Set<Instance::InverseSC*>::iterator inverse = (*iter)->inverseSC(*node).begin();
//                inverse != (*iter)->inverseSC(*node).end(); ++inverse)
//           {
