...
namespace gum {
  ...
  Database Database::createFromCSV ( const std::string& filename,
    ...
    modal_names.resize ( database.__nb_nodes,
                         HashTable<std::string, unsigned int>() );
  ...
  Database Database::createFromCSVAndBN ( IBayesNet<float>* bn,
    ...
    // get the number of nodes and the number of __cases n the database
    {
      ...
      database.__node_names = SplitCSVLine ( str, separator_separator, field_delimiter, escape_char );
      database.__nb_nodes = database.__node_names.size();
    ...
    if ( ! inFile.is_open() )
      GUM_ERROR ( IOError, "cannot open file8" << filename );
