...
namespace gum {
  ...
  namespace learning {
    ...
    /// default constructor
    ScoreLog2Likelihood::ScoreLog2Likelihood ( const Database& database,
    ...
    void
    ScoreLog2Likelihood::_computeScores
    ( const std::vector<unsigned int>& db_single_ids ) {
      ...
      for ( ListConstIteratorUnsafe<CountingTreeTargetSetBox*> iter =
              records.cbeginUnsafe (); iter != records.cendUnsafe (); ++iter ) {
      ...
      for ( unsigned int i = 0; i < db_single_ids.size (); ++i ) {
        _single_scores.insert ( db_single_ids[i], score[i] * _1log2 );
      }
    }
    ...
    ( const std::vector< std::pair<unsigned int,
      ...
      for ( unsigned int i = 0; i < db_pair_ids.size (); ++i ) {
        _pair_scores.insert ( db_pair_ids[i], score[i] * _1log2 );
      }
    }
