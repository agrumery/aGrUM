...
namespace gum {
  ...
  namespace learning {
    ...
    INLINE
    typename ParamEstimator2< ALLOC >::allocator_type
    ...
    operator=(const ParamEstimator2< ALLOC >& from) {
      if ( this != &from ) {
        ...
        _external_apriori =
          from._external_apriori->clone(this->getAllocator());
    ...
    Potential< GUM_SCALAR >& pot) {
      ...
      if ( node2cols.empty() ) {
        ...
        std::size_t col = node2cols[target_node];
        ...
        for ( std::size_t i = 1; i < vars.size(); ++i ) {
          col = node2cols[conditioning_nodes[i-1]];
    ...
    template < typename GUM_SCALAR >
    INLINE void
    ParamEstimator2< ALLOC >::setParameters(
      const NodeId target_node,
      const std::vector< NodeId, ALLOC< NodeId> >& conditioning_nodes,
      Potential< GUM_SCALAR >& pot) {
      _checkParameters(target_node, conditioning_nodes, pot);
      
      const std::vector< double, ALLOC< double > >
        nc(parameters(target_node,conditioning_nodes));
