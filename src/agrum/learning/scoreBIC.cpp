...
namespace gum {
  ...
  namespace learning {
      ...
      for ( const ListBucket<CountingTreeTargetSetBox*>* iter =
        ...
        for ( unsigned int i = 0; i < db_single_ids.size (); ++i ) {
          ...
          for ( unsigned int j = 0; j < records.size (); ++j ) { 
            score[i] += records[j] * log ( records[j] / Nij );
          }
        }
      }
    ...
    ScoreBIC::_computeScores
      ...
      // save the size of the pairs of variables
      std::vector<unsigned int> first_size ( db_pair_ids.size () );
      std::vector<unsigned int> total_size ( db_pair_ids.size () );
      for ( unsigned int i = 0; i < db_pair_ids.size (); ++i ) {
        first_size[i] = _database->nbrModalities ( db_pair_ids[i].first );
        total_size[i] = first_size[i] *
          _database->nbrModalities ( db_pair_ids[i].second );
      }
      ...
      for ( const ListBucket<CountingTreeTargetSetBox*>* iter =
        ...
        for ( unsigned int i = 0; i < db_pair_ids.size (); ++i ) {
          ...
          // parse the records
          for ( unsigned int j = 0; j < first_size.size (); ++j ) {
            // get the number Nij of occurences of the parents
            const float Nij = single_records[j];

            for ( unsigned int k = j; k < total_size.size ();
                  k += first_size[j] ) {
              score[i] += pair_records[k] * log ( pair_records[k] / Nij );
            }
          }
        }
      }
