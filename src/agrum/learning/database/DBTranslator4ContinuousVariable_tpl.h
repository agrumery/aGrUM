  ...
  namespace learning {
    ...
    DBTranslator4ContinuousVariable<ALLOC>::DBTranslator4ContinuousVariable (
      ...
      : DBTranslator<ALLOC> ( DBTranslatedValueType::CONTINUOUS,
                              missing_symbols, fit_range, 1, alloc )
      , __variable ( "var", "" )
    ...
    template <template<typename> class ALLOC>
    DBTranslatedValue
    DBTranslator4ContinuousVariable<ALLOC>::translate (
      ...
      if ( number < lower_bound ) {
        ...
        for ( auto iter = __status_float_missing_symbols.beginSafe ();
              iter != __status_float_missing_symbols.endSafe (); ++iter ) {
          if ( iter.val() == false ) {
