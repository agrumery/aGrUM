...
namespace gum {
  ...
  namespace learning {
    ...
    INLINE FilteredRowGeneratorSet< Generator >::FilteredRowGeneratorSet(
      ...
      if (size) __last_generator = *(__generators.rbegin());
    }
    ...
    operator=(const FilteredRowGeneratorSet< Generator >& from) {
      if (this != &from) {
    ...
    INLINE FilteredRowGeneratorSet< Generator >&
           FilteredRowGeneratorSet< Generator >::
    operator=(FilteredRowGeneratorSet< Generator >&& from) {
      if (this != &from) {
        __generators = std::move(from.__generators);
        __last_generator = from.__last_generator;
        from.__last_generator = nullptr;
    ...
    template < typename Generator >
    INLINE Size FilteredRowGeneratorSet< Generator >::nbGenerators() const
      noexcept {
    ...
    /// returns true if there are still rows that can be output by the RowFilter
    template < typename Generator >
    INLINE bool FilteredRowGeneratorSet< Generator >::hasRows() {
      return __hasRows(0);
    }
    ...
    template < typename Generator >
    FilteredRow& FilteredRowGeneratorSet< Generator >::__generate(std::size_t i) {
      auto gen = __generators[i];
      ...
      GUM_ERROR(UndefinedElement, "the generator cannot generate rows anymore");
    }


    /// generate new rows from the input row
    template < typename Generator >
    INLINE FilteredRow& FilteredRowGeneratorSet< Generator >::generate() {
      return __generate(0);
    }
