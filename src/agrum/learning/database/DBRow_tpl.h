...
namespace gum {
  ...
  namespace learning {
    ...
    INLINE ALLOC<T_DATA>
    DBRow<T_DATA,ALLOC>::getAllocator () const {
    ...
    DBRow<T_DATA,ALLOC>::row() const noexcept { return _row; }

    /// returns the current row
    template <typename T_DATA, template<typename> class ALLOC>
    INLINE std::vector<T_DATA,ALLOC<T_DATA>>&
    DBRow<T_DATA,ALLOC>::row() noexcept { return _row; }
    ...
    /// returns the weight
    template <typename T_DATA, template<typename> class ALLOC>
    INLINE double& DBRow<T_DATA,ALLOC>::weight() noexcept { return _weight; }
