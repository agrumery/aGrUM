...
namespace gum {
  ...
  namespace learning {
    ...
    DBRowGeneratorSet<ALLOC>::clone () const {
    ...
    DBRowGeneratorSet<ALLOC>::operator=(const DBRowGeneratorSet<ALLOC>& from) {
      if (this != &from) {
        ...
        std::vector<int,ALLOC<int>>
          setInputDBrow_performed ( from.__nb_generators, 0, getAllocator () );
    ...
    void DBRowGeneratorSet<ALLOC>::insertGenerator(
      ...
      ALLOC<DBRowGenerator<ALLOC>> allocator ( this->getAllocator () );
      __generators.insert ( __generators.begin () + i,
                            generator.clone ( allocator ) );
    ...
    INLINE void DBRowGeneratorSet<ALLOC>::reset() {
      for (auto gen : __generators) gen->reset();
      for ( auto& performed : __setInputRow_performed ) performed = 0;
      __output_row = nullptr;
    }
