...
namespace gum {
  ...
  namespace learning {
    ...
    /// destructor
    template < typename Translator >
    INLINE DBRowTranslatorSet< Translator >::~DBRowTranslatorSet() noexcept {
    ...
    DBRowTranslatorSet< Translator >::insertTranslator(
      const NewTranslator& translator, Idx deb_cols, Size nb_times, Idx incr) {
      // fix for a MVSC bug: by default, we wish the increment to be the
      // number of columns read by the translator
    ...
    template < typename Translator >
    INLINE bool DBRowTranslatorSet< Translator >::requiresInitialization() const
      noexcept {
      for (auto translator : __translators) {
    ...
    template < typename Translator >
    INLINE void
    DBRowTranslatorSet< Translator >::setInputRow(const DBRow& row) noexcept {
      for (auto translator : __translators) {
    ...
    template < typename Translator >
    INLINE FilteredRow& DBRowTranslatorSet< Translator >::outputRow() noexcept {
      return __output_row;
    }
    ...
    template < typename Translator >
    std::string
    DBRowTranslatorSet< Translator >::translateBack(Idx col,
    ...
    template < typename Translator >
    INLINE Size DBRowTranslatorSet< Translator >::outputSize() const noexcept {
    ...
    template < typename Translator >
    INLINE Size DBRowTranslatorSet< Translator >::nbTranslators() const noexcept {
