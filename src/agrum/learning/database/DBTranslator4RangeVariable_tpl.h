...
namespace gum {
  ...
  namespace learning {
    ...
    DBTranslator4RangeVariable<ALLOC>::DBTranslator4RangeVariable (
        ...
        DBTranslator4RangeVariable<ALLOC>::allocator_type& alloc )
      : DBTranslator<ALLOC> ( DBTranslatedValueType::DISCRETE,
                              missing_symbols, editable_dictionary,
                              max_dico_entries, alloc )
      , __variable ( var ) {
      ...
      if ( lower_bound <= upper_bound ) {
        for ( auto iter = this->_missing_symbols.beginSafe ();
              iter != this->_missing_symbols.endSafe (); ++iter ) {
          if ( DBCell::isInteger ( *iter ) ) {
            const long missing_val = std::stol ( *iter );
            if ( ( missing_val >= lower_bound ) &&
                 ( missing_val <= upper_bound ) ) {
    ...
    DBTranslator4RangeVariable<ALLOC>::clone (
      const typename
      DBTranslator4RangeVariable<ALLOC>::allocator_type& alloc ) const {
      ALLOC<DBTranslator4RangeVariable<ALLOC>>
        allocator ( alloc );
      DBTranslator4RangeVariable<ALLOC>*
        translator = allocator.allocate(1);
      try {
    ...
    template <template<typename> class ALLOC>
    DBTranslatedValue
    DBTranslator4RangeVariable<ALLOC>::translate (
      ...
      try {
        ...
        if ( __translated_int_missing_symbols.exists ( new_value ) ) {
          GUM_ERROR ( OperationNotAllowed, "the string corresponds to an "
        ...
        if ( new_value < __variable.minVal () ) {
          if ( std::size_t ( upper_bound - new_value + 1 ) >
               this->_max_dico_entries )
            GUM_ERROR ( SizeError, "the dictionary is full" );
          ...
          for ( auto iter = __status_int_missing_symbols.beginSafe ();
                iter != __status_int_missing_symbols.endSafe (); ++iter ) {
            if ( iter.val() == false ) {
          ...
          if ( std::size_t ( new_value - lower_bound + 1 ) >
               this->_max_dico_entries )
            GUM_ERROR ( SizeError, "the dictionary is full" );
    ...
    template <template<typename> class ALLOC>
    INLINE std::string
    DBTranslator4RangeVariable<ALLOC>::translateBack (
      ...
      try {
        ...
        if ( translated_val.discr_val ==
          ...
          if ( ! __nonint_missing_symbol.empty () )
    ...
    template <template<typename> class ALLOC>
    INLINE std::size_t
    DBTranslator4RangeVariable<ALLOC>::domainSize () const {
