...
namespace gum {
  ...
  namespace learning {
    ...
    template <typename IdSetAlloc, typename CountAlloc>
    bool ScoreLog2Likelihood<IdSetAlloc,CountAlloc>::isAprioriCompatible () const {
      ...
      if ( ! this->_apriori->isOfType ( AprioriDirichletType::type ) &&
        ...
        return false;
        // GUM_ERROR ( InvalidArgument, "The apriori is incompatible with the "
        //             "BIC score: shall be Dirichlet, smoothing or no apriori" );
      }
      else {
        return true;
      }
