...
#include <agrum/core/timer.h>
 namespace gum {
   ...
   // ##########################################################################
	...
	INLINE
	CorrectedMutualInformation<IdSetAlloc, CountAlloc>::CorrectedMutualInformation(
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE CorrectedMutualInformation<IdSetAlloc, CountAlloc>::~CorrectedMutualInformation() {
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE void
     CorrectedMutualInformation<IdSetAlloc, CountAlloc>::useCache( bool on_off ) noexcept {
       if ( !on_off ) clear();
       __use_cache = on_off;
     }
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE void
	 CorrectedMutualInformation<IdSetAlloc, CountAlloc>::useKCache( bool on_off ) noexcept {
       if ( !on_off ) clear();
       __use_Kcache = on_off;
     }
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE void CorrectedMutualInformation<IdSetAlloc, CountAlloc>::clearCache() {
       clear();
       __cache_2pt.clear();
       __cache_3pt.clear();
       __cache_K2pt.clear();
     ...
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::score(
							   ...
							   const std::vector<Idx>&      conditioning_ids ) {
    	 return this->_score( std::get<0>(vars), std::get<1>(vars),
							  std::get<2>(vars), conditioning_ids );
     }
     ...
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::score(
    		 	 	 	 	   const std::tuple<Idx, Idx, Idx>&   vars ) {
         return this->_score( std::get<0>(vars), std::get<1>(vars),
							  std::get<2>(vars), __empty_conditioning_set);
     }
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_score( Idx var1,
    		 	 	 	 	 	 	 	 	 	 	 	 	 	  Idx var2,
								 const std::vector<Idx>& conditioning_ids ) {
       /*std::cout << "    Recherche I de " << var1 << var2 << conditioning_ids << std::endl;*/
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_I_score( Idx var1,
    		 	 	 	 	 	 	 	 	 	 	 	 	 	  Idx var2,
								 const std::vector<Idx>& conditioning_ids ) {
       /*
     ...
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_K(
       ...
       if ( this->__use_Kcache ) {
        ...
        /*std::cout << "K de " << var1 << var2 << conditioning_ids << " vaut " << score << std::endl;*/
       return score;
     }
     ...
     double CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_I_score( Idx var1,
       ...
       if ( !conditioning_ids.empty() ) {
         ...
         if ( __use_cache ) {
           try {
             double score = __cache_3pt.score( var1, var2, var3,
         ...
         if ( this->_isUsingCache() ) {
           this->_insertIntoCache( var1, var2, var3, __empty_conditioning_set, score );
         }
         return score;
       }
     }
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE void
     CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_insertIntoCache( Idx var1,
    		 	 	 	 	 	 	 	 	 	 	 	 	 	 	  Idx var2,
									  const std::vector<Idx>& conditioning_ids,
                                                                 double score ) {
    	 /*
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE void
     CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_insertIntoKCache( Idx var1,
    		 	 	 	 	 	 	 	 	 	 	 	 	 	 	  Idx var2,
									  const std::vector<Idx>& conditioning_ids,
                                                                 double score ) {
       if ( !conditioning_ids.empty() ) {
	 ...
	 CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_insertIntoKCache( Idx var1,
       ...
       if ( !conditioning_ids.empty() ) {
         ...
         try {
           __cache_K3pt.insert( var1, var2, var3,
        		   this->__empty_conditioning_set, score );
         } catch ( const gum::DuplicateElement& ) {
         }
       }
     }
     ...
     template <typename IdSetAlloc, typename CountAlloc>
     INLINE bool CorrectedMutualInformation<IdSetAlloc, CountAlloc>::_isUsingCache() const
         noexcept {
       return __use_cache;
     }
