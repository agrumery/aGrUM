...
namespace gum {
  ...
  namespace learning {
    ...
    /// copy operator
    template < template < typename > class ALLOC >
    INLINE typename ScorefNML2< ALLOC >::CTable&
    ScorefNML2< ALLOC >::CTable::operator=(
      const typename ScorefNML2< ALLOC >::CTable& from) {
      return *this;
    }
    ...
    double ScorefNML2< ALLOC >::CTable::log2Cnr ( const std::size_t r,
      ...
      if ( n < ScorefNMLCTableNSize ) {
        ...
        double log2Cnr1 = 0.5 * std::log2 (n) + __cst1 +
          __cst2 / std::sqrt(n) + __cst3 / n;
    ...
    /// move operator
    template < template < typename > class ALLOC >
    ScorefNML2< ALLOC >& ScorefNML2< ALLOC >::operator=(ScorefNML2< ALLOC >&& from) {
      if (this != &from) {
    ...
    INLINE const Apriori2< ALLOC >&
                 ScorefNML2< ALLOC >::internalApriori() const {
