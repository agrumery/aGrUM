...
namespace gum {
  ...
  namespace learning {
    ...
    template <typename Alloc> INLINE
    unsigned int IdSet<Alloc>::operator[] ( unsigned int index ) const noexcept {
      return __ids[index];
    }
    ...
    template <typename Alloc> INLINE
    IdSet<Alloc>& IdSet<Alloc>::operator<< ( unsigned int id ) {
      ...
      while ( max_index > min_index ) {
        ...
        if ( __ids[middle] < id ) min_index = middle + 1;
        else max_index = middle;
      }
    ...
    template <typename Alloc> INLINE
    const std::vector<unsigned int,Alloc>& IdSet<Alloc>::ids () const noexcept {
      return __ids;
    }
    ...
    IdSet<Alloc>::isSubset ( const IdSet<OtherAlloc>& otherset ) const noexcept {
      ...
      for ( unsigned int i = 0, size = __ids.size (); i < size; ++i ) {
        ...
        while ( tmp_max_index > min_index ) {
          ...
          if ( otherset[middle] < nb ) min_index = middle + 1;
          else tmp_max_index = middle;
        }
  ...
  template <typename Alloc>
  Size HashFunc< learning::IdSet<Alloc> >::operator()
