...
namespace gum {
  ...
  // Default constructor.
  // Private.
  // You have to call MultiDimFunctionGraph< GUM_SCALAR, TerminalNodePolicy
  // >::manager() to get the instance
  ...
  // @throw OperationNotAllowed if MultiDimFunctionGraph has no this variable
  // yet.
  ...
  MultiDimFunctionGraphManager<GUM_SCALAR, TerminalNodePolicy>::setSon(
    ...
    // Check if associated modality is lower than node bound variable domain size
    if(__functionGraph->isInternalNode(node) && modality > __functionGraph->__internalNodeMap[node]->nodeVar()->domainSize()-1)
        GUM_ERROR(InvalidArgument, "Modality " << modality << "is higher than domain size " <<  __functionGraph->__internalNodeMap[node]->nodeVar()->domainSize()
                  << "minus 1 of variable " <<  __functionGraph->__internalNodeMap[node]->nodeVar()->name())

    // Check if variable order is respected
    if(__functionGraph->isInternalNode(sonNode) && __functionGraph->variablesSequence().pos(__functionGraph->__internalNodeMap[node]->nodeVar()) >=
        __functionGraph->variablesSequence().pos(__functionGraph->__internalNodeMap[sonNode]->nodeVar()))
                GUM_ERROR(OperationNotAllowed, "Variable " << __functionGraph->__internalNodeMap[node]->nodeVar() << " is after variable " << __functionGraph->__internalNodeMap[sonNode]->nodeVar() << "in Function Graph order.")
