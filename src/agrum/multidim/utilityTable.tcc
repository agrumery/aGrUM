...
namespace gum {
  ...
  template <typename GUM_SCALAR>
  INLINE void
  UtilityTable<GUM_SCALAR>::sum( const UtilityTable<GUM_SCALAR>& p1,
  ...
  template <typename GUM_SCALAR>
  void
  UtilityTable<GUM_SCALAR>::sumBy( const UtilityTable<GUM_SCALAR>& toAdd ) {
    UtilityTable<GUM_SCALAR> tab(
        static_cast<MultiDimImplementation<GUM_SCALAR>*>(
    ...
    if ( !utilitiesList.empty() ) {
      ...
      if ( partialInst.nbrDim() != globalInst.nbrDim() ) {
        ...
        for ( globalInst.setFirstIn( partialInst ), partialInst.setFirst();
              !partialInst.end();
              globalInst.incIn( partialInst ), ++partialInst ) {
