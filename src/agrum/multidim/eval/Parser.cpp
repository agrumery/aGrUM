...
void Parser::expression() {
		if (la->kind == _integer) {
			Get();
			__number((double)coco_atoi(t->val)); 
		} else if (la->kind == _float) {
			Get();
			__number((double)coco_atof(t->val)); 
		} else if (la->kind == _operator) {
			Get();
			__operator(narrow(t->val)[0]); 
		} else if (la->kind == 6 /* "(" */) {
			Get();
			__leftParenthesis(); 
		} else if (la->kind == 7 /* ")" */) {
...
void Parser::SynErr( const std::wstring& filename,int line, int col, int n ) {
  ...
  switch ( n ) {
			...
			case 9: s = coco_string_create(L"invalid expression"); break;
