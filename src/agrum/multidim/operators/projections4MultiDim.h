...
namespace gum {
   ...
   * defined over A,B,C,D and if del_vars = {A,C}, then the function returns
   * a table over B,D defined as Max_{A,C} table (A,B,C,D).
   * @return function ProjectMaxMultiDimDecisionDiagram allocates on the heap
   * the
  ...
  template <typename GUM_SCALAR>
  MultiDimImplementation<GUM_SCALAR>* projectMaxMultiDimDecisionDiagram(
      const MultiDimImplementation<GUM_SCALAR>* table,
      const Set<const DiscreteVariable*>& del_vars);
   ...
   * defined over A,B,C,D and if del_vars = {A,C}, then the function returns
   * a table over B,D defined as Min_{A,C} table (A,B,C,D).
   * @return function ProjectMaxMultiDimDecisionDiagrams allocates on the heap
   * the
   * new T multiDimDecisionDiagram and returns it
   * @warning In practice, do not use this function but rather operator/function
   * projectMin on multiDimDecorators. This operator will take care to select
  ...
  template <typename GUM_SCALAR>
  MultiDimImplementation<GUM_SCALAR>* projectMinMultiDimDecisionDiagram(
      const MultiDimImplementation<GUM_SCALAR>* table,
      const Set<const DiscreteVariable*>& del_vars);
  ...
  template <typename GUM_SCALAR>
  MultiDimImplementation<GUM_SCALAR>* projectSumMultiDimDecisionDiagram(
      const MultiDimImplementation<GUM_SCALAR>* table,
      const Set<const DiscreteVariable*>& del_vars);

  /// a specialized function for projecting a multiDimDecisionDiagram using
  /// products
  /** The function projects a table over the subset of its variables NOT IN
   * the set of variables del_vars. For instance, if table is a
   * MultiDimDecisionDiagram
   ...
   * implementations
   * of the projections for multidims depending in whether these multidim
   * contain
