...
namespace gum {
  ...
  namespace aggregator {
    ...
    *
    * Note that a <tt>Or(i)</tt> aggregator should have a binary aggregator variable since only 0 and 1 indexes are adressed...
    */
    /* ============================================================================ */

    template<typename T_DATA> class Or : public MultiDimAggregator<T_DATA> {
      public:
        Or(  );
        Or( const Or<T_DATA>& from );
        virtual ~Or();
