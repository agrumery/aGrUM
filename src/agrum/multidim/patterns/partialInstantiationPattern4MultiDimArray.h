...
#define GUM_MULTI_DIM_PARTIAL_INSTANTIATION_POINTER
        ...
        GUM_MULTI_DIM_PARTIAL_INSTANTIATION_POINTER_IMPL2ARRAY_NAME(
          ...
          // Compute as well the offsets and the domain size of the variables
          // that
          // belong to result. Finally, compute has_before_incr: this is a
          // Boolean
          // indicating whether the instantiated variables are the last
          // variables in
          // the variables sequence of table (true) or not (false). If this
          // Boolean
          // is
          // true, then we can fill result by parsing both table and result
          // using
          // only
          // 1-increments.
          ...
          // before we must perform a "major" increment: for instance, let A and
          // B be
          // two variables of size 10. Then, if table_and_result_value[A] = 3
          // and
          // table_and_result_value[B] = 2, this means that the offset they
          // represent
          // is
          // 78 (10^2 - 32). If we still increment B twice, then the offset
          // should be
          // 80, which means that we shall increment A once and decrease B by
          // 10.
          ...
          // appear
          // in "table". Hence, ++ operations on an instantiation on table will
          // more
          // or
          // less correspond to a ++ operation on an instantiation on result
          ...
          // that
          // were not instantiated. ptable and presult are pointers on the
          // arrays
          ...
          // but before doing so, check whether the instantiated variables are
          // the
      ...
      *presult = *ptable;
            ...
            // ones
            // in
            // the variables sequence of table. So, we must perform a more
            // complicated
            // parsing of ptable
            Idx table_offset = 0;
