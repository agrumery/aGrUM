        ...
        GUM_MULTI_DIM_OPERATOR_POINTER_NAME_F ( const MultiDimImplementation<T*>* t1,
          ...
          if ( t1_and_t2_var.size() ) {
            ...
            if ( nb_t1_t2_vars ==  t1_and_t2_var.size() ) {
              ...
              for ( auto iter = t2_vars.begin(); nb_t1_t2_vars !=  t1_and_t2_var.size();++iter, ++nb_t1_t2_vars )
          ...
          // increment a variable, its corresponding txxx_value is decreased by 1. When
          // the latter is equal to 0, this means that the variable itself should be
          // reinitialized to 0 as well and that the next variable of the table should be
