...
GUM_MULTI_DIM_DECISION_DIAGRAM_RECUR_FUNCTION( NonOrderedOperatorData<T>& opData,
    ...
    if ( opData.DD1->isTerminalNode( opData.conti.getDD1Node() ) && opData.DD2->isTerminalNode( opData.conti.getDD2Node() ) ) {
        ...
        newNode = opData.factory->addTerminalNode( GUM_MULTI_DIM_OPERATOR( opData.DD1->unsafeNodeValue( opData.conti.getDD1Node() ), opData.DD2->unsafeNodeValue( opData.conti.getDD2Node() ) ) );
    ...
    if ( !opData.DD2->isTerminalNode( opData.conti.getDD2Node() ) )
        if ( opData.retrogradeVarTable->exists( opData.conti.getDD2Node() ) ) {
            ...
            if ( !opData.DD1->isTerminalNode( opData.conti.getDD1Node() ) )
                indexFin = opData.factory->variablesSequence().pos( opData.DD1->unsafeNodeVariable( opData.conti.getDD1Node() ) );
    ...
    if ( !opData.DD1->isTerminalNode( opData.conti.getDD1Node() ) && !opData.DD2->isTerminalNode( opData.conti.getDD2Node() ) ) {
        ...
        if ( opData.factory->variablesSequence().exists( opData.DD1->unsafeNodeVariable( opData.conti.getDD1Node() ) ) )
            indexLeader = opData.factory->variablesSequence().pos(  opData.DD1->unsafeNodeVariable( opData.conti.getDD1Node() ) );
        if ( opData.factory->variablesSequence().exists( opData.DD2->unsafeNodeVariable( opData.conti.getDD2Node() ) ) )
            indexFollower = opData.factory->variablesSequence().pos(  opData.DD2->unsafeNodeVariable( opData.conti.getDD2Node() ) );
        ...
        if ( indexLeader == indexFollower ) {
            ...
            const std::vector< NodeId >* leaderSonsMap = opData.DD1->unsafeNodeSons( opData.conti.getDD1Node() );
            const std::vector< NodeId >* followerSonsMap = opData.DD2->unsafeNodeSons( opData.conti.getDD2Node() );
            ...
            HashTable< NodeId, Idx > nodeCount( sonsIds.size(), false, false);
            Idx nbUnexploredModalities = opData.DD1->unsafeNodeVariable( leaderCurrentNode )->domainSize();

            for ( Idx modality = 0; modality < opData.DD1->unsafeNodeVariable( leaderCurrentNode )->domainSize(); modality++ ) {
                ...
                if ( (*followerSonsMap)[ modality ] == 0 )
                    opData.conti.setDD2Node( opData.DD2->unsafeNodeDefaultSon( followerCurrentNode ) );
                else
                    opData.conti.setDD2Node( (*followerSonsMap)[ modality ] );

                sonsIds[ modality ] = GUM_MULTI_DIM_DECISION_DIAGRAM_RECUR_FUNCTION( opData, opData.DD1->unsafeNodeVariable( leaderCurrentNode ), tabu  );
            ...
            NodeId defaultSon = 0;
            if ( opData.DD1->unsafeHasNodeDefaultSon( leaderCurrentNode ) && opData.DD2->unsafeHasNodeDefaultSon( followerCurrentNode ) ) {
                opData.conti.setDD1Node( opData.DD1->unsafeNodeDefaultSon( leaderCurrentNode ) );
                opData.conti.setDD2Node( opData.DD2->unsafeNodeDefaultSon( followerCurrentNode ) );
                defaultSon = GUM_MULTI_DIM_DECISION_DIAGRAM_RECUR_FUNCTION( opData, opData.DD1->unsafeNodeVariable( leaderCurrentNode ), tabu  );
            ...
            newNode = insertNonTerminalNode( opData, opData.DD1->unsafeNodeVariable( leaderCurrentNode ), sonsIds, defaultSon, nodeCount );
...
GUM_MULTI_DIM_DECISION_DIAGRAM_GO_DOWN_ON_LEADER_FUNCTION( NonOrderedOperatorData<T>& opData,
    ...
    NodeId leaderCurrentNode = opData.conti.getDD1Node();
    const DiscreteVariable* leaderCurrentVar = opData.DD1->unsafeNodeVariable( leaderCurrentNode );
    bool isIndeedRetorgrade = opData.conti.isRetrogradeVar( leaderCurrentVar );
    const std::vector< NodeId >* sonsMap = opData.DD1->unsafeNodeSons( leaderCurrentNode );
...
GUM_MULTI_DIM_DECISION_DIAGRAM_GO_DOWN_ON_FOLLOWER_FUNCTION( NonOrderedOperatorData<T>& opData,
    ...
    NodeId followerCurrentNode = opData.conti.getDD2Node();
    const DiscreteVariable* followerCurrentVar = opData.DD2->unsafeNodeVariable( followerCurrentNode );
    bool isIndeedRetorgrade = opData.conti.isRetrogradeVar( followerCurrentVar );
    const std::vector< NodeId >* sonsMap = opData.DD2->unsafeNodeSons( followerCurrentNode );
