...
/// a specialized function for combining two MultiDimDecisionGraphs
...
#define GUM_MULTI_DIM_OPERATOR_TYPE GUM_SCALAR
  ...
  GUM_MULTI_DIM_OPERATOR_NAME( const MultiDimDecisionGraph< GUM_SCALAR >* t1,
...
#define GUM_MULTI_DIM_OPERATOR_TYPE GUM_SCALAR
      ...
      GUM_MULTI_DIM_OPERATOR_IMPL2DECISION_GRAPH_NAME( const MultiDimImplementation< GUM_SCALAR >* tt1,
        ...
        const MultiDimDecisionGraph< GUM_SCALAR >* t1 = reinterpret_cast<const MultiDimDecisionGraph< GUM_SCALAR >*>( tt1 );
        const MultiDimDecisionGraph< GUM_SCALAR >* t2 = reinterpret_cast<const MultiDimDecisionGraph< GUM_SCALAR >*>( tt2 );
#endif

        MultiDimDecisionGraph< GUM_MULTI_DIM_OPERATOR_TYPE >* ret = nullptr;

        if ( ( t1 == nullptr || t1->empty() ) && ( t2 == nullptr || t2->empty() ) )
          return ret;

        if ( t1 == nullptr || t1->empty() ) {
          ret = reinterpret_cast<MultiDimDecisionGraph< GUM_SCALAR >*>( t2->newFactory() );
          ret->copy( *t2 );
          return ret;
        }

        if ( t2 == nullptr || t2->empty() ) {
          ret = reinterpret_cast<MultiDimDecisionGraph< GUM_SCALAR >*>( t1->newFactory() );
          ret->copy( *t1 );
          return ret;
        }
...
//     std::cout << "Début opération" << std::endl;
        DGNonOrderedOperatorData< GUM_SCALAR > opData;
        opData.initialize( t1, t2 );

//#ifdef O4DDWITHORDER
//        GUM_MULTI_DIM_DECISION_GRAPH_RECUR_FUNCTION( opData, "" );
//#else
        opData.DG3->manager()->setRootNode( GUM_MULTI_DIM_DECISION_GRAPH_RECUR_FUNCTION( opData, nullptr, "" ) );
//#endif
