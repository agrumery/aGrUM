...
namespace gum {
  ...
  MultiDimLogit<GUM_SCALAR>::MultiDimLogit ( GUM_SCALAR external_weight, GUM_SCALAR default_weight ) :
...
/// Default constructor
  ...
  template<typename GUM_SCALAR> INLINE
  MultiDimLogit<GUM_SCALAR>::MultiDimLogit ( const MultiDimLogit<GUM_SCALAR>& from ) : MultiDimICIModel<GUM_SCALAR> ( from ) {
  ...
  template<typename GUM_SCALAR> INLINE
  MultiDimLogit<GUM_SCALAR>::MultiDimLogit ( const Bijection<const DiscreteVariable*, const DiscreteVariable*>& bij,
...
/// destructor
  ...
  const std::string MultiDimLogit<GUM_SCALAR>::toString() const {
    std::stringstream s;
    s << MultiDimImplementation<GUM_SCALAR>::variable ( 0 ) << "=logit(" << this->externalWeight();

    for ( Idx i = 1; i < MultiDimImplementation<GUM_SCALAR>::nbrDim(); i++ ) {
      GUM_SCALAR c=this->causalWeight ( MultiDimImplementation<GUM_SCALAR>::variable ( i ) );
      ...
      if ( c != GUM_SCALAR ( 0 ) ) {
        ...
        s<<this->causalWeight ( MultiDimImplementation<GUM_SCALAR>::variable ( i ) ) << "*" << MultiDimImplementation<GUM_SCALAR>::variable ( i );
      }
    }
...
// For friendly displaying the content of the variable.
  ...
  MultiDimContainer<GUM_SCALAR>* MultiDimLogit<GUM_SCALAR>::newFactory() const {
    return new MultiDimLogit<GUM_SCALAR> ( this->__external_weight, this->__default_weight );
