...
#  ifdef GUM_NO_INLINE
#    include <agrum/base/database/DBTranslator4RangeVariable_inl.h>
#  endif /* GUM_NO_INLINE */
    ...
    DBTranslatedValue DBTranslator4RangeVariable::translate(const std::string& str) {
      ...
      try {
        ...
        if (this->isMissingSymbol(str)) {
          auto* ptr_is_str_translated = _status_int_missing_symbols_.tryGet(str);
          if (ptr_is_str_translated != nullptr) {
        ...
        if (!this->hasEditableDictionary()) {
          GUM_ERROR(UnknownLabelInDatabase,
                    "The translation of String \"" << str << "\" could not be found for variable '"
                                                   << _variable_ << "'.");
        }
