...
namespace gum::learning {
   ...
   * @headerfile databaseTable.h <agrum/base/database/databaseTable.h>
  ...
  class DatabaseTable final: public IDatabaseTable< DBTranslatedValue > {
    ...
    /// default constructor
    explicit DatabaseTable(const MissingValType&  missing_symbols,
                           const DBTranslatorSet& translators = DBTranslatorSet());

    /// default constructor
    explicit DatabaseTable(const DBTranslatorSet& translators = DBTranslatorSet());
     ...
     * input database.
     * @warning if the translator does not exist, nothing is done. In
     ...
     * symbols of the translator now translating the kth column
     ...
     * particular, no exception is raised.
     */
    auto changeTranslator(const Variable&                   var,
                          std::size_t                       k,
    ...
    void setVariableNames(const std::vector< std::string >& names,
                          const bool                        from_external_object = true) override;
     ...
     */
    void insertRow(Row< DBTranslatedValue >&& new_row,
                   const IsMissing            contains_missing_data) override;
     ...
     * not belong to the domain of its corresponding translator.
     */
    void insertRow(const Row< DBTranslatedValue >& new_row,
                   const IsMissing                 contains_missing_data) override;
     ...
     * not belong to the domain of its corresponding translator.
     */
    void insertRows(Matrix< DBTranslatedValue >&& new_rows,
                    const DBVector< IsMissing >&  rows_have_missing_vals) override;
     ...
     * @InvalidArgument is raised if at least one element of new_row does
     * not belong to the domain of its corresponding translator.*/
    void insertRows(const Matrix< DBTranslatedValue >& new_rows,
                    const DBVector< IsMissing >&       rows_have_missing_vals) override;
