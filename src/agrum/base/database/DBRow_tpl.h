 ...
 */
#include <agrum/base/database/DBRow.h>   // to ease IDE parser
...
namespace gum {
  ...
  namespace learning {
    ...
    /// constructor with a given size for the row
    template < typename T_DATA >
    DBRow< T_DATA >::DBRow(const std::size_t size, const double weight) :
        DBRow(size, T_DATA(), weight) {}

    /// initializer list constructor
    template < typename T_DATA >
    DBRow< T_DATA >::DBRow(std::initializer_list< T_DATA > list, const double weight) :
    ...
    /// sets a new row
    template < typename T_DATA >
    void DBRow< T_DATA >::setRow(const std::vector< T_DATA >& new_row) {
    ...
    template < typename T_DATA >
    DBRow< T_DATA >::DBRow(const std::vector< T_DATA >& cells, const double weight) :
        weight_(weight) {
      setRow(cells);
      GUM_CONSTRUCTOR(DBRow);
    }
    ...
    /// initializer from a vector of cells
    template < typename T_DATA >
    DBRow< T_DATA >::DBRow(std::vector< T_DATA >&& cells, const double weight) :
    ...
    /// move constructor
    template < typename T_DATA >
    DBRow< T_DATA >::DBRow(DBRow< T_DATA >&& from) :
    ...
    template < typename T_DATA >
    DBRow< T_DATA >* DBRow< T_DATA >::clone() const {
    ...
    /// copy operator
    template < typename T_DATA >
    DBRow< T_DATA >& DBRow< T_DATA >::operator=(const DBRow< T_DATA >& from) {
      if (this != &from) {
        row_    = from.row_;
    ...
    /// move operator
    template < typename T_DATA >
    DBRow< T_DATA >& DBRow< T_DATA >::operator=(DBRow< T_DATA >&& from) {
      if (this != &from) {
    ...
    template < typename T_DATA >
    T_DATA& DBRow< T_DATA >::operator[](const std::size_t i) {
      return row_[i];
    }

    /// returns the ith T_DATA of the row
    template < typename T_DATA >
    const T_DATA& DBRow< T_DATA >::operator[](const std::size_t i) const {
      return row_[i];
    }
    ...
    /// returns the current row
    template < typename T_DATA >
    std::vector< T_DATA >& DBRow< T_DATA >::row() noexcept {
      return row_;
    }
    ...
    /// sets a new weight
    template < typename T_DATA >
    void DBRow< T_DATA >::setWeight(const double new_weight) {
      weight_ = new_weight;
    }

    /// returns the size of the row
    template < typename T_DATA >
    std::size_t DBRow< T_DATA >::size() const noexcept {
      return row_.size();
    }

    /// resize a given row
    template < typename T_DATA >
    void DBRow< T_DATA >::resize(const std::size_t new_size) {
      row_.resize(new_size);
    }

    /// reserve a size for the elements of a given row
    template < typename T_DATA >
    void DBRow< T_DATA >::reserve(const std::size_t new_size) {
      row_.reserve(new_size);
    }

    /// adds a new element at the end of the row
    template < typename T_DATA >
    void DBRow< T_DATA >::pushBack(const T_DATA& elt) {
      row_.push_back(elt);
    }
