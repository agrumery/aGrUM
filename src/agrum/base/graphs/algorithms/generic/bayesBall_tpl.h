...
namespace gum::graph {
  ...
  NodeSet
      requisiteNodes(const G& g, const NodeSet& query, const NodeSet& Zhard, const NodeSet& Zsoft) {
    return _bayesBall_< G, false >(g, query, Zhard, Zsoft);
  }

  template < GUM_DiGraphable G >
  NodeSet dConnected(const G& g, const NodeSet& query, const NodeSet& Zhard, const NodeSet& Zsoft) {
