...
// ensure binary read on windows
    ...
    bool Buffer::CanSeek() { return (stream != nullptr) && (ftell(stream) != -1); }
    ...
    void Scanner::NextCh() {
      if (oldEols > 0) {
        ...
        if (ch == EOL) { /*if (trace__) std::cout<<line<<std::endl;*/
          line++;
          col = 0;
        }
      }
    ...
    Token* Scanner::CreateToken() {
      ...
      if (((char*)heapTop + (int)sizeof(Token)) >= (char*)heapEnd) { CreateHeapBlock(); }
    ...
    Token* Scanner::NextToken() {
      ...
      switch (state) {
        ...
        case_6:
          recEnd  = pos;
          recKind = 6;
          if (ch == L'.' || (ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || ch == L'_'
              || (ch >= L'a' && ch <= L'z')) {
            AddCh();
            goto case_6;
          } else {
    ...
    Token* Scanner::Peek() {
      do {
        if (pt->next == nullptr) { pt->next = NextToken(); }
