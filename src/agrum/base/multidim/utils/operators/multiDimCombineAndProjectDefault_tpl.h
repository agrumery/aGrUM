...
#include <agrum/base/multidim/utils/operators/multiDimCombineAndProjectDefault.h>   // to ease IDE parser
        ...
        for (const auto ptrVar: vars) {
          if (del_vars.contains(ptrVar)) {
            ...
            for (const auto xptrVar: vars) {
              if (auto ptr = comb_vars.tryGet(xptrVar)) ++(*ptr);
      ...
      for (const auto mvar: marginal_vars) {
        if (del_vars.contains(mvar)) {
          ...
          for (const auto var: marginal_vars) {
            if (auto ptr = iter_vars.tryGet(var)) {
              ++(*ptr);
            } else {
