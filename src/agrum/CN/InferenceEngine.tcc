...
namespace gum {
  ...
  void InferenceEngine< GUM_SCALAR >::insertEvidence(const std::string & path) {
    ...
    if(!evi_stream.good()) {
      throw(Errors(0, "void InferenceEngine< GUM_SCALAR >::insertEvidence(const std::string & path) : could not open input file : " + path, 0));
    }
  ...
  void InferenceEngine< GUM_SCALAR >::insertQuery(const std::string & path) {
    ...
    if(!evi_stream.good()) {
      throw(Errors(0, "void InferenceEngine< GUM_SCALAR >::insertQuery(const std::string & path) : could not open input file : " + path, 0));
    }
    ...
    while(evi_stream.good()) {
      ...
      if(p == NULL) {
        ...
        while(p != NULL) {
          //tmp = p;
          if(atoi(p) >= dSize)
            throw(Errors(0, "void InferenceEngine< GUM_SCALAR >::insertQuery(const std::string & path) : query modality is higher or equal to cardinality", 0));
  ...
  void InferenceEngine< GUM_SCALAR >::saveMarginals(const std::string & path) const {
    ...
    if(! m_stream.good()) {
      throw(Errors(0, "void InferenceEngine< GUM_SCALAR >::saveMarginals(const std::string & path) const : could not open outpul file : " + path, 0));
    }
  ...
  void InferenceEngine< GUM_SCALAR >::_repetitiveInit() {
    ...
    for(gum::DAG::NodeIterator id = dag.beginNodes(); id != dag.endNodes(); ++id) {
      ...
      if(delim > var_name.size()) {
        throw(Errors(0, "void InferenceEngine< GUM_SCALAR >::_repetitiveInit() : the network does not appear to be dynamic", 0));
      }
