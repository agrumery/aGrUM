...
/*
      ...
      */
        ...
        */
        ...
        if(fabs(1. - sum) > __epsRedund) {
          std::cout << __src_bn.variable(*node_idIt).name() << " " << entry << std::endl;
          std::cout << vertex << std::endl;
          std::cout << (ins-=var_dSize) << std::endl;

          throw(gum::Errors(0, "bnToCredal() : potential does not sum to 1", 0));
        }
        // end check sum is 1
    ...
    std::ofstream min_file("min.bif", std::ios::out | std::ios::trunc);
    std::ofstream max_file("max.bif", std::ios::out | std::ios::trunc);

    if(! min_file.good())
      throw(gum::Errors(0, "bnToCredal() : could not open stream : min_file : min.bif", 0));

    if(! max_file.good()) {
      min_file.close();
      throw(gum::Errors(0, "bnToCredal() : could not open stream : max_file : max.bif", 0));
    }
...
__farley(num,den,num2,__denMax);
  ...
  void CredalNet< GUM_SCALAR >::__H2V(const std::vector< std::vector< GUM_SCALAR > > & h_rep, std::vector< std::vector< GUM_SCALAR > > & v_rep, const bool rationals) const {
    // write H rep file
    int64_t num, den;
    std::ofstream h_file("lrs_h_tmp.ine", std::ios::out | std::ios::trunc);
    
    if(! h_file.good())
      throw(gum::Errors(0, "__H2V : could not open lrs input file : lrs_h_tmp.ine", 0));
    ...
    // read V rep file
    std::ifstream v_file("lrs_v_tmp.ext", std::ios::in);
    
    if(! v_file.good())
      throw(gum::Errors(0, "__H2V() : could not open lrs ouput file : lrs_v_tmp.ext", 0));
    ...
    while(v_file.good() && keep_going) {
      ...
      else if(line.compare("end") == 0)
      {
        ...
        throw(gum::Errors(0, "__H2V() : reading something other than a vertex from lrs output file : lrs_v_tmp.ext", 0));
      }
