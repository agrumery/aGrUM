...
namespace gum {
  namespace credal {
    namespace lp {
      ...
      LpExpr::LpExpr() :
           _ileft_(false),  _imiddle_(false),  _iright_(false),  _lValue_(0.),
           _mValue_(0.),  _rValue_(0.),  _lCoeffs_(new HashTable< LpCol, double >()),
      ...
      LpExpr::LpExpr(const LpExpr& expr) :
           _ileft_(expr. _ileft_),  _imiddle_(expr. _imiddle_),
           _iright_(expr. _iright_),  _lValue_(expr. _lValue_),
           _mValue_(expr. _mValue_),  _rValue_(expr. _rValue_),
      ...
      LpExpr::LpExpr(const LpExpr& expr,
                     ...
                     bool          copyRight) :
           _ileft_(false),
           _imiddle_(false),  _iright_(false),  _lValue_(0.),  _mValue_(0.),
           _rValue_(0.),  _lCoeffs_(nullptr),  _mCoeffs_(nullptr),
           _rCoeffs_(nullptr) {
        if (copyLeft) {
           _lCoeffs_ = new HashTable< LpCol, double >(*expr. _lCoeffs_);
      ...
      LpExpr::LpExpr(LpExpr&& expr) :
           ...
           _iright_(expr. _iright_),  _lValue_(expr. _lValue_),
           _mValue_(expr. _mValue_),  _rValue_(expr. _rValue_),
           _lCoeffs_(expr. _lCoeffs_),  _mCoeffs_(expr. _mCoeffs_),
           _rCoeffs_(expr. _rCoeffs_) {
      ...
      LpRow::LpRow(const LpExpr& expr, const std::vector< LpCol >& cols) :
        ...
        if (expr. _ileft_ && !expr. _iright_) {
          ...
          for (const auto& col: cols) {
            ...
            if (expr. _lCoeffs_->exists(col))
              col_coeff = expr. _lCoeffs_->operator[](col);
          ...
          for (const auto& col: cols) {
            ...
            if (expr. _mCoeffs_->exists(col))
              col_coeff = expr. _mCoeffs_->operator[](col);
      ...
      LpRow::LpRow(const LpRow& row) :
           _cste_(row. _cste_),
           _coeffs_(new HashTable< LpCol, double >(*row. _coeffs_)) {
        GUM_CONS_CPY(LpRow);
      }
