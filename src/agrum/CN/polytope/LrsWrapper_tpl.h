...
namespace gum {
  namespace credal {
    ...
    void LRSWrapper< GUM_SCALAR >::setUpH(const Size& card) {
      ...
      tearDown();
       ...
       _input_ = std::vector< std::vector< GUM_SCALAR > >(
         card * 2 + 2,
    ...
    void LRSWrapper< GUM_SCALAR >::computeVolume() {
      ...
      int64_t Nsize
         = ( _dat_->Nvolume[0] > 0) ?  _dat_->Nvolume[0] : - _dat_->Nvolume[0];
      int64_t Dsize
         = ( _dat_->Dvolume[0] > 0) ?  _dat_->Dvolume[0] : - _dat_->Dvolume[0];
    ...
    void LRSWrapper< GUM_SCALAR >::elimRedundVrep() {
      ...
      for (decltype(m + d) index = lastdv + 1, end = m + d; index <= end;
        ...
        auto ineq
           =  _dat_->inequality[index - lastdv]; /* the input inequality number
                                                   corr. to this index */
      ...
      */
      ...
      for (decltype(m) i = 1; i <= m; i++)
        if (redineq[i] == 0)
           _output_.push_back(
             std::vector< GUM_SCALAR >(++ _input_[std::size_t(i - 1)].begin(),
    ...
    template < typename GUM_SCALAR >
    void LRSWrapper< GUM_SCALAR >:: _getLRSWrapperOutput_(
       lrs_mp                  Nin,
       lrs_mp                  Din,
    ...
    void LRSWrapper< GUM_SCALAR >:: _initLrs_() {
      ...
      if ( _dat_->homogeneous &&  _dat_->hull) {
        ...
        if (! _dat_->restart) {
          ...
          for (decltype( _dat_->nredundcol) col = startcol; col <  _dat_->nredundcol;
               col++)
  ...
  */
    ...
    void LRSWrapper< GUM_SCALAR >:: _freeLrs_() {
      ...
      if ( _dat_->nredundcol > 0)
        lrs_clear_mp_matrix( _Lin_,  _dat_->nredundcol,  _dat_->n);
