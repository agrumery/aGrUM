...
namespace gum {
  ...
  template < typename GUM_SCALAR, class BNInferenceEngine >
  inline bool CNInferenceEngines< GUM_SCALAR, BNInferenceEngine >::_updateThread( const gum::NodeId & id, const std::vector< GUM_SCALAR > & vertex, const bool & elimRedund ) {
  ...
  template < typename GUM_SCALAR, class BNInferenceEngine >
  inline void CNInferenceEngines< GUM_SCALAR, BNInferenceEngine >::__updateThreadCredalSets( const gum::NodeId & id, const std::vector< GUM_SCALAR > & vertex, const bool & elimRedund ) {
		int tId = gum_threads::getThreadNumber();
      ...
      int threadId = gum_threads::getThreadNumber();
			...
			for ( decltype ( nsize ) i = 0; i < nsize; i++ ) {
				...
				for ( decltype ( tsize ) tId = 0; tId < tsize; tId++ ) {
					...
					for ( auto & vtx : nodeThreadCredalSet ) {
						/* this is done by _updateCredalSets from parent class !!
						bool opt = false;
						for ( decltype ( dSize ) j = 0; j < dSize; j++ ) {
							if ( fabs( vtx[ j ] - this->_marginalMin[ i ][ j ] ) < 1e-6 || fabs( vtx[ j ] - this->_marginalMax[ i ][ j ] ) < 1e-6 ) {
								opt = true;
								break;
							}
						} // end of : all modalities
						...
						if ( ! opt )
							continue; // next thread vertex, current one not at any opt
						*/
						
						// we run redundancy elimination at each step
						// because there could be 100000 threads and the set will be so huge ...
						// BUT not if vertices are of dimension 2 ! opt check and equality should be enough
						infE::_updateCredalSets( i, vtx, ( vtx.size() > 2 ) ? true : false );
  ...
  void CNInferenceEngines< GUM_SCALAR, BNInferenceEngine >::_expFusion() {
    ...
    if ( this->_modal.empty() )
      return;
			...
			#pragma omp parallel
			{
				...
				if ( ! this->_l_modal[threadId].empty() ) {
					...
					for ( decltype ( nsize ) i = 0; i < nsize; i++ ) {
						...
						auto setsize = infE::_marginalSets[ i ].size();
						for ( auto & vertex : infE::_marginalSets[ i ] ) {
							GUM_SCALAR exp = 0;
							auto vsize = vertex.size();
							for ( decltype( vsize ) mod = 0; mod < vsize; mod++ )
								exp += vertex[ mod ] * _l_modal[ threadId ][ var_name ][ mod ];
							
							if( exp > infE::_expectationMax[ i ] )
								infE::_expectationMax[ i ] = exp;
							if( exp < infE::_expectationMin[ i ] )
								infE::_expectationMin[ i ] = exp;
						}						
		...
		// if ! infE::_storeVertices we need to do a fusion of threads local expectations
