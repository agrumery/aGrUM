    ...
    inline bool MultipleInferenceEngine< GUM_SCALAR, BNInferenceEngine >::_updateThread ( const NodeId& id, const std::vector< GUM_SCALAR >& vertex, const bool& elimRedund ) {
      ...
      for ( Size  mod = 0; mod < vsize; mod++ ) {
        ...
        if ( vertex[mod] > _l_marginalMax[tId][id][mod] ) {
          ...
          if ( __infE::_storeBNOpt && vertex[mod] == _l_marginalMin[tId][id][mod] && ! __infE::_evidence.exists ( id ) ) {
            std::vector< unsigned int > key ( 3 );
            key[0] = id; key[1] = mod; key[2] = 0;
          ...
          if ( __infE::_storeBNOpt && vertex[mod] == _l_marginalMax[tId][id][mod] && ! __infE::_evidence.exists ( id ) ) {
            std::vector< unsigned int > key ( 3 );
            key[0] = id; key[1] = mod; key[2] = 1;
