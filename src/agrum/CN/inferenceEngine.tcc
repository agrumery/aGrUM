...
//  _marginalSets.clear();
    ...
    void InferenceEngine< GUM_SCALAR >::insertModals ( const std::map< std::string, std::vector< GUM_SCALAR > >& modals ) {
      ...
      for ( auto it = modals.cbegin(), theEnd = modals.cend(); it != theEnd; ++it ) {
        ...
        //GUM_ERROR(OperationNotAllowed, "void InferenceEngine< GUM_SCALAR >::insertModals( const std::map< std::string, std::vector< GUM_SCALAR > > &modals) : modalities does not respect variable cardinality : " << _credalNet->current_bn().variable( id ).name() << " : " << dSize << " != " << it->second.size());
    ...
    template< typename GUM_SCALAR >
    const std::vector< GUM_SCALAR >& InferenceEngine< GUM_SCALAR >::marginalMin ( const NodeId id ) const {
      try {
    ...
    template< typename GUM_SCALAR >
    const std::vector< GUM_SCALAR >& InferenceEngine< GUM_SCALAR >::marginalMax ( const NodeId id ) const {
      try {
    ...
    template< typename GUM_SCALAR >
    const std::vector< std::vector< GUM_SCALAR > >& InferenceEngine< GUM_SCALAR >::vertices ( const NodeId id ) const {
    ...
    void InferenceEngine< GUM_SCALAR >::saveMarginals ( const std::string& path ) const {
      ...
      for ( const auto &  elt : _marginalMin ) {
        ...
        for ( Size mod = 0; mod < esize; mod++ ) {
          m_stream << _credalNet->current_bn().variable ( elt.first ).name() << " " << mod << " " << ( elt.second ) [mod] << " " << _marginalMax[elt.first][mod] << std::endl;
        }
      }
    ...
    std::string InferenceEngine< GUM_SCALAR >::toString() const {
      ...
      for ( const auto & elt : _marginalMin ) {
        ...
        for ( Size mod = 0; mod < esize; mod++ ) {
          output << "P(" << _credalNet->current_bn().variable ( elt.first ).name() << "=" << mod << "|e) = [ ";
          output << _marginalMin[elt.first][mod] << ", " << _marginalMax[elt.first][mod] << " ]";
    ...
    void InferenceEngine< GUM_SCALAR >::_dynamicExpectations() {
      ...
      for ( const auto & elt : _expectationMin ) {
        ...
        expectationsMin.getWithDefault ( var_name, innerMap() ).getWithDefault ( atoi ( time_step.c_str() ), 0 ) = elt.second; // we iterate with min iterators
        expectationsMax.getWithDefault ( var_name, innerMap() ).getWithDefault ( atoi ( time_step.c_str() ), 0 ) = _expectationMax[ elt.first ];
      }
      ...
      for ( const auto & node : _credalNet->current_bn().dag().nodes() ) {
        ...
        if ( time_step.compare ( "1" ) == 0 ) {
          ...
          for ( const auto & elt : _t0 ) {
            std::string var_0_name =  _credalNet->current_bn().variable ( elt.first ).name();
            delim = var_0_name.find_first_of ( "_" );
            var_0_name = var_0_name.substr ( 0, delim );

            if ( var_name.compare ( var_0_name ) == 0 ) {
              const Potential< GUM_SCALAR >* potential ( &_credalNet->current_bn().cpt ( node ) );
              const Potential< GUM_SCALAR >* potential2 ( &_credalNet->current_bn().cpt ( elt.first ) );
    ...
    template< typename GUM_SCALAR >
    void InferenceEngine< GUM_SCALAR >::_updateExpectations ( const NodeId& id, const std::vector< GUM_SCALAR >& vertex ) {
    ...
    template< typename GUM_SCALAR >
    void InferenceEngine< GUM_SCALAR >::_updateCredalSetsWithFiles ( const NodeId& id, const std::vector< GUM_SCALAR >& vertex, const bool& elimRedund ) {
    ...
    template< typename GUM_SCALAR >
    void InferenceEngine< GUM_SCALAR >::_updateCredalSets ( const NodeId& id, const std::vector< GUM_SCALAR >& vertex, const bool& elimRedund ) {
      auto& nodeCredalSet = _marginalSets[ id ];
    ...
    template< typename GUM_SCALAR >
    const typename Property< std::vector< NodeId > >::onNodes& InferenceEngine< GUM_SCALAR >::getT0Cluster() const {
      return _t0;
    }

    template< typename GUM_SCALAR >
    const typename Property< std::vector< NodeId > >::onNodes& InferenceEngine< GUM_SCALAR >::getT1Cluster() const {
      return _t1;
    }
