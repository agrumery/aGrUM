...
template <typename GUM_SCALAR>
Tensor<GUM_SCALAR>
Counterfactual<GUM_SCALAR>::_adaptToOriginalVariables_(const Tensor<GUM_SCALAR>& adj,
...
void Counterfactual<GUM_SCALAR>::print(std::ostream& os) const {
  ...
  os << " on = {";
  { bool first = true; for (const auto& s : _on) { if (!first) os << ", "; os << s; first = false; } }
  os << "}\n whatif = {";
  { bool first = true; for (const auto& s : _whatif) { if (!first) os << ", "; os << s; first = false; } }
  os << "}\n";

  if (!_profile.empty()) {
    os << " profile: ";
    bool first = true;
    for (const auto& kv : _profile) { if (!first) os << ", "; os << kv.first << "=" << kv.second; first = false; }
    os << "\n";
  }
  ...
  if (!_values.empty()) {
    os << " values: ";
    bool first = true;
    for (const auto& kv : _values) { if (!first) os << ", "; os << kv.first << "=" << kv.second; first = false; }
    os << "\n";
  }
...
Counterfactual<GUM_SCALAR>::_idAssignToNameAssign_(const CausalModel<GUM_SCALAR>& cm,
