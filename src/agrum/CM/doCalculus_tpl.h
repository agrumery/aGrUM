...
DoCalculus<GUM_SCALAR>::getBackDoorTree(NodeId cause,
  ...
  Set<std::string> knw;    knw.insert(xName);
  for (const auto& zn : zNames) knw.insert(zn);
...
std::string DoCalculus<GUM_SCALAR>::_hedgeExceptionMsg(const CausalModel<GUM_SCALAR>& cm,
  ...
  auto namesOf = [&](const NodeSet& Ns) {
    ...
    out.reserve(Ns.size());
    for (auto id : Ns) out.push_back(cm.nameFromId(id));
...
DoCalculus<GUM_SCALAR>::_cDecomposition_(const CausalModel<GUM_SCALAR>& cm) const {
  ...
  for (auto v : V) adj[v]; // ensure key exists
...
DoCalculus<GUM_SCALAR>::_topoObserved_(const CausalModel<GUM_SCALAR>& cm) const {
  ...
  for (auto v : V) {
    Size cnt = 0;
    for (auto p : cm.parents(v)) if (!lat.contains(p)) ++cnt;
    indeg[v] = cnt;
  }
...
DAG DoCalculus<GUM_SCALAR>::_removeIncomingInto_(const DAG& dag, const NodeSet& X) {
  ...
  for (auto x : X) {
    ...
    for (auto p : ps) g.eraseArc(Arc(p, x));
  }
  return g;
}
...
DAG DoCalculus<GUM_SCALAR>::_removeInIntoDoing_outOfKnowing_(const DAG& dag,
  ...
  for (auto z : knowing) {
    ...
    for (auto c : ch) g.eraseArc(Arc(z, c));
  }
  return g;
}
...
template <typename GUM_SCALAR>
typename DoCalculus<GUM_SCALAR>::FormulaPtr
DoCalculus<GUM_SCALAR>::identifyingIntervention(const NameList& Y, const NameList& X) const {
  NodeSet YY, XX;
  for (const auto& s : Y) YY.insert(_cm.idFromName(s));
  for (const auto& s : X) XX.insert(_cm.idFromName(s));
...
DoCalculus<GUM_SCALAR>::doCalculusWithObservation(const NodeSet& on,
  ...
  NameList Yw, W;
  for (auto y : on)      Yw.push_back(_cm.nameFromId(y));
  for (auto w : knowing) W.push_back(_cm.nameFromId(w));

  NameList Xn;
  for (auto x : doing) Xn.push_back(_cm.nameFromId(x));
...
DoCalculus<GUM_SCALAR>::doCalculusWithObservation(const NameList& on,
  ...
  for (const auto& s : doing)   DO.insert(_cm.idFromName(s));
  for (const auto& s : knowing) KN.insert(_cm.idFromName(s));
...
DoCalculus<GUM_SCALAR>::_ID_(const CausalModel<GUM_SCALAR>& cm,
  ...
  if (X.empty()) {
    if (!P) {
      // return P(Y)
      Set<std::string> Yn;
      for (auto y : Y) Yn.insert(cm.nameFromId(y));
    ...
    for (auto v : Vy) tosum.push_back(cm.nameFromId(v));
    ...
    if (Aobs.size() != V.size()) {
      ...
      if (P) {
        ...
        if (!VminusA.empty()) {
          ...
          for (auto v : VminusA) tosum.push_back(cm.nameFromId(v));
  ...
  // --- 4) c-decompose G[V\X]; if >1 component, multiply and sum ---
  {
    ...
    if (icomp.size() > 1) {
      ...
      for (size_t i = 0; i < icomp.size(); ++i) {
        NodeSet Si = icomp[i];
        NameList Sn;
        for (auto s : Si) Sn.push_back(cm.nameFromId(s));

        std::unique_ptr<ASTtree<GUM_SCALAR>> Pcopy = P ? std::unique_ptr<ASTtree<GUM_SCALAR>>(P->copy()) : nullptr;
      ...
      for (auto v : Vyx) tosum.push_back(cm.nameFromId(v));
    ...
    if (cdg.size() == 1) {
      ...
      if (all.size() == V.size()) {
        ...
        for (auto v : V) Vn.insert(cm.nameFromId(v));
        for (auto s : S) Sn.insert(cm.nameFromId(s));
    ...
    // --- 6) If S is also a c-component in G: product of local conditionals over S
    {
      ...
      if (sInCdg) {
        ...
        for (Size i = 0; i < order.size(); ++i) pos[order[i]] = i;

        // build in topo order filtered to S
        std::vector<NodeId> ordering;
        ordering.reserve(S.size());
        for (auto v : order) if (S.contains(v)) ordering.push_back(v);
        ...
        for (auto v : ordering) {
          ...
          if (!P) {
            ...
            for (Size i = 0; i < k; ++i) condNames.insert(cm.nameFromId(order[i]));
            Set<std::string> lhs; lhs.insert(cm.nameFromId(v));
            ...
            auto den = std::make_unique<ASTsum<GUM_SCALAR>>(tosum,
        ...
        for (auto v : Sy) tosum.push_back(cm.nameFromId(v));
    ...
    // --- 7) Else: pick a c-component S' of G, build Q[S'] then recurse on G[S']
    {
      ...
      bool found = false;
      for (const auto& comp : cdg) {
        if ((S - comp).empty()) { Spr = comp; found = true; break; }
      }
      if (!found) {
      ...
      for (auto v : order) if (Spr.contains(v)) ordering.push_back(v);
