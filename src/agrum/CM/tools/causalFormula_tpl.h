...
void CausalFormula<GUM_SCALAR>::_ensureVariablesExist() const {
  ...
  for (const auto& id : _on)      (void)bn.variable(id);
  for (const auto& id : _doing)   (void)bn.variable(id);
  for (const auto& id : _knowing) (void)bn.variable(id);
}
...
CausalFormula<GUM_SCALAR>::CausalFormula(const CausalModel<GUM_SCALAR>& cm,
  ...
  : _cm(cm),
    _root(std::move(root)),
    _on(_toNodeSetFromNames_(cm, on)),
    _doing(_toNodeSetFromNames_(cm, doing)),
    _knowing(_toNodeSetFromNames_(cm, knowing)),
...
std::string CausalFormula<GUM_SCALAR>::latexQuery() const {
  ...
  auto namesSorted = [&](const NodeSet& S) {
    ...
    res.reserve(S.size());
    for (const auto& id : S) res.emplace_back(bn.variable(id).name());
  ...
  if (!doingNames.empty() || !knowingNames.empty()) {
    ...
    if (!knowingNames.empty()) {
      ...
      for (size_t i = 1; i < knowingNames.size(); ++i) ss << "," << knowingNames[i];
    }
  }
...
std::unique_ptr<CausalFormula<GUM_SCALAR>> CausalFormula<GUM_SCALAR>::copy() const {
  return std::make_unique<CausalFormula<GUM_SCALAR>>(
      _cm, _root->copy(), _on, _doing, _knowing);
}
...
std::vector<std::string> CausalFormula<GUM_SCALAR>::onNames() const {
  ...
  for (const auto& id : _on) out.emplace_back(bn.variable(id).name());
...
std::vector<std::string> CausalFormula<GUM_SCALAR>::doingNames() const {
  ...
  for (const auto& id : _doing) out.emplace_back(bn.variable(id).name());
  std::sort(out.begin(), out.end());
  return out;
}
...
std::vector<std::string> CausalFormula<GUM_SCALAR>::knowingNames() const {
  ...
  for (const auto& id : _knowing) out.emplace_back(bn.variable(id).name());
  std::sort(out.begin(), out.end());
  return out;
}
