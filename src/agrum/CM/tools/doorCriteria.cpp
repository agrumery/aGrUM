...
namespace gum {
  ...
  DoorCriteria::NodeSetVec DoorCriteria::enumerateBackdoorSets(NodeId                    X,
                                                               NodeId                    Y,
  ...
  DoorCriteria::NodeSetVec DoorCriteria::enumerateBackdoorSets(NodeId X, NodeId Y) const {
    return enumerateBackdoorSets(X, Y, false);
  }

  DoorCriteria::NodeSetVec DoorCriteria::enumerateFrontdoorSets(NodeId                    X,
                                                                NodeId                    Y,
                                                                const EnumerationOptions& opts,
                                                                bool stopAtFirst) const {
    NodeSetVec out;
    ...
    if (possible.empty()) {
      ...
      NodeSet cc;
      {
        ...
        while (!st.empty()) {
          NodeId u = st.back();
          st.pop_back();
          for (auto v: _dag.parents(u))
            if (!cc.contains(v)) {
              cc.insert(v);
              st.push_back(v);
            }
          for (auto v: _dag.children(u))
            if (!cc.contains(v)) {
              cc.insert(v);
              st.push_back(v);
            }
  ...
  // Backward compatibility: default stopAtFirst = false
  DoorCriteria::NodeSetVec DoorCriteria::enumerateFrontdoorSets(NodeId X, NodeId Y) const {
