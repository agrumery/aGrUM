...
the original visibility-tagging pass) for each known tag and
...
"right order of magnitude" precision as the rest of this project's
visibility-tagging tooling) qualifies each name with its enclosing namespace so autolink has a
fair chance even for common short names.

Usage:
    python src/cmake/generate_visibility_tags_doc.py
    python src/cmake/generate_visibility_tags_doc.py --check   # exit 1 if the
                                                             # generated file
                                                             # would change
"""
...
def strip_line_comments(line: str, in_block: bool) -> tuple[str, bool]:
    """Strip `//` and `/* ... */` comments from `line` -- without this, braces
...
def render(results: dict[str, list[tuple[str, str, int]]]) -> str:
    header = Path(__file__).resolve().parent / "_dox_license_header.txt"
    ...
    body = [
        ...
        " * hand -- regenerate with `python src/cmake/generate_visibility_tags_doc.py`).",
        " */",
        "",
        "/*!",
        " * \\page visibility_tags_summary Visibility tags",
        " *",
        " * Auto-generated by `src/cmake/generate_visibility_tags_doc.py` (also run",
