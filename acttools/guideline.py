...
from .utils import notif, error, warn, recglob, srcAgrum, srcPyAgrum, srcGeneratorAgrum, srcCmakeAgrum, notif_oneline
...
def guideline(current: dict[str, str], details=False, correction=False) -> int:
    ...
    notif(
        f"[aGrUM {'detailed ' if details else ""}guideline {'(with correction)' if correction else ''}]")

    nbrError = 0

    notif("  [(1) ]*.cpp[ file for every ]*.h[ file]")
    nbrError += _aff_errors(_checkCppFileExists(details, correction),
                            "missing cpp file")
    notif("  [(2) check for ]LGPL+MIT[ license]")
    nbrError += _aff_errors(_checkLGPL_MIT_license_CPP(details, correction),
                            "missing LGPL+MIT cpp licence")
    nbrError += _aff_errors(_checkLGPL_MIT_license_py(details, correction),
                            "missing LGPL+MIT python licence")
    notif("  [(3) check for missing documentation in pyAgrum]")
    nbrError += _aff_errors(_checkMissingDocs(details),
                            "missing documentation")
    notif("  [(4) check for deps]")
    nbrError += _aff_errors(check_gum_dependencies(graph=current['build_graph'], details=details, correction=correction),
