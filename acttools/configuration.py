...
def initParams():
  ...
  cfg.swapOptions = {"verbose": {True: "verbose", False: "quiet"}, "withSQL": {True: "withSQL", False: "withoutSQL"}}


def configureOptions(current: dict[str, str]):
  cfg.parser = ArgumentParser(
    prog="act", description="Compilation tools for aGrUM and wrappers", epilog="[%(prog)s v" + cfg.act_version + "]"
  )
  ...
  cfg.parser.add_argument("--no-fun", help="no fancy output parser.", action="store_true", dest="no_fun", default=False)
  cfg.parser.add_argument(
    "-v",
    "--verbose",
  ...
  cfg.parser.add_argument(
    "-q", "--quiet", help="please be quiet.", action="store_false", dest="verbose", default=current["verbose"]
  )
  cfg.parser.add_argument(
    "--withSQL",
  ...
  cfg.parser.add_argument(
    "--static_lib", help="build static library.", action="store_true", dest="static_lib", default=False
  )
  cfg.parser.add_argument(
    "--python3target", help="python3 target.", metavar="PATH", dest="python3target", default=current["python3target"]
  )
  cfg.parser.add_argument(
    "--clangpath", help="path for clang.", metavar="FOLDER", dest="clangpath", default=current["clangpath"]
  )
  cfg.parser.add_argument(
    "--gccpath", help="path for gcc.", metavar="FOLDER", dest="gccpath", default=current["gccpath"]
  )
  cfg.parser.add_argument(
    "-tt",
    "--threads",
  ...
  cfg.parser.add_argument(
    "--profiling", help="act will add options for profiling", action="store_true", dest="profiling", default=False
  )
  cfg.parser.add_argument(
    "--stable_abi_off", help="build wheel without stable abi", action="store_true", dest="stable_abi_off", default=False
  )
