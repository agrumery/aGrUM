...
def _build_ancestral(
    ancestors: dict[str, dict[str, int]], deps: dict[str, list[str]], k: str
) -> dict[str, int]:
...
def _simplify_dependencies(
    ancestors: dict[str, dict[str, int]], deps: dict[str, list[str]]
):
    nbr = 0
    for k in deps.keys():
        first = 0
    ...
    for col in colors:
        if col != "legend":
            if col.startswith("base"):
                deps_graph.add_edge(pdp.Edge("legend_base", col))
            else:
...
def check_gum_dependencies(
    graph: bool = True, details: bool = True, correction: bool = False
):
