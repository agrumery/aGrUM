...
def draw_gum_dependencies(deps: dict[str, list[str]]):
  ...
  colors = {
    ...
    "base/database": ("blues9", "#8899AA"),
    "base/variables": ("blues9", "#8899BB"),
    "base/graphicalModels": ("blues9", "#99AACC"),
    "base/graphs": ("blues9", "#99AADD"),
    "base/multidim": ("blues9", "#AAAAEE"),
    "base/stattests": ("blues9", "#AABBFF"),
    "base/io": ("blues9", "#BBCCFF"),
    "CN": ("set38", 1),
    "PRM": ("set38", 2),
    "MRF": ("set38", 3),
    "BN": ("set38", 4),
  ...
  for col in colors:
    if not col.startswith("legend"):
      if col.startswith("base"):
        deps_graph.add_node(_get_node(col, col[5:]))
      else:
        deps_graph.add_node(_get_node(name=col, label=col))

  deps_graph.add_node(_get_node("legend", "pyAgrum"))
  deps_graph.add_node(_get_node("legend_base", label="base"))

  for col in colors:
    if col != "legend":
      if col.startswith("base"):
        deps_graph.add_edge(pdp.Edge("legend_base", col, arrowsize=asize))
      else:
        deps_graph.add_edge(pdp.Edge("legend", col, arrowsize=asize))
  ...
  for k in deps.keys():
    for nei in deps[k]:
      deps_graph.add_edge(pdp.Edge(nei, k, arrowsize=asize))

  notif("    - drawing in pdf (please be patient...)")
  with open("agrum-map.dot", "w") as f:
    f.write(deps_graph.to_string())
